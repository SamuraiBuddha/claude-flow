...
const { colors } = { colors: chalk_1.default }; // Compatibility shim
const agent_manager_js_1 = require("../../agents/agent-manager.js");
const event_bus_js_1 = require("../../core/event-bus.js");
const logger_js_1 = require("../../core/logger.js");
const distributed_memory_js_1 = require("../../memory/distributed-memory.js");
...
async function initializeAgentManager() {
    ...
    const memorySystem = new distributed_memory_js_1.DistributedMemorySystem({}, // Use default config
    logger, eventBus);
    await memorySystem.initialize();
    agentManager = new agent_manager_js_1.AgentManager({
        maxAgents: 100,
...
function createAgentCommand() {
    const agentCommand = new commander_1.Command('agent')
        .description('Comprehensive Claude-Flow agent management with advanced features')
        .action(() => {
        console.log(chalk_1.default.cyan('🤖 Claude-Flow Agent Management System'));
        console.log('');
    ...
    agentCommand
        ...
        try {
            ...
            console.log('\n' + chalk_1.default.cyan('System Overview:'));
            console.log(`Total Agents: ${stats.totalAgents} | Active: ${stats.activeAgents} | Healthy: ${stats.healthyAgents}`);
            console.log(`Average Health: ${(0, formatters_js_1.formatPercentage)(stats.averageHealth)} | Pools: ${stats.pools}`);
        }
        catch (error) {
            console.error(chalk_1.default.red('Error listing agents:'), error instanceof Error ? error.message : String(error));
            process.exit(1);
        }
    });
    // Spawn command
    agentCommand
        ...
        try {
            ...
            if (options.config) {
                const configPath = node_path_1.default.resolve(options.config);
            ...
            if (options.interactive) {
                ...
                if (!templateName) {
                    console.error(chalk_1.default.red('Error: Template name is required. Use --interactive for guided setup.'));
                    return;
                }
                ...
                if (!selectedTemplate) {
                    console.error(chalk_1.default.red(`Template '${templateName}' not found.`));
            ...
            console.log(`Agent ID: ${chalk_1.default.bold(agentId)}`);
            ...
            if (options.pool) {
                ...
                if (targetPool) {
                    ...
                    console.log(chalk_1.default.blue(`Added to pool: ${targetPool.name}`));
                }
                else {
                    console.log(chalk_1.default.yellow(`Warning: Pool '${options.pool}' not found`));
                }
            }
            // Start agent if requested
            if (options.start) {
                console.log(chalk_1.default.cyan('Starting agent...'));
                await manager.startAgent(agentId);
                console.log(chalk_1.default.green('✅ Agent started and ready!'));
            }
            else {
                console.log(chalk_1.default.yellow(`Use 'claude-flow agent start ${agentId}' to start the agent`));
            }
            // Display agent info
            ...
            console.error(chalk_1.default.red('Error creating agent:'), error instanceof Error ? error.message : String(error));
            process.exit(1);
        }
    });
...
exports.agentCommand = createAgentCommand();
...
function displayDetailedAgentList(agents, manager) {
    agents.forEach((agent, index) => {
        if (index > 0)
            console.log('\n' + '-'.repeat(60));
        console.log(`\n${chalk_1.default.bold(agent.name)} (${agent.id.id.slice(-8)})`);
        console.log(`Type: ${chalk_1.default.blue(agent.type)} | Status: ${getStatusDisplay(agent.status)}`);
        ...
        if (agent.metrics) {
            ...
            console.log(`Success Rate: ${(0, formatters_js_1.formatPercentage)(agent.metrics.successRate)}`);
            console.log(`CPU: ${(0, formatters_js_1.formatPercentage)(agent.metrics.cpuUsage)} | Memory: ${(0, formatters_js_1.formatBytes)(agent.metrics.memoryUsage)}`);
        }
        const health = manager.getAgentHealth(agent.id.id);
        if (health && health.issues.length > 0) {
            console.log(chalk_1.default.red(`Issues: ${health.issues.length} active`));
        }
    });
}
function displayAgentSummary(agent) {
    console.log('\n' + chalk_1.default.dim('Agent Summary:'));
...
function getStatusDisplay(status) {
    ...
    return `${color}${status.toUpperCase()}${chalk_1.default.reset}`;
}
function getHealthDisplay(health) {
    ...
    return `${color}${percentage}%${chalk_1.default.reset}`;
}
