...
exports.helpCommand = new commander_1.Command()
    .name('help')
...
function showMainHelp() {
    ...
    console.log(help_formatter_js_1.HelpFormatter.formatHelp(mainHelp));
}
function showCommandHelp(command) {
    const commandHelp = getCommandHelp(command);
    if (commandHelp) {
        console.log(help_formatter_js_1.HelpFormatter.formatHelp(commandHelp));
    }
    else {
        console.error(help_formatter_js_1.HelpFormatter.formatError(`Unknown command: ${command}`, 'claude-flow help', 'claude-flow help [command]'));
    }
}
...
function showAllTopics() {
    console.log(chalk_1.default.cyan.bold('All Help Topics'));
    ...
    for (const topic of HELP_TOPICS) {
        table.push([chalk_1.default.cyan(topic.name), chalk_1.default.yellow(topic.category), topic.description]);
    }
    console.log(table.toString());
    console.log();
    console.log(chalk_1.default.gray('Use "claude-flow help <topic>" for detailed information.'));
}
async function showTopicHelp(topicName, options) {
    const topic = HELP_TOPICS.find((t) => t.name === topicName);
    if (!topic) {
        console.log(chalk_1.default.red(`Help topic '${topicName}' not found.`));
        console.log();
        // Suggest similar topics
        const similar = HELP_TOPICS.filter((t) => t.name.includes(topicName) || t.description.toLowerCase().includes(topicName.toLowerCase()));
        if (similar.length > 0) {
            console.log(chalk_1.default.gray('Did you mean:'));
            for (const suggestion of similar) {
                console.log(chalk_1.default.cyan(`  ${suggestion.name}`));
            }
        }
        else {
            console.log(chalk_1.default.gray('Use "claude-flow help --all" to see all topics.'));
        }
        return;
    }
    console.log(chalk_1.default.cyan.bold(`Help: ${topic.name}`));
    console.log('─'.repeat(50));
    console.log(chalk_1.default.white(topic.description));
    console.log();
    if (options.tutorial && topic.tutorial) {
        console.log(chalk_1.default.yellow.bold('Tutorial:'));
    ...
    if (options.examples && topic.examples) {
        ...
        for (const example of topic.examples) {
            console.log(chalk_1.default.white.bold(`${example.description}:`));
            console.log(chalk_1.default.cyan(`  ${example.command}`));
            if (example.explanation) {
                console.log(chalk_1.default.gray(`  ${example.explanation}`));
            }
            console.log();
        }
    }
    if (!options.examples && !options.tutorial) {
        ...
        if (topic.tutorial) {
            console.log(chalk_1.default.yellow.bold('Overview:'));
            console.log('─'.repeat(20));
            ...
            console.log();
            console.log(chalk_1.default.gray('Use --tutorial for complete tutorial.'));
            console.log();
        }
        if (topic.examples) {
            console.log(chalk_1.default.yellow.bold('Common Examples:'));
            console.log('─'.repeat(20));
            ...
            if (topic.examples.length > 3) {
                console.log(chalk_1.default.gray(`    ... and ${topic.examples.length - 3} more`));
            }
            console.log();
            console.log(chalk_1.default.gray('Use --examples for all examples.'));
            console.log();
        }
    }
    if (topic.related && topic.related.length > 0) {
        console.log(chalk_1.default.yellow.bold('Related Topics:'));
        console.log('─'.repeat(20));
        for (const related of topic.related) {
            console.log(chalk_1.default.cyan(`  claude-flow help ${related}`));
        }
        console.log();
    }
}
async function startInteractiveHelp() {
    console.log(chalk_1.default.cyan.bold('Interactive Help Mode'));
