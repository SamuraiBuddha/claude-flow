...
async function hiveAction(ctx) {
    ...
    if (!objective) {
        (0, cli_core_js_1.error)('Usage: hive <objective> [options]');
        showHiveHelp();
        return;
    }
    const options = {
    ...
    const hiveId = (0, helpers_js_1.generateId)('hive');
    (0, cli_core_js_1.success)(`🐝 Initializing Hive Mind: ${hiveId}`);
    ...
    try {
        ...
        if (options.sparc) {
            (0, cli_core_js_1.info)('🧪 SPARC methodology enabled - full TDD workflow');
        ...
        if (!options.background) {
            ...
            (0, cli_core_js_1.success)(`✅ Hive Mind ${hiveId} completed successfully`);
        }
    }
    catch (err) {
        (0, cli_core_js_1.error)(`Hive Mind error: ${err.message}`);
    }
}
...
async function conductConsensusRound(memory, agents, phase, context) {
    const roundId = (0, helpers_js_1.generateId)('round');
    // Store round context
