...
const { bold, blue, yellow } = chalk_2.default;
const orchestrator_fixed_js_1 = require("../../core/orchestrator-fixed.js");
const config_js_1 = require("../../core/config.js");
const event_bus_js_1 = require("../../core/event-bus.js");
const logger_js_1 = require("../../core/logger.js");
const json_persistence_js_1 = require("../../core/json-persistence.js");
const swarm_js_1 = require("./swarm.js");
const memory_js_1 = require("./memory.js");
const sparc_js_1 = require("./sparc.js");
...
async function getPersistence() {
    if (!persistence) {
        persistence = new json_persistence_js_1.JsonPersistenceManager();
...
async function getOrchestrator() {
    if (!orchestrator) {
        ...
        orchestrator = new orchestrator_fixed_js_1.Orchestrator(config, eventBus, logger);
    }
    return orchestrator;
}
async function getConfigManager() {
    if (!configManager) {
        configManager = config_js_1.ConfigManager.getInstance();
...
function setupCommands(cli) {
    ...
    cli.command({
        ...
        action: async (ctx) => {
            try {
                (0, cli_core_js_1.success)('Initializing Claude Code integration files...');
                ...
                if (existingFiles.length > 0 && !force) {
                    (0, cli_core_js_1.warning)(`The following files already exist: ${existingFiles.join(', ')}`);
                ...
                (0, cli_core_js_1.success)('Claude Code integration files initialized successfully!');
                console.log('\nNext steps:');
                ...
                (0, cli_core_js_1.error)(`Failed to initialize files: ${err.message}`);
            }
        },
    });
    // Start command
    cli.command({
        ...
        action: async (ctx) => {
            (0, cli_core_js_1.success)('Starting Claude-Flow orchestration system...');
            try {
                const orch = await getOrchestrator();
                await orch.start();
                (0, cli_core_js_1.success)('System started successfully!');
                (0, cli_core_js_1.info)('Components initialized:');
                ...
                if (!ctx.flags.daemon) {
                    (0, cli_core_js_1.info)('Press Ctrl+C to stop the system');
                ...
                (0, cli_core_js_1.error)(`Failed to start system: ${err.message}`);
                process.exit(1);
            }
        },
    });
            ...
            switch (subcommand) {
                case 'create': {
                    ...
                    try {
                        ...
                        (0, cli_core_js_1.success)(`Task created successfully!`);
                        console.log(`📝 Task ID: ${taskId}`);
                        console.log(`🎯 Type: ${type}`);
                        console.log(`📄 Description: ${description}`);
                    }
                    catch (err) {
                        (0, cli_core_js_1.error)(`Failed to create task: ${err.message}`);
                    }
                    break;
                }
                case 'list': {
                    try {
                        ...
                        if (tasks.length === 0) {
                            (0, cli_core_js_1.info)('No active tasks');
                        }
                        else {
                            (0, cli_core_js_1.success)(`Active tasks (${tasks.length}):`);
                        ...
                        (0, cli_core_js_1.error)(`Failed to list tasks: ${err.message}`);
                    }
                    break;
                }
                case 'assign': {
                    ...
                    if (!taskId || !agentId) {
                        (0, cli_core_js_1.error)('Usage: task assign <task-id> <agent-id>');
                        break;
                    }
                    try {
                        const persist = await getPersistence();
                        ...
                        if (!task) {
                            (0, cli_core_js_1.error)(`Task not found: ${taskId}`);
                            break;
                        }
                        if (!agent) {
                            (0, cli_core_js_1.error)(`Agent not found: ${agentId}`);
                            break;
                        }
                        ...
                        (0, cli_core_js_1.success)(`Task ${taskId} assigned to agent ${agentId}`);
                        console.log(`📝 Task: ${task.description}`);
                        console.log(`🤖 Agent: ${agent.name} (${agent.type})`);
                    }
                    catch (err) {
                        (0, cli_core_js_1.error)(`Failed to assign task: ${err.message}`);
                    }
                    break;
                }
                case 'workflow': {
                    ...
                    if (!workflowFile) {
                        (0, cli_core_js_1.error)('Usage: task workflow <workflow-file>');
                        break;
                    }
                    try {
                        ...
                        (0, cli_core_js_1.success)('Workflow loaded:');
                        ...
                        if (ctx.flags.execute) {
                            (0, cli_core_js_1.warning)('Workflow execution would start here (not yet implemented)');
                            // TODO: Implement workflow execution
                        }
                        else {
                            (0, cli_core_js_1.info)('To execute this workflow, ensure Claude-Flow is running');
                        }
                    }
                    catch (err) {
                        (0, cli_core_js_1.error)(`Failed to load workflow: ${err.message}`);
                    }
                    break;
                }
                default: {
    ...
    cli.command({
        ...
        action: async (ctx) => {
            ...
            const { agentCommand } = await Promise.resolve().then(() => __importStar(require('./agent.js')));
            ...
            try {
                ...
                switch (subcommand) {
                    ...
                    case 'health':
                        ...
                        console.log(chalk_1.default.cyan('🚀 Using enhanced agent management system...'));
                        // Create a simplified wrapper around the enhanced command
                        const agentManager = await Promise.resolve().then(() => __importStar(require('../../agents/agent-manager.js')));
                        const { MemoryManager } = await Promise.resolve().then(() => __importStar(require('../../memory/manager.js')));
                        const { EventBus } = await Promise.resolve().then(() => __importStar(require('../../core/event-bus.js')));
                        const { Logger } = await Promise.resolve().then(() => __importStar(require('../../core/logger.js')));
                        const { DistributedMemorySystem } = await Promise.resolve().then(() => __importStar(require('../../memory/distributed-memory.js')));
                        (0, cli_core_js_1.warning)('Enhanced agent management is available!');
                        ...
                        break;
                    default: {
                        console.log(chalk_1.default.cyan('📋 Agent Management Commands:'));
                ...
                (0, cli_core_js_1.error)(`Enhanced agent management unavailable: ${err.message}`);
                ...
                switch (subcommand) {
                    ...
                    case 'list': {
                        try {
                            ...
                            if (agents.length === 0) {
                                (0, cli_core_js_1.info)('No active agents');
                            }
                            else {
                                (0, cli_core_js_1.success)(`Active agents (${agents.length}):`);
    ...
    try {
        ...
        const enhancedStatusAction = async (ctx) => {
            ...
            console.log(chalk_1.default.cyan('🔍 Enhanced Status Command'));
            ...
            try {
                ...
                const isRunning = await access('orchestrator.log')
                    .then(() => true)
                    .catch(() => false);
                (0, cli_core_js_1.success)('Claude-Flow System Status:');
                ...
                if (options.watch) {
                    (0, cli_core_js_1.warning)('Watch mode available in enhanced status command');
        ...
        (0, cli_core_js_1.warning)('Enhanced status command not available, using basic version');
            ...
            switch (subcommand) {
                case 'start': {
                    ...
                    try {
                        ...
                        if (!health.healthy) {
                            (0, cli_core_js_1.warning)("Orchestrator is not running. Start it first with 'claude-flow start'");
                            return;
                        }
                        (0, cli_core_js_1.success)(`MCP server is running as part of the orchestration system`);
                        ...
                        (0, cli_core_js_1.error)(`Failed to check MCP server: ${err.message}`);
                    }
                    break;
                }
                case 'stop': {
                    try {
                        ...
                        const health = await orch.healthCheck();
                        if (!health.healthy) {
                            (0, cli_core_js_1.info)('MCP server is not running');
                        }
                        else {
                            (0, cli_core_js_1.warning)("MCP server runs as part of the orchestrator. Use 'claude-flow stop' to stop the entire system");
                        }
                    }
                    catch (err) {
                        (0, cli_core_js_1.error)(`Failed to check MCP server: ${err.message}`);
                ...
                case 'status': {
                    try {
                        const orch = await getOrchestrator();
                        const health = await orch.healthCheck();
                        (0, cli_core_js_1.success)('MCP Server Status:');
                        ...
                        (0, cli_core_js_1.error)(`Failed to get MCP status: ${err.message}`);
                    }
                    break;
                }
                case 'tools': {
                    try {
                        (0, cli_core_js_1.success)('Available MCP Tools:');
                        ...
                        (0, cli_core_js_1.error)(`Failed to list tools: ${err.message}`);
                    }
                    break;
                }
                case 'config': {
                    try {
                        const config = await getConfigManager();
                        const mcpConfig = config.get().mcp;
                        (0, cli_core_js_1.success)('MCP Configuration:');
                        console.log(JSON.stringify(mcpConfig, null, 2));
                    }
                    catch (err) {
                        (0, cli_core_js_1.error)(`Failed to show MCP config: ${err.message}`);
                    }
                    break;
                }
                case 'restart': {
                    try {
                        (0, cli_core_js_1.warning)("MCP server runs as part of the orchestrator. Use 'claude-flow stop' then 'claude-flow start' to restart the entire system");
                    }
                    catch (err) {
                        (0, cli_core_js_1.error)(`Failed to restart MCP server: ${err.message}`);
                    }
                    break;
                }
                case 'logs': {
                    ...
                    try {
                        ...
                        (0, cli_core_js_1.success)(`MCP Server Logs (last ${lines} lines):`);
                        ...
                        (0, cli_core_js_1.error)(`Failed to get logs: ${err.message}`);
                    }
                    break;
                }
                default: {
                    (0, cli_core_js_1.error)(`Unknown mcp subcommand: ${subcommand}`);
        ...
        action: async (ctx) => {
            ...
            switch (subcommand) {
                ...
                case 'query': {
                    ...
                    if (!search) {
                        (0, cli_core_js_1.error)('Usage: memory query <search>');
                        break;
                    }
                    try {
                        ...
                        if (results.length === 0) {
                            (0, cli_core_js_1.warning)('No results found');
                            return;
                        }
                        (0, cli_core_js_1.success)(`Found ${results.length} results:`);
                        ...
                        (0, cli_core_js_1.error)(`Failed to query: ${err.message}`);
                    }
                    break;
                }
                case 'export': {
                    ...
                    if (!file) {
                        (0, cli_core_js_1.error)('Usage: memory export <file>');
                        break;
                    }
                    try {
                        await memory.exportData(file);
                        const stats = await memory.getStats();
                        (0, cli_core_js_1.success)('Memory exported successfully');
                        console.log(`📁 File: ${file}`);
                        console.log(`📊 Entries: ${stats.totalEntries}`);
                        console.log(`💾 Size: ${(stats.sizeBytes / 1024).toFixed(2)} KB`);
                    }
                    catch (err) {
                        (0, cli_core_js_1.error)(`Failed to export: ${err.message}`);
                    }
                    break;
                }
                case 'import': {
                    const file = ctx.args[1];
                    if (!file) {
                        (0, cli_core_js_1.error)('Usage: memory import <file>');
                        break;
                    }
                    try {
                        await memory.importData(file);
                        const stats = await memory.getStats();
                        (0, cli_core_js_1.success)('Memory imported successfully');
                        console.log(`📁 File: ${file}`);
                        console.log(`📊 Entries: ${stats.totalEntries}`);
                        console.log(`🗂️  Namespaces: ${stats.namespaces}`);
                    }
                    catch (err) {
                        (0, cli_core_js_1.error)(`Failed to import: ${err.message}`);
                    }
                    break;
                }
                case 'stats': {
                    try {
                        const stats = await memory.getStats();
                        (0, cli_core_js_1.success)('Memory Bank Statistics:');
                        ...
                        (0, cli_core_js_1.error)(`Failed to get stats: ${err.message}`);
                    }
                    break;
                }
                case 'cleanup': {
                    try {
                        ...
                        (0, cli_core_js_1.success)('Cleanup completed');
                        console.log(`🗑️  Removed: ${removed} entries older than ${days} days`);
                    }
                    catch (err) {
                        (0, cli_core_js_1.error)(`Failed to cleanup: ${err.message}`);
                    }
                    break;
                }
                default: {
        ...
        action: async (ctx) => {
            ...
            switch (subcommand) {
                case 'spawn': {
                    ...
                    if (!task) {
                        (0, cli_core_js_1.error)('Usage: claude spawn <task description>');
                        break;
                    }
                    try {
...
Now, please proceed with the task: ${task}`;
                        ...
                        if (ctx.flags.dryRun || ctx.flags['dry-run'] || ctx.flags.d) {
                            (0, cli_core_js_1.warning)('DRY RUN - Would execute:');
                        ...
                        (0, cli_core_js_1.success)(`Spawning Claude instance: ${instanceId}`);
                        console.log(`📝 Original Task: ${task}`);
                        console.log(`🔧 Tools: ${tools}`);
                        ...
                        if (status.success) {
                            (0, cli_core_js_1.success)(`Claude instance ${instanceId} completed successfully`);
                        }
                        else {
                            (0, cli_core_js_1.error)(`Claude instance ${instanceId} exited with code ${status.code}`);
                        }
                    }
                    catch (err) {
                        (0, cli_core_js_1.error)(`Failed to spawn Claude: ${err.message}`);
                    }
                    break;
                }
                case 'batch': {
                    const workflowFile = ctx.args[1];
                    if (!workflowFile) {
                        (0, cli_core_js_1.error)('Usage: claude batch <workflow-file>');
                        break;
                    }
                    try {
                        const { readFile } = await Promise.resolve().then(() => __importStar(require('fs/promises')));
                        const content = await readFile(workflowFile, 'utf-8');
                        const workflow = JSON.parse(content);
                        (0, cli_core_js_1.success)(`Loading workflow: ${workflow.name || 'Unnamed'}`);
                        console.log(`📋 Tasks: ${workflow.tasks?.length || 0}`);
                        if (!workflow.tasks || workflow.tasks.length === 0) {
                            (0, cli_core_js_1.warning)('No tasks found in workflow');
                            return;
                        }
                        const promises = [];
                        for (const task of workflow.tasks) {
                            ...
                            if (workflow.parallel) {
                                ...
                                if (!status.success) {
                                    (0, cli_core_js_1.error)(`Task ${taskId} failed with code ${status.code}`);
                                }
                            }
                        }
                        if (workflow.parallel && promises.length > 0) {
                            (0, cli_core_js_1.success)('All Claude instances spawned in parallel mode');
                            const results = await Promise.all(promises);
                            const failed = results.filter((s) => !s.success).length;
                            if (failed > 0) {
                                (0, cli_core_js_1.warning)(`${failed} tasks failed`);
                            }
                            else {
                                (0, cli_core_js_1.success)('All tasks completed successfully');
                            }
                        }
                    }
                    catch (err) {
                        (0, cli_core_js_1.error)(`Failed to process workflow: ${err.message}`);
                    }
                    break;
                }
                default: {
    ...
    try {
        const enhancedMonitorAction = async (ctx) => {
            ...
            console.log(chalk_1.default.cyan('📊 Enhanced Monitor Command'));
            ...
            try {
                ...
                (0, cli_core_js_1.info)('Starting enhanced monitoring dashboard...');
                ...
                while (running) {
                    try {
                        ...
                        const tasks = await persist.getActiveTasks();
                        // Enhanced header
                        (0, cli_core_js_1.success)('Claude-Flow Enhanced Live Monitor');
                        ...
                        (0, cli_core_js_1.error)(`Monitor error: ${err.message}`);
                        await new Promise((resolve) => setTimeout(resolve, interval));
                    }
                }
                ...
                (0, cli_core_js_1.error)(`Failed to start enhanced monitor: ${err.message}`);
            }
        };
        cli.command({
            name: 'monitor',
        ...
        (0, cli_core_js_1.warning)('Enhanced monitor command not available, using basic version');
        ...
        cli.command({
            ...
            action: async (ctx) => {
                ...
                try {
                    ...
                    (0, cli_core_js_1.info)('Starting basic monitoring dashboard...');
                    console.log('Press Ctrl+C to exit');
                    ...
                    while (running) {
                        console.clear();
                        const stats = await persist.getStats();
                        (0, cli_core_js_1.success)('Claude-Flow Live Monitor');
                    ...
                    (0, cli_core_js_1.error)(`Failed to start monitor: ${err.message}`);
                }
            },
        });
    }
    ...
    cli.command({
        ...
        action: async (ctx) => {
            try {
                console.log(chalk_1.default.cyan('🚀 Enhanced SPARC Development Mode'));
                ...
                await (0, sparc_js_1.sparcAction)(ctx);
            }
            catch (err) {
                (0, cli_core_js_1.error)(`Enhanced SPARC failed: ${err.message}`);
            }
        },
    });
    // Migration command
    const migrateCmd = (0, migrate_js_1.createMigrateCommand)();
    ...
    cli.command({
        ...
        action: async (ctx) => {
            // Force UI mode
            ctx.flags.ui = true;
            await (0, swarm_js_1.swarmAction)(ctx);
        },
    });
    // Enhanced session command integration
    try {
        const enhancedSessionAction = async (ctx) => {
            console.log(chalk_1.default.cyan('💾 Enhanced Session Management'));
        ...
        (0, cli_core_js_1.warning)('Enhanced session command not available');
    }
    // Enhanced orchestration start command integration
    try {
        const enhancedStartAction = async (ctx) => {
            console.log(chalk_1.default.cyan('🧠 Enhanced Claude-Flow Orchestration System'));
            ...
            try {
                const orch = await getOrchestrator();
                await orch.start();
                (0, cli_core_js_1.success)('Enhanced orchestration system started!');
                (0, cli_core_js_1.info)('Components initialized with enhanced features:');
                ...
                if (!options.daemon) {
                    (0, cli_core_js_1.info)('Press Ctrl+C to stop the enhanced system');
                    const controller = new AbortController();
                    const shutdown = () => {
                ...
                (0, cli_core_js_1.error)(`Failed to start enhanced system: ${err.message}`);
                process.exit(1);
            }
        };
        ...
        (0, cli_core_js_1.warning)('Enhanced start command not available, using basic version');
    }
    // Help command
    cli.command({
        name: 'help',
    ...
    cli.command({
        ...
        action: async (ctx) => {
            try {
                ...
                const { hiveMindCommand } = await Promise.resolve().then(() => __importStar(require('./hive-mind/index.js')));
                // Execute the appropriate subcommand
                switch (subcommand) {
                    case 'init':
                        const { initCommand } = await Promise.resolve().then(() => __importStar(require('./hive-mind/init.js')));
                        await initCommand.parseAsync(process.argv.slice(3));
                        break;
                    case 'spawn':
                        const { spawnCommand } = await Promise.resolve().then(() => __importStar(require('./hive-mind/spawn.js')));
                        await spawnCommand.parseAsync(process.argv.slice(3));
                        break;
                    case 'status':
                        const { statusCommand } = await Promise.resolve().then(() => __importStar(require('./hive-mind/status.js')));
                        await statusCommand.parseAsync(process.argv.slice(3));
                        break;
                    case 'task':
                        const { taskCommand } = await Promise.resolve().then(() => __importStar(require('./hive-mind/task.js')));
                        await taskCommand.parseAsync(process.argv.slice(3));
                        break;
                    case 'stop':
                        const { stopCommand } = await Promise.resolve().then(() => __importStar(require('./hive-mind/stop.js')));
                        await stopCommand.parseAsync(process.argv.slice(3));
                        break;
                    case 'pause':
                        const { pauseCommand } = await Promise.resolve().then(() => __importStar(require('./hive-mind/pause.js')));
                        await pauseCommand.parseAsync(process.argv.slice(3));
                        break;
                    case 'resume':
                        const { resumeCommand } = await Promise.resolve().then(() => __importStar(require('./hive-mind/resume.js')));
                        await resumeCommand.parseAsync(process.argv.slice(3));
                        break;
                    case 'ps':
                        const { psCommand } = await Promise.resolve().then(() => __importStar(require('./hive-mind/ps.js')));
                        await psCommand.parseAsync(process.argv.slice(3));
                        break;
                    case 'wizard':
                    default:
                        const { wizardCommand } = await Promise.resolve().then(() => __importStar(require('./hive-mind/wizard.js')));
                        await wizardCommand.parseAsync(process.argv.slice(3));
                        break;
                }
            }
            catch (err) {
                (0, cli_core_js_1.error)(`Hive Mind error: ${(0, error_handler_js_1.getErrorMessage)(err)}`);
            }
        },
    });
        ...
        action: async (ctx) => {
            try {
                ...
                await new Promise((resolve, reject) => {
                    ...
                    child.on('error', (err) => {
                        (0, cli_core_js_1.error)(`Failed to execute hook command: ${(0, error_handler_js_1.getErrorMessage)(err)}`);
                        resolve();
                    });
                });
            }
            catch (err) {
                (0, cli_core_js_1.error)(`Hook command error: ${(0, error_handler_js_1.getErrorMessage)(err)}`);
            }
        },
    });
    // Add enterprise commands
    for (const command of enterprise_js_1.enterpriseCommands) {
        cli.command(command);
    }
}
