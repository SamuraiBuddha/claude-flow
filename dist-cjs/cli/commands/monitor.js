...
exports.monitorCommand = new commander_1.Command()
...
class Dashboard {
    ...
    async start() {
        ...
        const cleanup = () => {
            ...
            console.log('\n' + chalk_1.default.gray('Monitor stopped'));
            process.exit(0);
        };
    ...
    renderHeader(data) {
        ...
        console.log(chalk_1.default.cyan.bold('Claude-Flow Live Monitor') + chalk_1.default.gray(` - ${time}`));
        console.log('═'.repeat(80));
    }
    renderSystemOverview(data) {
        console.log(chalk_1.default.white.bold('System Overview'));
        console.log('─'.repeat(40));
        const cpuBar = (0, formatter_js_1.formatProgressBar)(data.system.cpu, 100, 20, 'CPU');
        const memoryBar = (0, formatter_js_1.formatProgressBar)(data.system.memory, 1024, 20, 'Memory');
        console.log(`${cpuBar} ${data.system.cpu.toFixed(1)}%`);
        console.log(`${memoryBar} ${data.system.memory.toFixed(0)}MB`);
        console.log(`${chalk_1.default.white('Agents:')} ${data.system.agents} active`);
        console.log(`${chalk_1.default.white('Tasks:')} ${data.system.tasks} in queue`);
        console.log();
    }
    renderComponentsStatus(data) {
        console.log(chalk_1.default.white.bold('Components'));
        console.log('─'.repeat(40));
        const tableData = [];
    ...
    renderAgentsAndTasks(data) {
        // Agents table
        console.log(chalk_1.default.white.bold('Active Agents'));
        console.log('─'.repeat(40));
        ...
        console.log();
        // Recent tasks
        console.log(chalk_1.default.white.bold('Recent Tasks'));
        console.log('─'.repeat(40));
        if (data.tasks.length > 0) {
            ...
            for (const task of data.tasks.slice(0, 5)) {
                ...
                taskTable.push([
                    chalk_1.default.gray(task.id.substring(0, 8) + '...'),
                    chalk_1.default.white(task.type),
                    `${statusIcon} ${task.status}`,
                    task.duration ? (0, formatter_js_1.formatDuration)(task.duration) : '-',
                ]);
            }
    ...
    renderRecentEvents(data) {
        console.log(chalk_1.default.white.bold('Recent Events'));
        console.log('─'.repeat(40));
        if (data.events.length > 0) {
            for (const event of data.events.slice(0, 3)) {
                const time = new Date(event.timestamp).toLocaleTimeString();
                const icon = this.getEventIcon(event.type);
                console.log(`${chalk_1.default.gray(time)} ${icon} ${event.message}`);
            }
        }
        else {
            console.log(chalk_1.default.gray('No recent events'));
        }
        console.log();
    }
    renderPerformanceGraphs() {
        console.log(chalk_1.default.white.bold('Performance (Last 60s)'));
        console.log('─'.repeat(40));
        if (this.data.length >= 2) {
            // CPU graph
            console.log(chalk_1.default.cyan('CPU Usage:'));
            console.log(this.createSparkline(this.data.map((d) => d.system.cpu), 30));
            // Memory graph
            console.log(chalk_1.default.cyan('Memory Usage:'));
            console.log(this.createSparkline(this.data.map((d) => d.system.memory), 30));
        }
        else {
            console.log(chalk_1.default.gray('Collecting data...'));
        }
        console.log();
    }
    renderFocusedComponent(data, componentName) {
        ...
        console.log(chalk_1.default.white.bold(`${componentName} Details`));
        console.log('─'.repeat(40));
        const statusIcon = (0, formatter_js_1.formatStatusIndicator)(component.status);
        console.log(`${statusIcon} Status: ${component.status}`);
        console.log(`Load: ${(0, formatter_js_1.formatProgressBar)(component.load, 100, 30)} ${component.load.toFixed(1)}%`);
        // Add component-specific metrics here
        console.log();
    }
    renderFooter() {
        console.log('─'.repeat(80));
        console.log(chalk_1.default.gray('Press Ctrl+C to exit • Update interval: ') +
            chalk_1.default.yellow(`${this.options.interval}s`));
    }
    renderError(error) {
        console.clear();
        console.log(chalk_1.default.red.bold('Monitor Error'));
        console.log('─'.repeat(40));
        if (error.message.includes('ECONNREFUSED')) {
            console.log(chalk_1.default.red('✗ Cannot connect to Claude-Flow'));
            console.log(chalk_1.default.gray('Make sure Claude-Flow is running with: claude-flow start'));
        }
        else {
            console.log(chalk_1.default.red('Error:'), error.message);
        }
        console.log('\n' + chalk_1.default.gray('Retrying in ') + chalk_1.default.yellow(`${this.options.interval}s...`));
    }
    ...
    createSparkline(data, width) {
        ...
        return recent
            ...
            return chalk_1.default.cyan(chars[charIndex]);
        })
            .join('');
    }
    getEventIcon(type) {
        const icons = {
            agent_spawned: chalk_1.default.green('↗'),
            agent_terminated: chalk_1.default.red('↙'),
            task_completed: chalk_1.default.green('✓'),
            task_failed: chalk_1.default.red('✗'),
            task_assigned: chalk_1.default.blue('→'),
            system_warning: chalk_1.default.yellow('⚠'),
            system_error: chalk_1.default.red('✗'),
        };
        return icons[type] || chalk_1.default.blue('•');
    }
    generateMockAgents() {
        return [
            {
    ...
    async checkSystemRunning() {
        try {
            return await (0, fs_1.existsSync)('.claude-flow.pid');
        }
        catch {
            return false;
        }
        ...
        try {
            const exportData = {
                metadata: {
                    ...
                    duration: (0, formatter_js_1.formatDuration)(Date.now() - this.startTime),
            ...
            await node_fs_1.promises.writeFile(this.options.export, JSON.stringify(exportData, null, 2));
            console.log(chalk_1.default.green(`✓ Monitoring data exported to ${this.options.export}`));
        }
        catch (error) {
            console.error(chalk_1.default.red('Failed to export data:'), error.message);
        }
    }
}
async function startMonitorDashboard(options) {
    ...
    if (options.interval < 1) {
        console.error(chalk_1.default.red('Update interval must be at least 1 second'));
        return;
    }
    if (options.threshold < 1 || options.threshold > 100) {
        console.error(chalk_1.default.red('Threshold must be between 1 and 100'));
        return;
    }
    if (options.export) {
        // Check if export path is writable
        try {
            await node_fs_1.promises.writeFile(options.export, '');
            await Deno.remove(options.export);
        }
        catch {
            console.error(chalk_1.default.red(`Cannot write to export file: ${options.export}`));
            return;
        }
    }
