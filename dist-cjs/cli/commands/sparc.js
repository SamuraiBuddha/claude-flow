...
exports.sparcAction = sparcAction;
const cli_core_js_1 = require("../cli-core.js");
const chalk_1 = __importDefault(require("chalk"));
const { blue, yellow, green, magenta, cyan } = chalk_1.default;
...
async function listSparcModes(ctx) {
    try {
        ...
        (0, cli_core_js_1.success)('Available SPARC Modes:');
        console.log();
        ...
        if (!verbose) {
            console.log();
            (0, cli_core_js_1.info)('Use --verbose for detailed descriptions');
        }
    }
    catch (err) {
        (0, cli_core_js_1.error)(`Failed to list SPARC modes: ${err.message}`);
    }
}
async function showModeInfo(ctx) {
    const modeSlug = ctx.args[1];
    if (!modeSlug) {
        (0, cli_core_js_1.error)('Usage: sparc info <mode-slug>');
        return;
    }
    try {
        const config = await loadSparcConfig();
        ...
        if (!mode) {
            (0, cli_core_js_1.error)(`Mode not found: ${modeSlug}`);
        ...
        (0, cli_core_js_1.success)(`SPARC Mode: ${mode.name}`);
        console.log();
        ...
        (0, cli_core_js_1.error)(`Failed to show mode info: ${err.message}`);
    }
}
async function runSparcMode(ctx) {
    ...
    if (!modeSlug || !taskDescription) {
        (0, cli_core_js_1.error)('Usage: sparc run <mode-slug> <task-description>');
        return;
    }
    try {
        const config = await loadSparcConfig();
        ...
        if (ctx.flags.dryRun || ctx.flags['dry-run']) {
            (0, cli_core_js_1.warning)('DRY RUN - SPARC Mode Configuration:');
        ...
        (0, cli_core_js_1.success)(`Starting SPARC mode: ${mode.name}`);
        console.log(`📝 Instance ID: ${instanceId}`);
        ...
        (0, cli_core_js_1.error)(`Failed to run SPARC mode: ${err.message}`);
    }
}
async function runTddFlow(ctx) {
    const taskDescription = ctx.args.slice(1).join(' ');
    if (!taskDescription) {
        (0, cli_core_js_1.error)('Usage: sparc tdd <task-description>');
        return;
    }
    try {
        const config = await loadSparcConfig();
        ...
        if (ctx.flags.dryRun || ctx.flags['dry-run']) {
            (0, cli_core_js_1.warning)('DRY RUN - TDD Workflow:');
            for (const step of workflow) {
                console.log(`${cyan(step.phase)}: ${step.mode} - ${step.description}`);
            }
            return;
        }
        (0, cli_core_js_1.success)('Starting SPARC TDD Workflow');
        ...
        for (let i = 0; i < workflow.length; i++) {
            ...
            if (!mode) {
                (0, cli_core_js_1.warning)(`Mode not found: ${step.mode}, skipping step`);
                continue;
            }
            (0, cli_core_js_1.info)(`Phase ${i + 1}/5: ${step.phase} (${mode.name})`);
        ...
        (0, cli_core_js_1.success)('SPARC TDD Workflow completed!');
    }
    catch (err) {
        (0, cli_core_js_1.error)(`Failed to run TDD flow: ${err.message}`);
    }
}
async function runSparcWorkflow(ctx) {
    const workflowFile = ctx.args[1];
    if (!workflowFile) {
        (0, cli_core_js_1.error)('Usage: sparc workflow <workflow-file.json>');
        return;
    }
    try {
        const { readFile } = await Promise.resolve().then(() => __importStar(require('fs/promises')));
        ...
        if (!workflow.steps || !Array.isArray(workflow.steps)) {
            (0, cli_core_js_1.error)("Invalid workflow file: missing 'steps' array");
            return;
        }
        const config = await loadSparcConfig();
        (0, cli_core_js_1.success)(`Loading SPARC workflow: ${workflow.name || 'Unnamed'}`);
        console.log(`📋 Steps: ${workflow.steps.length}`);
        console.log(`📝 Description: ${workflow.description || 'No description'}`);
        console.log();
        if (ctx.flags.dryRun || ctx.flags['dry-run']) {
            (0, cli_core_js_1.warning)('DRY RUN - Workflow Steps:');
        ...
        for (let i = 0; i < workflow.steps.length; i++) {
            ...
            const mode = config.customModes.find((m) => m.slug === step.mode);
            if (!mode) {
                (0, cli_core_js_1.warning)(`Mode not found: ${step.mode}, skipping step ${i + 1}`);
                continue;
            }
            (0, cli_core_js_1.info)(`Step ${i + 1}/${workflow.steps.length}: ${mode.name}`);
        ...
        (0, cli_core_js_1.success)('SPARC workflow completed!');
    }
    catch (err) {
        (0, cli_core_js_1.error)(`Failed to run workflow: ${err.message}`);
    }
}
...
async function executeClaudeWithSparc(enhancedTask, tools, instanceId, flags) {
    ...
    try {
        ...
        if (status.success) {
            (0, cli_core_js_1.success)(`SPARC instance ${instanceId} completed successfully`);
        }
        else {
            (0, cli_core_js_1.error)(`SPARC instance ${instanceId} exited with code ${status.code}`);
        }
    }
    catch (err) {
        (0, cli_core_js_1.error)(`Failed to execute Claude: ${err.message}`);
    }
}
