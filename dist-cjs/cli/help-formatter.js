...
class HelpFormatter {
    ...
    static formatValidationError(value, paramName, validOptions, command) {
        return this.formatError(`'${value}' is not a valid ${paramName}. Valid options are: ${validOptions.join(', ')}.`, command);
    }
    static formatSection(title, content) {
        return `${title}\n${content.map((line) => `${this.INDENT}${line}`).join('\n')}`;
    }
    static formatCommands(commands) {
        const maxNameLength = Math.max(this.MIN_DESCRIPTION_COLUMN, ...commands.map((cmd) => {
    ...
    static formatOptions(options) {
        const maxFlagsLength = Math.max(this.MIN_DESCRIPTION_COLUMN, ...options.map((opt) => opt.flags.length));
