...
const command_registry_js_1 = require("./command-registry.js");
...
function showHelpWithCommands() {
    printHelp();
    console.log('\nRegistered Commands:');
    const commands = (0, command_registry_js_1.listCommands)();
...
async function main() {
    ...
    const { flags, args: parsedArgs } = (0, utils_js_1.parseFlags)(args.slice(1));
    ...
    switch (command) {
        ...
        case '-h':
            if (parsedArgs.length > 0) {
                (0, command_registry_js_1.showCommandHelp)(parsedArgs[0]);
            }
            else {
    ...
    if ((0, command_registry_js_1.hasCommand)(command)) {
        try {
            await (0, command_registry_js_1.executeCommand)(command, parsedArgs, flags);
            return;
        }
        catch (err) {
...
`);
        ...
        config: async (key) => {
            try {
                const config = JSON.parse(await node_fs_1.promises.readFile('claude-flow.config.json', 'utf-8'));
                if (key) {
...
async function createSparcStructureManually() {
    try {
        ...
        try {
            ...
            roomodesContent = await node_fs_1.promises.readFile('.roomodes');
