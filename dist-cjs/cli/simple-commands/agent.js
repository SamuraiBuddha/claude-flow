...
async function agentCommand(subArgs, flags) {
...
async function spawnAgent(subArgs, flags) {
    ...
    (0, utils_js_1.printSuccess)(`Spawning ${agentType} agent: ${agentName}`);
...
async function listAgents(subArgs, flags) {
    (0, utils_js_1.printSuccess)('Active agents:');
...
async function manageHierarchy(subArgs, flags) {
    ...
    switch (hierarchyCmd) {
        case 'create':
            ...
            (0, utils_js_1.printSuccess)(`Creating ${hierarchyType} agent hierarchy`);
            ...
            break;
        case 'show':
            (0, utils_js_1.printSuccess)('Current agent hierarchy:');
...
async function manageNetwork(subArgs, flags) {
    ...
    switch (networkCmd) {
        case 'topology':
            (0, utils_js_1.printSuccess)('Agent network topology:');
            console.log('🌐 Network visualization would show agent connections');
            break;
        case 'metrics':
            (0, utils_js_1.printSuccess)('Network performance metrics:');
...
async function manageEcosystem(subArgs, flags) {
    ...
    switch (ecosystemCmd) {
        case 'status':
            (0, utils_js_1.printSuccess)('Agent ecosystem status:');
            ...
            break;
        case 'optimize':
            (0, utils_js_1.printSuccess)('Optimizing agent ecosystem...');
...
async function provisionAgent(subArgs, flags) {
    ...
    if (!provision) {
        (0, utils_js_1.printError)('Usage: agent provision <count>');
        return;
    }
    const count = parseInt(provision);
    if (isNaN(count) || count < 1) {
        (0, utils_js_1.printError)('Count must be a positive number');
        return;
    }
    (0, utils_js_1.printSuccess)(`Provisioning ${count} agents...`);
...
async function terminateAgent(subArgs, flags) {
    ...
    if (!agentId) {
        (0, utils_js_1.printError)('Usage: agent terminate <agent-id>');
        return;
    }
    (0, utils_js_1.printSuccess)(`Terminating agent: ${agentId}`);
    console.log('🛑 Agent would be gracefully shut down');
}
async function showAgentInfo(subArgs, flags) {
    const agentId = subArgs[1];
    if (!agentId) {
        (0, utils_js_1.printError)('Usage: agent info <agent-id>');
        return;
    }
    (0, utils_js_1.printSuccess)(`Agent information: ${agentId}`);
