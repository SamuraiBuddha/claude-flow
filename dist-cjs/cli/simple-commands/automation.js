...
async function automationAction(subArgs, flags) {
    ...
    try {
        switch (subcommand) {
            ...
            case 'workflow-select':
                ...
                break;
            default:
                (0, utils_js_1.printError)(`Unknown automation command: ${subcommand}`);
                showAutomationHelp();
        }
    }
    catch (err) {
        (0, utils_js_1.printError)(`Automation command failed: ${err.message}`);
    }
}
async function autoAgentCommand(subArgs, flags) {
    ...
    (0, utils_js_1.printSuccess)(`✅ Auto-agent spawning completed`);
...
async function smartSpawnCommand(subArgs, flags) {
    ...
    if (requirement.includes('development') || requirement.includes('coding')) {
        recommendedAgents.push({ type: 'coordinator', count: 1, reason: 'Task orchestration' }, { type: 'coder', count: 3, reason: 'Core development work' }, { type: 'tester', count: 1, reason: 'Quality assurance' });
    }
    if (requirement.includes('research') || requirement.includes('analysis')) {
        recommendedAgents.push({ type: 'researcher', count: 2, reason: 'Information gathering' }, { type: 'analyst', count: 1, reason: 'Data analysis' });
    }
    if (requirement.includes('enterprise') || requirement.includes('production')) {
        recommendedAgents.push({ type: 'coordinator', count: 2, reason: 'Multi-tier coordination' }, { type: 'coder', count: 4, reason: 'Parallel development' }, { type: 'researcher', count: 2, reason: 'Requirements analysis' }, { type: 'analyst', count: 1, reason: 'Performance monitoring' }, { type: 'tester', count: 2, reason: 'Comprehensive testing' });
    }
    // Default fallback
    ...
    (0, utils_js_1.printSuccess)(`✅ Smart spawn analysis completed`);
    ...
    if (totalRecommended > maxAgents) {
        (0, utils_js_1.printWarning)(`⚠️  Recommended configuration exceeds max agents. Consider increasing limit or simplifying requirements.`);
    }
}
async function workflowSelectCommand(subArgs, flags) {
    ...
    (0, utils_js_1.printSuccess)(`✅ Workflow selection completed`);
