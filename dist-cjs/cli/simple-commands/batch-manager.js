...
exports.batchManagerCommand = batchManagerCommand;
// batch-manager.js - Batch configuration management utility
const utils_js_1 = require("../utils.js");
const batch_init_js_1 = require("./init/batch-init.js");
const node_compat_js_1 = require("../node-compat.js");
async function batchManagerCommand(subArgs, flags) {
...
async function createBatchConfig(args, flags) {
    ...
    try {
        await node_compat_js_1.Deno.writeTextFile(outputFile, JSON.stringify(config, null, 2));
        (0, utils_js_1.printSuccess)(`Created batch configuration template: ${outputFile}`);
        console.log('Edit the file to customize your batch initialization setup.');
    }
    catch (error) {
        (0, utils_js_1.printError)(`Failed to create config file: ${error.message}`);
    }
}
async function createInteractiveConfig(outputFile) {
    ...
    const config = {
        ...
        _templates: Object.keys(batch_init_js_1.PROJECT_TEMPLATES),
        _environments: Object.keys(batch_init_js_1.ENVIRONMENT_CONFIGS),
        baseOptions: {
            sparc: true,
            parallel: true,
            maxConcurrency: 5,
    ...
    try {
        await node_compat_js_1.Deno.writeTextFile(outputFile, JSON.stringify(config, null, 2));
        (0, utils_js_1.printSuccess)(`Created interactive batch configuration: ${outputFile}`);
        console.log('\nNext steps:');
        ...
        (0, utils_js_1.printError)(`Failed to create interactive config: ${error.message}`);
    }
}
async function validateBatchConfig(args, flags) {
    const configFile = args[0];
    if (!configFile) {
        (0, utils_js_1.printError)('Please specify a configuration file to validate');
        return;
    }
    try {
        ...
        if (config.baseOptions) {
            ...
            if (template && !batch_init_js_1.PROJECT_TEMPLATES[template]) {
                issues.push(`Unknown template: ${template}. Available: ${Object.keys(batch_init_js_1.PROJECT_TEMPLATES).join(', ')}`);
            }
            if (environments) {
                for (const env of environments) {
                    if (!batch_init_js_1.ENVIRONMENT_CONFIGS[env]) {
                        issues.push(`Unknown environment: ${env}. Available: ${Object.keys(batch_init_js_1.ENVIRONMENT_CONFIGS).join(', ')}`);
                    }
                }
            }
        }
        ...
        if (config.projectConfigs) {
            for (const [projectName, projectConfig] of Object.entries(config.projectConfigs)) {
                if (projectConfig.template && !batch_init_js_1.PROJECT_TEMPLATES[projectConfig.template]) {
                    issues.push(`Project ${projectName}: Unknown template ${projectConfig.template}`);
                }
                if (projectConfig.environment && !batch_init_js_1.ENVIRONMENT_CONFIGS[projectConfig.environment]) {
        ...
        if (issues.length === 0) {
            ...
            (0, utils_js_1.printError)('❌ Configuration has issues:');
        ...
        if (error instanceof node_compat_js_1.Deno.errors.NotFound) {
            (0, utils_js_1.printError)(`Configuration file not found: ${configFile}`);
        }
        else if (error instanceof SyntaxError) {
            (0, utils_js_1.printError)(`Invalid JSON in configuration file: ${error.message}`);
        }
        else {
            (0, utils_js_1.printError)(`Failed to validate configuration: ${error.message}`);
        }
    }
}
function listTemplates() {
    ...
    console.log('==============================\n');
    for (const [key, template] of Object.entries(batch_init_js_1.PROJECT_TEMPLATES)) {
        console.log(`🏗️  ${key}`);
...
function listEnvironments() {
    ...
    console.log('=======================================\n');
    for (const [key, env] of Object.entries(batch_init_js_1.ENVIRONMENT_CONFIGS)) {
        console.log(`⚙️  ${key}`);
...
async function estimateBatchOperation(args, flags) {
    const configFile = args[0];
    if (!configFile) {
        (0, utils_js_1.printError)('Please specify a configuration file to estimate');
        return;
    }
    try {
        const content = await node_compat_js_1.Deno.readTextFile(configFile);
        ...
        (0, utils_js_1.printError)(`Failed to estimate batch operation: ${error.message}`);
    }
}
