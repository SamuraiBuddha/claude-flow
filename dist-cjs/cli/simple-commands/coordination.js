...
exports.coordinationAction = coordinationAction;
const utils_js_1 = require("../utils.js");
// Simple ID generator
function generateId(prefix = 'id') {
    return `${prefix}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
}
async function coordinationAction(subArgs, flags) {
    ...
    try {
        switch (subcommand) {
            ...
            case 'task-orchestrate':
                ...
                break;
            default:
                (0, utils_js_1.printError)(`Unknown coordination command: ${subcommand}`);
                showCoordinationHelp();
        }
    }
    catch (err) {
        (0, utils_js_1.printError)(`Coordination command failed: ${err.message}`);
    }
}
async function swarmInitCommand(subArgs, flags) {
    ...
    if (isAvailable) {
        try {
            ...
            const swarmResult = await (0, utils_js_1.callRuvSwarmMCP)('swarm_init', {
                swarmId: swarmId,
                topology: topology,
                maxAgents: maxAgents,
                strategy: strategy,
            ...
            if (swarmResult.success) {
                (0, utils_js_1.printSuccess)(`✅ Swarm coordination initialized successfully`);
                ...
                (0, utils_js_1.printError)(`Swarm initialization failed: ${swarmResult.error || 'Unknown error'}`);
            }
        }
        catch (err) {
            (0, utils_js_1.printError)(`Swarm initialization failed: ${err.message}`);
    ...
    if (!isAvailable) {
        ...
        (0, utils_js_1.printSuccess)(`✅ Local swarm coordination initialized successfully`);
        console.log(`\n🎯 COORDINATION SETUP COMPLETE:`);
        console.log(`  🐝 Swarm: ${swarmId}`);
        console.log(`  🏗️  Topology: ${topology}`);
        console.log(`  📊 Capacity: ${maxAgents} agents`);
...
async function agentSpawnCommand(subArgs, flags) {
    ...
    if (!validTypes.includes(agentType)) {
        (0, utils_js_1.printWarning)(`⚠️  Unknown agent type '${agentType}'. Using 'general' instead.`);
    ...
    (0, utils_js_1.printSuccess)(`✅ Agent spawned and coordinated successfully`);
...
async function taskOrchestrateCommand(subArgs, flags) {
    ...
    if (!task) {
        (0, utils_js_1.printError)('Task description is required');
        return;
    }
    ...
    await new Promise((resolve) => setTimeout(resolve, 400));
    (0, utils_js_1.printSuccess)(`✅ Task orchestration configured successfully`);
