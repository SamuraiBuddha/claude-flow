...
exports.fixHookVariablesCommandConfig = void 0;
exports.fixHookVariablesCommand = fixHookVariablesCommand;
...
`;
            await promises_1.default.writeFile(path_1.default.join(hooksDir, 'post-edit-hook.sh'), script, { mode: 0o755 });
...
async function fixHookVariables(settingsPath, options = {}) {
    ...
    try {
        // Read settings
        const content = await promises_1.default.readFile(settingsPath, 'utf8');
        const settings = JSON.parse(content);
        if (!settings.hooks) {
            (0, utils_js_1.printWarning)('No hooks found in settings.json');
            return { success: true, changes: 0 };
        }
        // Backup if requested
        if (backup) {
            const backupPath = `${settingsPath}.backup-${Date.now()}`;
            await promises_1.default.writeFile(backupPath, content);
        ...
        console.log(chalk_1.default.blue(`  Using ${targetSyntax} syntax`));
        ...
        if (targetSyntax === 'wrapper' && commands.length > 0) {
            console.log(chalk_1.default.blue('  Creating wrapper scripts...'));
            const scripts = await createWrapperScripts(commands);
            console.log(chalk_1.default.green(`  Created ${scripts.size} wrapper scripts`));
        }
 ...
 */
async function fixHookVariablesCommand(args = [], flags = {}) {
    ...
    if (files.length === 0) {
        (0, utils_js_1.printError)('No settings.json files found');
        console.log('\nSearched locations:');
        console.log('  - .claude/settings.json');
        console.log('  - settings.json');
        console.log('  - .vscode/.claude/settings.json');
        console.log(`  - ${path_1.default.join(process.env.HOME || '', '.claude', 'settings.json')}`);
        return;
    }
    ...
    for (const file of files) {
        console.log(chalk_1.default.cyan(`Processing: ${file}`));
        const result = await fixHookVariables(file, options);
        if (result.success) {
            successCount++;
            totalChanges += result.changes;
            console.log(chalk_1.default.green(`  ✅ Fixed ${result.changes} hook commands`));
        }
        else {
...
async function createTestHook() {
    ...
    await promises_1.default.mkdir('.claude', { recursive: true });
    await promises_1.default.writeFile('.claude/test-settings.json', JSON.stringify(testSettings, null, 2));
...
exports.fixHookVariablesCommandConfig = {
...
//# sourceMappingURL=fix-hook-variables.js.map