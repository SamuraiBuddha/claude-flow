    ...
    if ((0, os_1.platform)() === 'win32') {
        ...
        try {
            ...
            for (const dir of commonPaths) {
                try {
                    await (0, promises_1.access)((0, path_1.join)(dir, command), promises_1.constants.X_OK);
                    return true;
                }
                catch (e) {
...
async function githubCommand(args, flags) {
    ...
    if (!objective) {
        (0, utils_js_1.printError)(`❌ Usage: github ${mode} <objective>`);
    ...
    if (!GITHUB_MODES[mode]) {
        (0, utils_js_1.printError)(`❌ Unknown GitHub mode: ${mode}`);
        console.log('\nAvailable modes:');
        for (const [modeName, info] of Object.entries(GITHUB_MODES)) {
            console.log(`  ${modeName} - ${info.description}`);
        }
    ...
    (0, utils_js_1.printSuccess)(`🐙 GitHub ${mode} mode activated`);
    ...
    try {
        ...
        if (!isClaudeAvailable) {
            (0, utils_js_1.printWarning)('⚠️  Claude CLI not found. GitHub automation requires Claude.');
...
Begin execution now. Create all necessary GitHub workflow files and configuration.`;
        ...
        await new Promise((resolve, reject) => {
            claudeProcess.on('close', (code) => {
                if (code === 0) {
                    (0, utils_js_1.printSuccess)('✅ GitHub automation completed successfully!');
                    resolve();
                }
                else {
        ...
        (0, utils_js_1.printError)(`❌ GitHub automation failed: ${error.message}`);
