...
exports.hiveMindOptimizeCommand = hiveMindOptimizeCommand;
const node_compat_js_1 = require("../node-compat.js");
const path_1 = __importDefault(require("path"));
const fs_1 = require("fs");
const chalk_1 = __importDefault(require("chalk"));
const inquirer_1 = __importDefault(require("inquirer"));
const db_optimizer_js_1 = require("./hive-mind/db-optimizer.js");
/**
 * Show help for hive-mind-optimize command
 */
function showOptimizeHelp() {
    console.log(`
${chalk_1.default.yellow('🔧 Hive Mind Database Optimization')}
...
${chalk_1.default.bold('EXAMPLES:')}
  ${chalk_1.default.gray('# Interactive optimization')}
  claude-flow hive-mind-optimize

  ${chalk_1.default.gray('# Auto-optimize with all features')}
  claude-flow hive-mind-optimize --auto --vacuum --clean-memory --archive-tasks

  ${chalk_1.default.gray('# Generate report only')}
  claude-flow hive-mind-optimize --report

  ${chalk_1.default.gray('# Custom retention periods')}
  claude-flow hive-mind-optimize --clean-memory --memory-days 60 --task-days 14

${chalk_1.default.bold('FEATURES:')}
  ${chalk_1.default.cyan('🚀')} Performance indexes for faster queries
  ${chalk_1.default.cyan('📊')} Query optimization and statistics
  ${chalk_1.default.cyan('🧹')} Memory cleanup and archiving
  ${chalk_1.default.cyan('📈')} Performance tracking tables
  ${chalk_1.default.cyan('🔍')} Behavioral pattern analysis
  ${chalk_1.default.cyan('💾')} Backward-compatible upgrades
 ...
 */
async function hiveMindOptimizeCommand(args, flags) {
    ...
    if (!(0, fs_1.existsSync)(dbPath)) {
        console.error(chalk_1.default.red('Error: Hive Mind database not found'));
...
async function interactiveOptimization(dbPath, flags) {
    ...
    const report = await (0, db_optimizer_js_1.generateOptimizationReport)(dbPath);
    if (report) {
        console.log(chalk_1.default.cyan('Current Database Status:'));
    ...
    if (!needsOptimization) {
        console.log(chalk_1.default.green('✓ Database is already fully optimized!\n'));
        const { maintenance } = await inquirer_1.default.prompt([
            {
                type: 'confirm',
                name: 'maintenance',
        ...
        if (!maintenance) {
            console.log(chalk_1.default.gray('No changes made.'));
            return;
        }
    }
    else {
        console.log(chalk_1.default.yellow(`⚠ Database can be optimized from version ${schemaVersion} to 1.5\n`));
    }
    ...
    if (!answers.confirm) {
        console.log(chalk_1.default.gray('Optimization cancelled.'));
        return;
    }
...
async function runOptimization(dbPath, options) {
    ...
    const result = await (0, db_optimizer_js_1.optimizeHiveMindDatabase)(dbPath, options);
    if (!result.success) {
    ...
    if (options.cleanMemory || options.archiveTasks || options.checkIntegrity) {
        ...
        await (0, db_optimizer_js_1.performMaintenance)(dbPath, options);
    }
...
async function generateReport(dbPath) {
    const report = await (0, db_optimizer_js_1.generateOptimizationReport)(dbPath);
    if (!report) {
        console.error(chalk_1.default.red('Failed to generate report'));
        return;
    }
    console.log(chalk_1.default.bold('\n📊 Database Optimization Report\n'));
    console.log(chalk_1.default.cyan('Schema Version:'), report.schemaVersion);
    console.log(chalk_1.default.cyan('Indexes:'), report.indexes.length);
    ...
    if (report.schemaVersion >= 1.5) {
        console.log(chalk_1.default.green('  ✓ Database is fully optimized'));
    }
    else {
        console.log(chalk_1.default.yellow(`  ⚠ Can be upgraded from v${report.schemaVersion} to v1.5`));
        console.log(chalk_1.default.gray('    Run: claude-flow hive-mind-optimize'));
    }
    ...
    if (largeMemoryTable || largeTaskTable) {
        ...
        if (largeMemoryTable) {
            console.log(chalk_1.default.yellow('  • Consider cleaning old memory entries'));
        }
        if (largeTaskTable) {
            console.log(chalk_1.default.yellow('  • Consider archiving completed tasks'));
        }
    }
}
/**
...
async function createBackup(dbPath) {
    try {
        ...
        console.error(chalk_1.default.yellow('⚠ Backup failed:', error.message));
...
// Export for CLI
exports.default = hiveMindOptimizeCommand;
