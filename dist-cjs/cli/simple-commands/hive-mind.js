...
exports.hiveMindCommand = hiveMindCommand;
exports.showHiveMindHelp = showHiveMindHelp;
exports.initHiveMind = initHiveMind;
exports.spawnSwarm = spawnSwarm;
...
const interactive_detector_js_1 = require("../utils/interactive-detector.js");
const safe_interactive_js_1 = require("../utils/safe-interactive.js");
...
const mcp_wrapper_js_1 = require("./hive-mind/mcp-wrapper.js");
const core_js_1 = require("./hive-mind/core.js");
const queen_js_1 = require("./hive-mind/queen.js");
const memory_js_1 = require("./hive-mind/memory.js");
const communication_js_1 = require("./hive-mind/communication.js");
const session_manager_js_1 = require("./hive-mind/session-manager.js");
const auto_save_middleware_js_1 = require("./hive-mind/auto-save-middleware.js");
function showHiveMindHelp() {
    console.log(`
${chalk_1.default.yellow('🧠 Claude Flow Hive Mind System')}

${chalk_1.default.bold('USAGE:')}
  claude-flow hive-mind [subcommand] [options]

${chalk_1.default.bold('SUBCOMMANDS:')}
  ${chalk_1.default.green('init')}         Initialize hive mind system
  ${chalk_1.default.green('spawn')}        Spawn hive mind swarm for a task
  ${chalk_1.default.green('status')}       Show hive mind status
  ${chalk_1.default.green('resume')}       Resume a paused hive mind session
  ${chalk_1.default.green('stop')}         Stop a running hive mind session
  ${chalk_1.default.green('sessions')}     List all hive mind sessions
  ${chalk_1.default.green('consensus')}    View consensus decisions
  ${chalk_1.default.green('memory')}       Manage collective memory
  ${chalk_1.default.green('metrics')}      View performance metrics
  ${chalk_1.default.green('wizard')}       Interactive hive mind wizard

${chalk_1.default.bold('EXAMPLES:')}
  ${chalk_1.default.gray('# Initialize hive mind')}
  claude-flow hive-mind init

  ${chalk_1.default.gray('# Spawn swarm with interactive wizard')}
  claude-flow hive-mind spawn

  ${chalk_1.default.gray('# Quick spawn with objective')}
  claude-flow hive-mind spawn "Build microservices architecture"

  ${chalk_1.default.gray('# View current status')}
  claude-flow hive-mind status

  ${chalk_1.default.gray('# Interactive wizard')}
  claude-flow hive-mind wizard

  ${chalk_1.default.gray('# Spawn with Claude Code coordination')}
  claude-flow hive-mind spawn "Build REST API" --claude

  ${chalk_1.default.gray('# Auto-spawn coordinated Claude Code instances')}
  claude-flow hive-mind spawn "Research AI trends" --auto-spawn --verbose

  ${chalk_1.default.gray('# List all sessions')}
  claude-flow hive-mind sessions

  ${chalk_1.default.gray('# Resume a paused session')}
  claude-flow hive-mind resume session-1234567890-abc123

${chalk_1.default.bold('KEY FEATURES:')}
  ${chalk_1.default.cyan('🐝')} Queen-led coordination with worker specialization
  ${chalk_1.default.cyan('🧠')} Collective memory and knowledge sharing
  ${chalk_1.default.cyan('🤝')} Consensus building for critical decisions
  ${chalk_1.default.cyan('⚡')} Parallel task execution with auto-scaling
  ${chalk_1.default.cyan('🔄')} Work stealing and load balancing
  ${chalk_1.default.cyan('📊')} Real-time metrics and performance tracking
  ${chalk_1.default.cyan('🛡️')} Fault tolerance and self-healing
  ${chalk_1.default.cyan('🔒')} Secure communication between agents
...
${chalk_1.default.bold('For more information:')}
${chalk_1.default.blue('https://github.com/ruvnet/claude-flow/tree/main/docs/hive-mind')}
`);
}
/**
...
async function initHiveMind(flags) {
    const spinner = (0, ora_1.default)('Initializing Hive Mind system...').start();
    try {
        // Create hive mind directory structure
        const hiveMindDir = path_1.default.join((0, node_compat_js_1.cwd)(), '.hive-mind');
        if (!(0, fs_1.existsSync)(hiveMindDir)) {
            (0, fs_1.mkdirSync)(hiveMindDir, { recursive: true });
        }
        // Initialize SQLite database
        const dbPath = path_1.default.join(hiveMindDir, 'hive.db');
    ...
    `);
        ...
        await (0, promises_1.writeFile)(path_1.default.join(hiveMindDir, 'config.json'), JSON.stringify(config, null, 2));
        spinner.succeed('Hive Mind system initialized successfully!');
        console.log('\n' + chalk_1.default.green('✓') + ' Created .hive-mind directory');
        console.log(chalk_1.default.green('✓') + ' Initialized SQLite database');
        console.log(chalk_1.default.green('✓') + ' Created configuration file');
        console.log('\n' + chalk_1.default.yellow('Next steps:'));
        console.log('  1. Run ' + chalk_1.default.cyan('claude-flow hive-mind spawn') + ' to create your first swarm');
        console.log('  2. Use ' + chalk_1.default.cyan('claude-flow hive-mind wizard') + ' for interactive setup');
    }
    catch (error) {
        spinner.fail('Failed to initialize Hive Mind system');
        console.error(chalk_1.default.red('Error:'), error.message);
        (0, node_compat_js_1.exit)(1);
    }
}
/**
...
const hiveMindWizard = (0, safe_interactive_js_1.safeInteractive)(
// Interactive version
async function (flags = {}) {
    ...
    switch (action) {
        ...
        case 'config':
            ...
            break;
        case 'exit':
            console.log(chalk_1.default.gray('Exiting wizard...'));
            break;
    }
}, 
...
async function (flags = {}) {
    ...
    console.log(chalk_1.default.cyan('Creating new swarm with default settings...'));
    console.log(chalk_1.default.gray('Use command-line flags to customize:'));
    console.log(chalk_1.default.gray('  --objective "Your task"    Set swarm objective'));
    console.log(chalk_1.default.gray('  --queen-type strategic     Set queen type'));
    console.log(chalk_1.default.gray('  --max-workers 8            Set worker count'));
    console.log();
...
async function spawnSwarm(args, flags) {
    ...
    if (!objective && !flags.wizard) {
        console.error(chalk_1.default.red('Error: Please provide an objective or use --wizard flag'));
        console.log('Example: claude-flow hive-mind spawn "Build REST API"');
        return;
    }
    // Validate parameters
    if (flags.verbose) {
        console.log(chalk_1.default.gray('🔍 Debug: Parsed flags:'));
        console.log(chalk_1.default.gray(JSON.stringify(flags, null, 2)));
    }
    // Validate queen type
    ...
    if (!validQueenTypes.includes(queenType)) {
        console.error(chalk_1.default.red(`Error: Invalid queen type '${queenType}'. Must be one of: ${validQueenTypes.join(', ')}`));
        return;
    }
    // Validate max workers
    const maxWorkers = parseInt(flags.maxWorkers || flags['max-workers'] || '8');
    if (isNaN(maxWorkers) || maxWorkers < 1 || maxWorkers > 20) {
        console.error(chalk_1.default.red('Error: max-workers must be a number between 1 and 20'));
        return;
    }
    ...
    if (!validConsensusTypes.includes(consensusAlgorithm)) {
        console.error(chalk_1.default.red(`Error: Invalid consensus algorithm '${consensusAlgorithm}'. Must be one of: ${validConsensusTypes.join(', ')}`));
        return;
    }
    const spinner = (0, ora_1.default)('Spawning Hive Mind swarm...').start();
    try {
        // Initialize hive mind core with error handling
        let hiveMind;
        try {
            spinner.text = 'Initializing Hive Mind Core...';
            hiveMind = new core_js_1.HiveMindCore({
                objective,
        ...
        const dbDir = path_1.default.join((0, node_compat_js_1.cwd)(), '.hive-mind');
        const dbPath = path_1.default.join(dbDir, 'hive.db');
    ...
    `);
      ...
      `).run(swarmId, hiveMind.config.name, objective, hiveMind.config.queenType);
        ...
        const autoSave = (0, auto_save_middleware_js_1.createAutoSaveMiddleware)(sessionId, sessionManager, {
        ...
        // Initialize Queen
        const queen = new queen_js_1.QueenCoordinator({
            swarmId,
    ...
    `).run(...Object.values(queenAgent));
      ...
      `).run(...Object.values(worker));
        ...
        const memory = new memory_js_1.CollectiveMemory({
            swarmId,
        ...
        const communication = new communication_js_1.SwarmCommunication({
            swarmId,
            encryption: hiveMind.config.encryption,
        });
        db.close();
        ...
        console.log('\n' + chalk_1.default.bold('🐝 Swarm Summary:'));
        console.log(chalk_1.default.gray('─'.repeat(50)));
        console.log(chalk_1.default.cyan('Swarm ID:'), swarmId);
        console.log(chalk_1.default.cyan('Session ID:'), sessionId);
        console.log(chalk_1.default.cyan('Name:'), hiveMind.config.name);
        console.log(chalk_1.default.cyan('Objective:'), objective);
        console.log(chalk_1.default.cyan('Queen Type:'), hiveMind.config.queenType);
        console.log(chalk_1.default.cyan('Workers:'), workers.length);
        console.log(chalk_1.default.cyan('Worker Types:'), workerTypes.join(', '));
        console.log(chalk_1.default.cyan('Consensus:'), hiveMind.config.consensusAlgorithm);
        console.log(chalk_1.default.cyan('Auto-scaling:'), hiveMind.config.autoScale ? 'Enabled' : 'Disabled');
        console.log(chalk_1.default.gray('─'.repeat(50)));
        // Launch monitoring if requested
        if (flags.monitor) {
            console.log('\n' + chalk_1.default.yellow('Launching monitoring dashboard...'));
            // TODO: Implement monitoring dashboard
        }
        // Enhanced coordination instructions with MCP tools
        console.log('\n' + chalk_1.default.green('✓') + ' Swarm is ready for coordination');
        console.log(chalk_1.default.gray('Use "claude-flow hive-mind status" to view swarm activity'));
        console.log(chalk_1.default.gray('Session auto-save enabled - progress saved every 30 seconds'));
        console.log(chalk_1.default.blue('💡 To pause:') + ' Press Ctrl+C to safely pause and resume later');
        console.log(chalk_1.default.blue('💡 To resume:') + ' claude-flow hive-mind resume ' + sessionId);
        ...
        const sigintHandler = async () => {
            if (isExiting)
                return;
            isExiting = true;
            console.log('\n\n' + chalk_1.default.yellow('⏸️  Pausing session...'));
            try {
                ...
                console.log(chalk_1.default.bold(`  claude-flow hive-mind resume ${sessionId}`));
                console.log();
        ...
        if (flags.claude || flags.spawn) {
            ...
            console.log('\n' +
                chalk_1.default.blue('💡 Pro Tip:') +
                ' Add --claude to spawn coordinated Claude Code instances');
            console.log(chalk_1.default.gray('   claude-flow hive-mind spawn "objective" --claude'));
        }
    }
    catch (error) {
...
async function showStatus(flags) {
    try {
        ...
        if (!(0, fs_1.existsSync)(dbPath)) {
            console.error(chalk_1.default.red('Error: Hive Mind not initialized'));
    ...
    `)
            .all();
        if (swarms.length === 0) {
            console.log(chalk_1.default.gray('No active swarms found'));
            db.close();
            return;
        }
        ...
        for (const swarm of swarms) {
            ...
            console.log(chalk_1.default.cyan('Swarm:'), swarm.name);
            console.log(chalk_1.default.cyan('ID:'), swarm.id);
            console.log(chalk_1.default.cyan('Objective:'), swarm.objective);
            console.log(chalk_1.default.cyan('Queen Type:'), swarm.queen_type);
            console.log(chalk_1.default.cyan('Status:'), chalk_1.default.green(swarm.status));
            console.log(chalk_1.default.cyan('Created:'), new Date(swarm.created_at).toLocaleString());
            // Get agents
            const agents = db
                .prepare(`
        SELECT * FROM agents 
      ...
      `)
                .all(swarm.id);
            console.log('\n' + chalk_1.default.bold('Agents:'));
            // Group by role
            const queen = agents.find((a) => a.role === 'queen');
            const workers = agents.filter((a) => a.role === 'worker');
            if (queen) {
                console.log('  ' + chalk_1.default.magenta('👑 Queen:'), queen.name, chalk_1.default.gray(`(${queen.status})`));
            }
            console.log('  ' + chalk_1.default.blue('🐝 Workers:'));
            workers.forEach((worker) => {
                const statusColor = worker.status === 'active' ? 'green' : worker.status === 'busy' ? 'yellow' : 'gray';
                console.log(`    - ${worker.name} (${worker.type}) ${chalk_1.default[statusColor](worker.status)}`);
            });
            // Get task statistics
            const taskStats = db
                .prepare(`
      ...
      `)
                .get(swarm.id);
            console.log('\n' + chalk_1.default.bold('Tasks:'));
            console.log(`  Total: ${taskStats.total}`);
            console.log(`  Completed: ${chalk_1.default.green(taskStats.completed)}`);
            console.log(`  In Progress: ${chalk_1.default.yellow(taskStats.in_progress)}`);
            console.log(`  Pending: ${chalk_1.default.gray(taskStats.pending)}`);
            // Get memory stats
      ...
      `)
                .get(swarm.id);
            console.log('\n' + chalk_1.default.bold('Collective Memory:'));
      ...
      `)
                .get(swarm.id);
            console.log('\n' + chalk_1.default.bold('Consensus Decisions:'));
...
async function showConsensus(flags) {
    ...
    `)
        ...
        if (decisions.length === 0) {
            console.log(chalk_1.default.gray('No consensus decisions found'));
            db.close();
            return;
        }
        ...
        decisions.forEach((decision) => {
            ...
            console.log(chalk_1.default.cyan('Swarm:'), decision.swarm_name);
            console.log(chalk_1.default.cyan('Topic:'), decision.topic);
            console.log(chalk_1.default.cyan('Decision:'), decision.decision);
            console.log(chalk_1.default.cyan('Algorithm:'), decision.algorithm);
            console.log(chalk_1.default.cyan('Confidence:'), `${(decision.confidence * 100).toFixed(1)}%`);
            console.log(chalk_1.default.cyan('Time:'), new Date(decision.created_at).toLocaleString());
            if (decision.votes) {
                const votes = JSON.parse(decision.votes);
                console.log(chalk_1.default.cyan('Votes:'));
...
async function showMetrics(flags) {
    ...
    `)
            .all();
        console.log(chalk_1.default.cyan('Overall Statistics:'));
        ...
        if (taskBreakdown.length > 0) {
            console.log('\n' + chalk_1.default.cyan('Task Status Breakdown:'));
                ...
                const statusColor = status.status === 'completed'
                    ...
                    : status.status === 'in_progress'
                        ...
                        : status.status === 'failed'
                            ? 'red'
                            : 'gray';
                console.log(`  ${chalk_1.default[statusColor](status.status.charAt(0).toUpperCase() + status.status.slice(1))}: ${status.count} (${percentage}%)`);
            });
        }
      ...
      `)
        ...
        if (agentPerf.length > 0) {
            console.log('\n' + chalk_1.default.cyan('Top Performing Agents:'));
...
async function manageMemoryWizard() {
    ...
    const { continue: continueAction } = await inquirer_1.default.prompt([
        {
            type: 'confirm',
            name: 'continue',
...
async function configureWizard() {
    ...
    console.log(chalk_1.default.yellow('Configuration wizard coming soon...'));
}
/**
 * Main hive mind command handler
 */
async function hiveMindCommand(args, flags) {
    ...
    if ((subcommand === 'spawn' && (flags.claude || flags.spawn)) || subcommand === 'wizard') {
        (0, interactive_detector_js_1.warnNonInteractive)('hive-mind ' + subcommand);
    }
    switch (subcommand) {
        case 'init':
...
async function listMemories() {
    try {
        ...
        if (!memories || memories.length === 0) {
            console.log(chalk_1.default.yellow('No memories found in the collective store.'));
            console.log(chalk_1.default.gray('Try storing some memories first using the "💾 Store new memory" option.'));
            return;
        }
        memories.forEach((memory, index) => {
            console.log(chalk_1.default.cyan(`${index + 1}. ${memory.key || `memory-${index}`}`));
        ...
        console.error(chalk_1.default.red('Error listing memories:'), error.message);
        console.log(chalk_1.default.gray('This might be because no memories have been stored yet.'));
    }
}
/**
...
async function searchMemories() {
    try {
        ...
        memories.forEach((memory, index) => {
            console.log(chalk_1.default.green(`${index + 1}. ${memory.key || `result-${index}`}`));
            console.log(`   Category: ${memory.type || 'general'}`);
            console.log(`   Created: ${new Date(memory.timestamp || Date.now()).toLocaleString()}`);
        ...
        console.error(chalk_1.default.red('Error searching memories:'), error.message);
    }
}
/**
...
async function storeMemoryWizard() {
    try {
        ...
        console.error(chalk_1.default.red('Error storing memory:'), error.message);
    }
}
/**
...
async function showMemoryStats() {
    try {
        ...
        if (!memories || memories.length === 0) {
            ...
            console.log(chalk_1.default.gray('Use "Store new memory" to create your first memory.'));
            return;
        }
        ...
        console.log(chalk_1.default.cyan('Total memories:'), stats.total);
        console.log(chalk_1.default.cyan('Estimated size:'), `${(stats.totalSize / 1024).toFixed(2)} KB`);
        console.log(chalk_1.default.cyan('Date range:'), `${stats.oldestDate?.toLocaleDateString()} - ${stats.newestDate?.toLocaleDateString()}`);
        console.log(chalk_1.default.cyan('\nBy category:'));
        ...
        console.error(chalk_1.default.red('Error getting memory stats:'), error.message);
    }
}
/**
 * Clean old memories
...
async function cleanMemories() {
    try {
        ...
        console.log(chalk_1.default.gray('Note: Individual memory deletion not yet implemented in MCPWrapper.'));
        console.log(chalk_1.default.gray('Consider implementing batch deletion or memory lifecycle management.'));
    }
    catch (error) {
        console.error(chalk_1.default.red('Error cleaning memories:'), error.message);
    }
}
/**
...
async function exportMemoryBackup() {
    try {
        ...
        console.log(chalk_1.default.cyan(`Exported ${memories.length} memories`));
    }
    catch (error) {
        console.error(chalk_1.default.red('Error exporting memory backup:'), error.message);
    }
}
/**
...
async function spawnClaudeCodeInstances(swarmId, swarmName, objective, workers, flags) {
    console.log('\n' + chalk_1.default.bold('🚀 Launching Claude Code with Hive Mind Coordination'));
    console.log(chalk_1.default.gray('─'.repeat(60)));
    const spinner = (0, ora_1.default)('Preparing Hive Mind coordination prompt...').start();
    try {
        ...
        console.log('\n' + chalk_1.default.bold('🧠 Hive Mind Configuration'));
        console.log(chalk_1.default.gray('─'.repeat(60)));
        console.log(chalk_1.default.cyan('Swarm ID:'), swarmId);
        console.log(chalk_1.default.cyan('Objective:'), objective);
        console.log(chalk_1.default.cyan('Queen Type:'), flags.queenType || 'strategic');
        console.log(chalk_1.default.cyan('Worker Count:'), workers.length);
        console.log(chalk_1.default.cyan('Worker Types:'), Object.keys(workerGroups).join(', '));
        console.log(chalk_1.default.cyan('Consensus Algorithm:'), flags.consensus || 'majority');
        console.log(chalk_1.default.cyan('MCP Tools:'), 'Full Claude-Flow integration enabled');
        try {
            // ALWAYS save the prompt file first (fix for issue #330)
            // Ensure sessions directory exists
            const sessionsDir = path_1.default.join('.hive-mind', 'sessions');
            await (0, node_compat_js_1.mkdirAsync)(sessionsDir, { recursive: true });
            const promptFile = path_1.default.join(sessionsDir, `hive-mind-prompt-${swarmId}.txt`);
            await (0, promises_1.writeFile)(promptFile, hiveMindPrompt, 'utf8');
            console.log(chalk_1.default.green(`\n✓ Hive Mind prompt saved to: ${promptFile}`));
            // Check if claude command exists
            const { spawn: childSpawn, execSync } = await Promise.resolve().then(() => __importStar(require('child_process')));
            ...
            try {
                ...
                console.log(chalk_1.default.gray('Install it with: npm install -g @anthropic-ai/claude-code'));
            ...
            if (claudeAvailable && !flags.dryRun) {
                ...
                if (flags['dangerously-skip-permissions'] !== false && !flags['no-auto-permissions']) {
                    ...
                    console.log(chalk_1.default.yellow('🔓 Using --dangerously-skip-permissions by default for seamless hive-mind execution'));
                }
                ...
                const sigintHandler = async () => {
                    if (isExiting)
                        return;
                    isExiting = true;
                    console.log('\n\n' + chalk_1.default.yellow('⏸️  Pausing session and terminating Claude Code...'));
                    try {
                ...
                console.log(chalk_1.default.blue('  The Queen coordinator will orchestrate all worker agents'));
                console.log(chalk_1.default.blue('  Use MCP tools for collective intelligence and task distribution'));
                console.log(chalk_1.default.gray(`  Prompt file saved at: ${promptFile}`));
            }
            else if (flags.dryRun) {
                console.log(chalk_1.default.blue('\nDry run - would execute Claude Code with prompt:'));
                console.log(chalk_1.default.gray('Prompt length:'), hiveMindPrompt.length, 'characters');
                console.log(chalk_1.default.gray('\nFirst 500 characters of prompt:'));
                console.log(chalk_1.default.yellow(hiveMindPrompt.substring(0, 500) + '...'));
                ...
                console.log(chalk_1.default.gray('─'.repeat(50)));
                console.log(chalk_1.default.gray('1. Install Claude Code:'));
                console.log(chalk_1.default.green('   npm install -g @anthropic-ai/claude-code'));
                console.log(chalk_1.default.gray('\n2. Run with the saved prompt:'));
                console.log(chalk_1.default.green(`   claude < ${promptFile}`));
                console.log(chalk_1.default.gray('\n3. Or copy the prompt manually:'));
                console.log(chalk_1.default.green(`   cat ${promptFile} | claude`));
                console.log(chalk_1.default.gray('\n4. With auto-permissions:'));
                console.log(chalk_1.default.green(`   claude --dangerously-skip-permissions < ${promptFile}`));
            }
        }
        catch (error) {
...
async function showSessions(flags) {
    try {
        ...
        if (sessions.length === 0) {
            console.log(chalk_1.default.gray('No active or paused sessions found'));
            sessionManager.close();
            return;
        }
        ...
        sessions.forEach((session, index) => {
            ...
            console.log(chalk_1.default.yellow('═'.repeat(60)));
            console.log(`${statusIcon} ${chalk_1.default.bold(session.swarm_name)}`);
            console.log(chalk_1.default.cyan('Session ID:'), session.id);
            console.log(chalk_1.default.cyan('Status:'), chalk_1.default[statusColor](session.status));
            console.log(chalk_1.default.cyan('Objective:'), session.objective);
            console.log(chalk_1.default.cyan('Progress:'), `${session.completion_percentage}%`);
            console.log(chalk_1.default.cyan('Created:'), new Date(session.created_at).toLocaleString());
            console.log(chalk_1.default.cyan('Last Updated:'), new Date(session.updated_at).toLocaleString());
            if (session.paused_at) {
                console.log(chalk_1.default.cyan('Paused At:'), new Date(session.paused_at).toLocaleString());
            }
            ...
            if (session.checkpoint_data) {
                console.log('\n' + chalk_1.default.bold('Last Checkpoint:'));
                console.log(chalk_1.default.gray(JSON.stringify(session.checkpoint_data, null, 2).substring(0, 200) + '...'));
            }
        });
        console.log(chalk_1.default.yellow('═'.repeat(60)) + '\n');
...
async function resumeSession(args, flags) {
    ...
    if (!sessionId) {
        console.error(chalk_1.default.red('Error: Please provide a session ID'));
    ...
    const spinner = (0, ora_1.default)('Resuming Hive Mind session...').start();
    try {
        const sessionManager = new session_manager_js_1.HiveMindSessionManager();
        // Get session details
        ...
        console.log('\n' + chalk_1.default.bold('📋 Resumed Session Summary:'));
        console.log(chalk_1.default.gray('─'.repeat(50)));
        console.log(chalk_1.default.cyan('Session ID:'), sessionId);
        console.log(chalk_1.default.cyan('Swarm Name:'), resumedSession.swarm_name);
        console.log(chalk_1.default.cyan('Objective:'), resumedSession.objective);
        console.log(chalk_1.default.cyan('Progress:'), `${resumedSession.statistics.completionPercentage}%`);
        console.log(chalk_1.default.cyan('Active Agents:'), `${resumedSession.statistics.activeAgents}/${resumedSession.statistics.totalAgents}`);
        console.log(chalk_1.default.cyan('Tasks:'), `${resumedSession.statistics.completedTasks}/${resumedSession.statistics.totalTasks} completed`);
        console.log(chalk_1.default.gray('─'.repeat(50)));
        // Show task breakdown
        console.log('\n' + chalk_1.default.bold('📊 Task Status:'));
        ...
        if (resumedSession.recentLogs && resumedSession.recentLogs.length > 0) {
            console.log('\n' + chalk_1.default.bold('📜 Recent Activity:'));
        ...
        if (resumedSession.checkpoint_data) {
            console.log('\n' + chalk_1.default.bold('♻️  Restoring from checkpoint...'));
            console.log(chalk_1.default.gray('Checkpoint data available for restoration'));
        }
        sessionManager.close();
        // Offer to spawn Claude Code with restored context
        if (flags.claude || flags.spawn) {
            console.log('\n' + chalk_1.default.yellow('🚀 Launching Claude Code with restored context...'));
            ...
            console.log(chalk_1.default.gray('   claude-flow hive-mind resume ' + sessionId + ' --claude'));
        }
    }
    catch (error) {
...
async function stopSession(args, flags) {
    ...
    const spinner = (0, ora_1.default)('Stopping Hive Mind session...').start();
    try {
        const sessionManager = new session_manager_js_1.HiveMindSessionManager();
        // Get session details
        const session = sessionManager.getSession(sessionId);
        ...
        // Display session summary
        console.log('\n' + chalk_1.default.bold('🛑 Stopped Session Summary:'));
        console.log(chalk_1.default.gray('─'.repeat(50)));
        console.log(chalk_1.default.cyan('Session ID:'), sessionId);
        console.log(chalk_1.default.cyan('Swarm Name:'), session.swarm_name || 'Unknown');
        console.log(chalk_1.default.cyan('Final Status:'), 'Stopped');
        console.log(chalk_1.default.cyan('Active Agents:'), session.statistics ? session.statistics.activeAgents : 0);
        console.log(chalk_1.default.gray('─'.repeat(50)));
        console.log('\n' + chalk_1.default.yellow('💡 Session has been stopped and all processes cleaned up.'));
        console.log(chalk_1.default.gray('To resume this session later, use: claude-flow hive-mind resume ' + sessionId));
        sessionManager.close();
    }
    catch (error) {
...
async function launchClaudeWithContext(prompt, flags) {
    try {
        ...
        await (0, promises_1.writeFile)(promptFile, prompt);
        ...
        try {
            ...
            console.log(chalk_1.default.gray('Install Claude Code: npm install -g @anthropic-ai/claude-code'));
            console.log(chalk_1.default.gray(`Run with: claude < ${promptFile}`));
            return;
        }
        if (claudeAvailable && !flags.dryRun) {
            ...
            const sigintHandler = () => {
                console.log('\n\n' + chalk_1.default.yellow('⏹️  Terminating Claude Code...'));
                if (claudeProcess && !claudeProcess.killed) {
                    claudeProcess.kill('SIGTERM');
                }
                process.exit(0);
        ...
        console.error(chalk_1.default.red('Failed to launch Claude Code:'), error.message);
    }
}
/**
...
async function getMcpWrapper() {
    const { MCPToolWrapper } = await Promise.resolve().then(() => __importStar(require('./hive-mind/mcp-wrapper.js')));
