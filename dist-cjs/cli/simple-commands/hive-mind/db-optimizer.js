...
exports.optimizeHiveMindDatabase = optimizeHiveMindDatabase;
 ...
 */
async function optimizeHiveMindDatabase(dbPath, options = {}) {
    const spinner = (0, ora_1.default)('Optimizing Hive Mind database...').start();
    try {
        ...
        if (optimizationsApplied.length > 0) {
            console.log('\n' + chalk_1.default.green('✓') + ' Optimizations applied:');
            optimizationsApplied.forEach((opt) => {
                console.log('  - ' + opt);
            });
        }
        else {
            console.log('\n' + chalk_1.default.yellow('ℹ') + ' Database already optimized');
        }
 ...
 */
async function performMaintenance(dbPath, options = {}) {
    const spinner = (0, ora_1.default)('Performing database maintenance...').start();
    try {
        const db = new better_sqlite3_1.default(dbPath);
      ...
      `)
          ...
          `)
                    ...
                    console.log(chalk_1.default.green(`✓ Removed ${result.changes} old memory entries`));
                }
                catch (error) {
                    console.warn(chalk_1.default.yellow(`⚠ Could not clean memory entries: ${error.message}`));
                }
            }
            else {
                console.log(chalk_1.default.yellow('⚠ collective_memory table not found, skipping memory cleanup'));
            }
        }
      ...
      `)
        ...
        `)
            ...
            console.log(chalk_1.default.green(`✓ Archived ${archived.changes} completed tasks`));
        }
        // Update statistics
        ...
        if (options.checkIntegrity) {
            ...
            if (integrityCheck.integrity_check === 'ok') {
                console.log(chalk_1.default.green('✓ Database integrity check passed'));
            }
            else {
                console.log(chalk_1.default.yellow('⚠ Database integrity issues detected'));
            }
        }
        db.close();
 ...
 */
async function generateOptimizationReport(dbPath) {
    try {
