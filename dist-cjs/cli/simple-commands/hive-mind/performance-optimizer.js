 ...
 */
class BatchProcessor extends events_1.default {
    constructor(config = {}) {
        super();
        this.config = {
    ...
    getMetrics() {
        return {
            ...
            pendingItems: Array.from(this.batches.values()).reduce((sum, batch) => sum + batch.items.length, 0),
        };
    }
    close() {
 ...
 */
class PerformanceOptimizer extends events_1.default {
    constructor(config = {}) {
        super();
        this.config = {
        ...
        this.asyncQueue = new AsyncOperationQueue(this.config.asyncQueueConcurrency, this.config.asyncTimeout || 30000);
