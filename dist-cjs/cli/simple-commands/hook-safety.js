...
exports.SafeHookExecutor = exports.HookConfigValidator = exports.HookCircuitBreaker = exports.HookCommandValidator = exports.HookContextManager = void 0;
exports.hookSafetyCommand = hookSafetyCommand;
exports.addSafetyFlags = addSafetyFlags;
...
exports.HookContextManager = HookContextManager;
/**
 * Command Validator - Validates commands for hook safety
 */
class HookCommandValidator {
    /**
...
exports.HookCommandValidator = HookCommandValidator;
/**
...
class HookCircuitBreaker {
    ...
    static checkExecution(hookType) {
        ...
        if (hookType === 'Stop' && executionCount > 1) {
            (0, utils_js_1.printWarning)(`⚠️  Stop hook execution #${executionCount} detected. Monitor for recursion.`);
        }
        return true;
    }
    static reset() {
        executionTracker.reset();
        (0, utils_js_1.printSuccess)('Circuit breaker reset successfully.');
    }
    static getStatus() {
        return {
...
exports.HookCircuitBreaker = HookCircuitBreaker;
/**
...
class HookConfigValidator {
    ...
    static validateClaudeCodeConfig(configPath = null) {
        if (!configPath) {
            ...
            const possiblePaths = [
                path_1.default.join(process.env.HOME || '.', '.claude', 'settings.json'),
                path_1.default.join(process.cwd(), '.claude', 'settings.json'),
                path_1.default.join(process.cwd(), 'settings.json'),
            ];
            configPath = possiblePaths.find((p) => (0, fs_1.existsSync)(p));
            if (!configPath) {
...
exports.HookConfigValidator = HookConfigValidator;
/**
 * Safe Hook Execution Wrapper
 */
class SafeHookExecutor {
    /**
    ...
    static async executeHookCommand(command, hookType, options = {}) {
        try {
            ...
            for (const warning of validation.warnings) {
                (0, utils_js_1.printWarning)(warning.message);
            }
            // Block on errors
            if (!validation.safe) {
                for (const error of validation.errors) {
                    (0, utils_js_1.printError)(error.message);
                }
            ...
            (0, utils_js_1.printError)(`Hook execution failed: ${err.message}`);
...
exports.SafeHookExecutor = SafeHookExecutor;
/**
 * Hook Safety CLI Commands
 */
async function hookSafetyCommand(subArgs, flags) {
...
async function validateConfigCommand(subArgs, flags) {
    ...
    if (result.safe) {
        (0, utils_js_1.printSuccess)('✅ Hook configuration is safe!');
        if (result.configPath) {
            console.log(`📄 Validated: ${result.configPath}`);
        }
    }
    else {
        (0, utils_js_1.printError)('❌ DANGEROUS hook configuration detected!');
        if (result.errors) {
            console.log('\n🚨 CRITICAL ERRORS:');
...
async function resetCommand(subArgs, flags) {
    ...
    HookContextManager.clearContext();
    (0, utils_js_1.printSuccess)('✅ Hook safety systems reset successfully!');
...
async function safeModeCommand(subArgs, flags) {
    ...
    if (enable) {
        ...
        (0, utils_js_1.printSuccess)('🛡️  Safe mode enabled!');
        ...
        (0, utils_js_1.printSuccess)('⚡ Safe mode disabled.');
 ...
 */
function addSafetyFlags(command) {
