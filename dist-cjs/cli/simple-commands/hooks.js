...
exports.hooksAction = hooksAction;
const utils_js_1 = require("../utils.js");
const sqlite_store_js_1 = require("../../memory/sqlite-store.js");
// Initialize memory store
let memoryStore = null;
async function getMemoryStore() {
    if (!memoryStore) {
        memoryStore = new sqlite_store_js_1.SqliteMemoryStore();
...
async function hooksAction(subArgs, flags) {
    ...
    try {
        switch (subcommand) {
            ...
            case 'notify':
                ...
                break;
            default:
                (0, utils_js_1.printError)(`Unknown hooks command: ${subcommand}`);
                showHooksHelp();
        }
    }
    catch (err) {
        (0, utils_js_1.printError)(`Hooks command failed: ${err.message}`);
    }
}
// ===== PRE-OPERATION HOOKS =====
async function preTaskCommand(subArgs, flags) {
    ...
    try {
        ...
        try {
            const checkPromise = (0, utils_js_1.checkRuvSwarmAvailable)();
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), 3000));
            const isAvailable = await Promise.race([checkPromise, timeoutPromise]);
            if (isAvailable) {
                console.log(`\n🔄 Executing ruv-swarm pre-task hook...`);
                const hookResult = await (0, utils_js_1.execRuvSwarmHook)('pre-task', {
                    description,
                    'task-id': taskId,
                ...
                if (hookResult.success) {
                    ...
                    (0, utils_js_1.printSuccess)(`✅ Pre-task hook completed successfully`);
                }
            }
        }
        catch (err) {
        ...
        (0, utils_js_1.printError)(`Pre-task hook failed: ${err.message}`);
...
async function preEditCommand(subArgs, flags) {
    ...
    if (autoAssignAgents)
        console.log(`🤖 Auto-assign agents: ENABLED`);
    if (loadContext)
        console.log(`🔄 Load context: ENABLED`);
    try {
        const store = await getMemoryStore();
        ...
        (0, utils_js_1.printSuccess)(`✅ Pre-edit hook completed`);
    }
    catch (err) {
        (0, utils_js_1.printError)(`Pre-edit hook failed: ${err.message}`);
    }
}
async function preBashCommand(subArgs, flags) {
    ...
    if (validateSafety)
        console.log(`🔒 Safety validation: ENABLED`);
    if (prepareResources)
        console.log(`🛠️  Resource preparation: ENABLED`);
    try {
        const store = await getMemoryStore();
        ...
        if (validateSafety) {
            ...
            if (isDangerous) {
                ...
                (0, utils_js_1.printError)(`Command blocked due to safety validation: ${command}`);
                return;
            }
        }
        if (prepareResources) {
        ...
        (0, utils_js_1.printSuccess)(`✅ Pre-bash hook completed`);
    }
    catch (err) {
        (0, utils_js_1.printError)(`Pre-bash hook failed: ${err.message}`);
    }
}
// ===== POST-OPERATION HOOKS =====
async function postTaskCommand(subArgs, flags) {
    ...
    try {
        ...
        (0, utils_js_1.printSuccess)(`✅ Post-task hook completed`);
    }
    catch (err) {
        (0, utils_js_1.printError)(`Post-task hook failed: ${err.message}`);
    }
}
async function postEditCommand(subArgs, flags) {
    ...
    if (updateMemory)
        console.log(`🧠 Memory update: ENABLED`);
    if (trainNeural)
        console.log(`🤖 Neural training: ENABLED`);
    try {
        const store = await getMemoryStore();
        const path = await Promise.resolve().then(() => __importStar(require('path')));
        const fs = await Promise.resolve().then(() => __importStar(require('fs')));
        ...
        (0, utils_js_1.printSuccess)(`✅ Post-edit hook completed`);
    }
    catch (err) {
        (0, utils_js_1.printError)(`Post-edit hook failed: ${err.message}`);
    }
}
async function postBashCommand(subArgs, flags) {
    ...
    if (storeResults)
        console.log(`💾 Results storage: ENABLED`);
    try {
        const store = await getMemoryStore();
        ...
        (0, utils_js_1.printSuccess)(`✅ Post-bash hook completed`);
    }
    catch (err) {
        (0, utils_js_1.printError)(`Post-bash hook failed: ${err.message}`);
    }
}
async function postSearchCommand(subArgs, flags) {
    ...
    try {
        ...
        (0, utils_js_1.printSuccess)(`✅ Post-search hook completed`);
    }
    catch (err) {
        (0, utils_js_1.printError)(`Post-search hook failed: ${err.message}`);
    }
}
// ===== MCP INTEGRATION HOOKS =====
async function mcpInitializedCommand(subArgs, flags) {
    ...
    try {
        ...
        (0, utils_js_1.printSuccess)(`✅ MCP initialized hook completed`);
    }
    catch (err) {
        (0, utils_js_1.printError)(`MCP initialized hook failed: ${err.message}`);
    }
}
async function agentSpawnedCommand(subArgs, flags) {
    ...
    try {
        ...
        (0, utils_js_1.printSuccess)(`✅ Agent spawned hook completed`);
    }
    catch (err) {
        (0, utils_js_1.printError)(`Agent spawned hook failed: ${err.message}`);
    }
}
async function taskOrchestratedCommand(subArgs, flags) {
    ...
    try {
        ...
        (0, utils_js_1.printSuccess)(`✅ Task orchestrated hook completed`);
    }
    catch (err) {
        (0, utils_js_1.printError)(`Task orchestrated hook failed: ${err.message}`);
    }
}
async function neuralTrainedCommand(subArgs, flags) {
    ...
    try {
        ...
        (0, utils_js_1.printSuccess)(`✅ Neural trained hook completed`);
    }
    catch (err) {
        (0, utils_js_1.printError)(`Neural trained hook failed: ${err.message}`);
    }
}
// ===== SESSION HOOKS =====
async function sessionEndCommand(subArgs, flags) {
    ...
    try {
        ...
        (0, utils_js_1.printSuccess)(`✅ Session-end hook completed`);
    }
    catch (err) {
        (0, utils_js_1.printError)(`Session-end hook failed: ${err.message}`);
    }
}
async function sessionRestoreCommand(subArgs, flags) {
    ...
    try {
        ...
        if (sessionData) {
            ...
            (0, utils_js_1.printSuccess)(`✅ Session restore completed`);
        }
        else {
            (0, utils_js_1.printWarning)(`No session found with ID: ${sessionId}`);
        }
    }
    catch (err) {
        (0, utils_js_1.printError)(`Session restore hook failed: ${err.message}`);
    }
}
async function notifyCommand(subArgs, flags) {
    ...
    try {
        ...
        (0, utils_js_1.printSuccess)(`✅ Notify hook completed`);
    }
    catch (err) {
        (0, utils_js_1.printError)(`Notify hook failed: ${err.message}`);
    }
}
