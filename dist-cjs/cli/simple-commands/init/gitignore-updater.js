...
exports.needsGitignoreUpdate = needsGitignoreUpdate;
 ...
 */
async function updateGitignore(workingDir, force = false, dryRun = false) {
    const gitignorePath = `${workingDir}/.gitignore`;
    try {
        let gitignoreContent = '';
        let fileExists = false;
        // Check if .gitignore exists
        if ((0, node_compat_js_1.existsSync)(gitignorePath)) {
            fileExists = true;
            gitignoreContent = await (0, node_compat_js_1.readTextFile)(gitignorePath);
        }
        ...
        // Write the file
        if (!dryRun) {
            await (0, node_compat_js_1.writeTextFile)(gitignorePath, newContent);
        }
        return {
            success: true,
            message: fileExists
 ...
 */
async function needsGitignoreUpdate(workingDir) {
    const gitignorePath = `${workingDir}/.gitignore`;
    if (!(0, node_compat_js_1.existsSync)(gitignorePath)) {
        return true;
    }
    try {
        const content = await (0, node_compat_js_1.readTextFile)(gitignorePath);
