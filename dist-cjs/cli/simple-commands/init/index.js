...
const executable_wrapper_js_1 = require("./executable-wrapper.js");
const sparc_structure_js_1 = require("./sparc-structure.js");
const slash_commands_js_1 = require("./claude-commands/slash-commands.js");
const optimized_slash_commands_js_1 = require("./claude-commands/optimized-slash-commands.js");
const child_process_1 = require("child_process");
const fs_1 = require("fs");
const template_copier_js_1 = require("./template-copier.js");
const copy_revised_templates_js_1 = require("./copy-revised-templates.js");
const agent_copier_js_1 = require("./agent-copier.js");
const help_js_1 = require("./help.js");
const batch_init_js_1 = require("./batch-init.js");
const index_js_1 = require("./validation/index.js");
const index_js_2 = require("./rollback/index.js");
const enhanced_templates_js_1 = require("./templates/enhanced-templates.js");
const npx_isolated_cache_js_1 = require("../../../utils/npx-isolated-cache.js");
const gitignore_updater_js_1 = require("./gitignore-updater.js");
...
async function setupMcpServers(dryRun = false) {
    ...
    for (const server of servers) {
        try {
            if (!dryRun) {
                ...
                (0, child_process_1.execSync)(`claude mcp add ${server.name} ${server.command}`, { stdio: 'inherit' });
    ...
    if (!dryRun) {
        ...
        try {
            (0, child_process_1.execSync)('claude mcp list', { stdio: 'inherit' });
        }
        catch (err) {
...
async function initCommand(subArgs, flags) {
    ...
    try {
        process_1.default.chdir(workingDir);
    }
    catch (err) {
        (0, utils_js_1.printWarning)(`Could not change to directory ${workingDir}: ${err.message}`);
    }
    try {
        (0, utils_js_1.printSuccess)('Initializing Claude Code integration files...');
        // Check if files already exist in the working directory
        const files = ['CLAUDE.md', 'memory-bank.md', 'coordination.md'];
        const existingFiles = [];
        for (const file of files) {
            try {
                await node_compat_js_1.Deno.stat(`${workingDir}/${file}`);
        ...
        if (existingFiles.length > 0 && !initForce) {
            (0, utils_js_1.printWarning)(`The following files already exist: ${existingFiles.join(', ')}`);
        ...
        const validation = (0, copy_revised_templates_js_1.validateTemplatesExist)();
        if (validation.valid) {
            console.log('  📁 Copying revised template files...');
            const revisedResults = await (0, copy_revised_templates_js_1.copyRevisedTemplates)(workingDir, {
                force: initForce,
                dryRun: initDryRun,
                verbose: true,
                sparc: initSparc
            ...
            const copyResults = await (0, template_copier_js_1.copyTemplates)(workingDir, templateOptions);
            if (!copyResults.success) {
                (0, utils_js_1.printError)('Failed to copy templates:');
        ...
        if (!initDryRun) {
            await (0, executable_wrapper_js_1.createLocalExecutable)(workingDir);
        }
        else {
        ...
        if (initSparc) {
            ...
            if (initDryRun) {
                ...
                try {
                    ...
                    const createSparcCommand = new node_compat_js_1.Deno.Command('npx', {
                        args: ['-y', 'create-sparc', 'init', '--force'],
                        cwd: workingDir, // Use the original working directory
                        stdout: 'inherit',
                        stderr: 'inherit',
                        env: (0, npx_isolated_cache_js_1.getIsolatedNpxEnv)({
                    ...
                    if (createSparcResult.success) {
                        ...
                        (0, utils_js_1.printWarning)('create-sparc failed, creating basic SPARC structure manually...');
                    ...
                    (0, utils_js_1.printWarning)('create-sparc not available, creating basic SPARC structure manually...');
                    // Fallback: create basic SPARC structure manually
                    await (0, sparc_structure_js_1.createSparcStructureManually)();
                    sparcInitialized = true; // Manual creation still counts as initialized
                }
                ...
                if (sparcInitialized) {
                    try {
                        if (initOptimized) {
                            await (0, optimized_slash_commands_js_1.createOptimizedClaudeSlashCommands)(workingDir, selectedModes);
                        }
                        else {
                        ...
                        (0, utils_js_1.printWarning)(`Could not create Claude Code slash commands: ${err.message}`);
                    }
                }
            }
        }
        if (initDryRun) {
            (0, utils_js_1.printSuccess)("🔍 Dry run completed! Here's what would be created:");
            ...
            (0, utils_js_1.printSuccess)('🎉 Claude Code integration files initialized successfully!');
            if (initOptimized) {
            ...
            // Update .gitignore
            const gitignoreResult = await (0, gitignore_updater_js_1.updateGitignore)(workingDir, initForce, initDryRun);
        ...
        (0, utils_js_1.printError)(`Failed to initialize files: ${err.message}`);
    }
}
...
async function handleBatchInit(subArgs, flags) {
    try {
        ...
        // Validate options
        const validationErrors = (0, batch_init_js_1.validateBatchOptions)(options);
        if (validationErrors.length > 0) {
            (0, utils_js_1.printError)('Batch options validation failed:');
        ...
        if (flags.config) {
            ...
            (0, utils_js_1.printSuccess)(`Loading batch configuration from: ${configFile}`);
            const results = await (0, batch_init_js_1.batchInitFromConfig)(configFile, options);
            if (results) {
                (0, utils_js_1.printSuccess)('Batch initialization from config completed');
            }
            return;
        }
        // Batch init mode
        if (flags['batch-init']) {
            ...
            (0, utils_js_1.printSuccess)(`Initializing ${projects.length} projects in batch mode`);
            const results = await (0, batch_init_js_1.batchInitCommand)(projects, options);
            if (results) {
                const successful = results.filter((r) => r.success).length;
                const failed = results.filter((r) => !r.success).length;
                if (failed === 0) {
                    (0, utils_js_1.printSuccess)(`All ${successful} projects initialized successfully`);
                }
                else {
                    (0, utils_js_1.printWarning)(`${successful} projects succeeded, ${failed} failed`);
                }
            }
            return;
        }
        (0, utils_js_1.printError)('No batch operation specified. Use --batch-init <projects> or --config <file>');
    }
    catch (err) {
        (0, utils_js_1.printError)(`Batch initialization failed: ${err.message}`);
    }
}
/**
...
async function enhancedInitCommand(subArgs, flags) {
    ...
    const workingDir = node_compat_js_1.Deno.env.get('PWD') || node_compat_js_1.Deno.cwd();
    // Initialize systems
    const rollbackSystem = new index_js_2.RollbackSystem(workingDir);
    const validationSystem = new index_js_1.ValidationSystem(workingDir);
    let atomicOp = null;
    try {
        // Parse options
        const initOptions = {
        ...
        if (!initOptions.skipPreValidation) {
            ...
            if (!preValidation.success) {
                (0, utils_js_1.printError)('Pre-initialization validation failed:');
                preValidation.errors.forEach((error) => console.error(`  ❌ ${error}`));
                return;
            }
            if (preValidation.warnings.length > 0) {
                (0, utils_js_1.printWarning)('Pre-initialization warnings:');
                preValidation.warnings.forEach((warning) => console.warn(`  ⚠️  ${warning}`));
            }
            (0, utils_js_1.printSuccess)('Pre-initialization validation passed');
        }
        ...
        if (!options.skipBackup) {
            ...
            if (!backupResult.success) {
                (0, utils_js_1.printError)('Backup creation failed:');
        ...
        atomicOp = (0, index_js_2.createAtomicOperation)(rollbackSystem, 'enhanced-init');
        const atomicBegin = await atomicOp.begin();
        if (!atomicBegin) {
            (0, utils_js_1.printError)('Failed to begin atomic operation');
            return;
        }
        ...
        if (!postValidation.success) {
            (0, utils_js_1.printError)('Post-initialization validation failed:');
            postValidation.errors.forEach((error) => console.error(`  ❌ ${error}`));
            // Attempt automatic rollback
            console.log('\n🔄 Attempting automatic rollback...');
            await atomicOp.rollback();
            (0, utils_js_1.printWarning)('Initialization rolled back due to validation failure');
            return;
        }
        ...
        if (configValidation.warnings.length > 0) {
            (0, utils_js_1.printWarning)('Configuration warnings:');
        ...
        if (healthChecks.warnings.length > 0) {
            (0, utils_js_1.printWarning)('Health check warnings:');
            healthChecks.warnings.forEach((warning) => console.warn(`  ⚠️  ${warning}`));
        }
        // Commit atomic operation
        await atomicOp.commit();
        // Generate and display validation report
        const fullValidation = await (0, index_js_1.runFullValidation)(workingDir, {
            postInit: true,
            skipPreInit: options.skipPreValidation,
        });
        console.log('\n📊 Validation Report:');
        console.log(fullValidation.report);
        (0, utils_js_1.printSuccess)('🎉 Enhanced initialization completed successfully!');
        console.log('\n✨ Your SPARC environment is fully validated and ready to use');
    }
    catch (error) {
        (0, utils_js_1.printError)(`Enhanced initialization failed: ${error.message}`);
        // Attempt rollback if atomic operation is active
        if (atomicOp && !atomicOp.completed) {
            console.log('\n🔄 Performing emergency rollback...');
            try {
                await atomicOp.rollback();
                (0, utils_js_1.printWarning)('Emergency rollback completed');
            }
            catch (rollbackError) {
                (0, utils_js_1.printError)(`Rollback also failed: ${rollbackError.message}`);
            }
        }
    }
}
...
async function handleValidationCommand(subArgs, flags) {
    ...
    try {
        const validationResults = await (0, index_js_1.runFullValidation)(workingDir, options);
        console.log('\n📊 Validation Results:');
        console.log(validationResults.report);
        if (validationResults.success) {
            (0, utils_js_1.printSuccess)('✅ All validation checks passed');
        }
        else {
            (0, utils_js_1.printError)('❌ Some validation checks failed');
            process_1.default.exit(1);
        }
    }
    catch (error) {
        (0, utils_js_1.printError)(`Validation failed: ${error.message}`);
        process_1.default.exit(1);
    }
}
/**
    ...
    try {
        ...
        if (subArgs.includes('--full')) {
            ...
            if (phaseIndex !== -1 && subArgs[phaseIndex + 1]) {
                ...
                (0, utils_js_1.printError)('Partial rollback requires --phase <phase-name>');
            }
        }
        else {
            ...
            if (rollbackPoints.rollbackPoints.length === 0) {
                (0, utils_js_1.printWarning)('No rollback points available');
                return;
            }
            ...
            if (latest) {
                ...
                if (result.success) {
                    (0, utils_js_1.printSuccess)('Rollback completed successfully');
                }
                else {
                    (0, utils_js_1.printError)('Rollback failed');
                }
            }
        }
    }
    catch (error) {
        (0, utils_js_1.printError)(`Rollback operation failed: ${error.message}`);
    }
}
/**
...
async function handleListBackups(subArgs, flags) {
    ...
    try {
        ...
        (0, utils_js_1.printError)(`Failed to list backups: ${error.message}`);
    }
}
/**
...
async function performInitializationWithCheckpoints(rollbackSystem, options, workingDir, dryRun = false) {
    const phases = [
        ...
        { name: 'executable-creation', action: () => (0, executable_wrapper_js_1.createLocalExecutable)(workingDir, dryRun) },
    ];
    if (options.sparc) {
        phases.push({ name: 'sparc-init', action: () => (0, sparc_structure_js_1.createSparcStructureManually)() }, { name: 'claude-commands', action: () => (0, slash_commands_js_1.createClaudeSlashCommands)(workingDir) });
    }
...
async function createInitialFiles(options, workingDir, dryRun = false) {
    if (!dryRun) {
        const claudeMd = options.sparc
            ? (0, claude_md_js_1.createSparcClaudeMd)()
            : options.minimal
                ? (0, claude_md_js_1.createMinimalClaudeMd)()
                : (0, claude_md_js_1.createFullClaudeMd)();
        await node_compat_js_1.Deno.writeTextFile(`${workingDir}/CLAUDE.md`, claudeMd);
        const memoryBankMd = options.minimal ? (0, memory_bank_md_js_1.createMinimalMemoryBankMd)() : (0, memory_bank_md_js_1.createFullMemoryBankMd)();
        await node_compat_js_1.Deno.writeTextFile(`${workingDir}/memory-bank.md`, memoryBankMd);
        const coordinationMd = options.minimal
            ? (0, coordination_md_js_1.createMinimalCoordinationMd)()
            : (0, coordination_md_js_1.createFullCoordinationMd)();
        await node_compat_js_1.Deno.writeTextFile(`${workingDir}/coordination.md`, coordinationMd);
    }
}
async function createDirectoryStructure(workingDir, dryRun = false) {
    ...
    if (!dryRun) {
        for (const dir of directories) {
            await node_compat_js_1.Deno.mkdir(`${workingDir}/${dir}`, { recursive: true });
        }
    }
}
async function setupMemorySystem(workingDir, dryRun = false) {
    if (!dryRun) {
        ...
        await node_compat_js_1.Deno.writeTextFile(`${workingDir}/memory/claude-flow-data.json`, JSON.stringify(initialData, null, 2));
        await node_compat_js_1.Deno.writeTextFile(`${workingDir}/memory/agents/README.md`, (0, readme_files_js_1.createAgentsReadme)());
        await node_compat_js_1.Deno.writeTextFile(`${workingDir}/memory/sessions/README.md`, (0, readme_files_js_1.createSessionsReadme)());
    }
}
...
async function enhancedClaudeFlowInit(flags, subArgs = []) {
    ...
    const workingDir = process_1.default.cwd();
    ...
    try {
        ...
        for (const file of filesToCheck) {
            if ((0, node_compat_js_1.existsSync)(`${workingDir}/${file}`)) {
                existingFiles.push(file);
            }
        }
        ...
        // Create CLAUDE.md
        if (!dryRun) {
            await node_compat_js_1.Deno.writeTextFile(`${workingDir}/CLAUDE.md`, (0, enhanced_templates_js_1.createEnhancedClaudeMd)());
            (0, utils_js_1.printSuccess)('✓ Created CLAUDE.md (Claude Flow v2.0.0)');
        }
        else {
        ...
        if (!dryRun) {
            await node_compat_js_1.Deno.mkdir(claudeDir, { recursive: true });
            await node_compat_js_1.Deno.mkdir(`${claudeDir}/commands`, { recursive: true });
            await node_compat_js_1.Deno.mkdir(`${claudeDir}/helpers`, { recursive: true });
            (0, utils_js_1.printSuccess)('✓ Created .claude directory structure');
        }
        else {
        ...
        if (!dryRun) {
            await node_compat_js_1.Deno.writeTextFile(`${claudeDir}/settings.json`, (0, enhanced_templates_js_1.createEnhancedSettingsJson)());
            (0, utils_js_1.printSuccess)('✓ Created .claude/settings.json with hooks and MCP configuration');
        }
        else {
        ...
        if (!dryRun) {
            await node_compat_js_1.Deno.writeTextFile(`${claudeDir}/settings.local.json`, JSON.stringify(settingsLocal, null, 2));
            (0, utils_js_1.printSuccess)('✓ Created .claude/settings.local.json with default MCP permissions');
        }
        else {
        ...
        if (!dryRun) {
            await node_compat_js_1.Deno.writeTextFile(`${workingDir}/.mcp.json`, JSON.stringify(mcpConfig, null, 2));
            (0, utils_js_1.printSuccess)('✓ Created .mcp.json at project root for MCP server configuration');
        }
        else {
        ...
        if (!dryRun) {
            await node_compat_js_1.Deno.writeTextFile(`${workingDir}/claude-flow.config.json`, JSON.stringify(claudeFlowConfig, null, 2));
            (0, utils_js_1.printSuccess)('✓ Created claude-flow.config.json for Claude Flow settings');
        }
        else {
            console.log('[DRY RUN] Would create claude-flow.config.json for Claude Flow settings');
        }
        // Create command documentation
        for (const [category, commands] of Object.entries(enhanced_templates_js_1.COMMAND_STRUCTURE)) {
            const categoryDir = `${claudeDir}/commands/${category}`;
            if (!dryRun) {
                await node_compat_js_1.Deno.mkdir(categoryDir, { recursive: true });
...
`;
                await node_compat_js_1.Deno.writeTextFile(`${categoryDir}/README.md`, categoryReadme);
                // Create individual command docs
                for (const command of commands) {
                    const doc = (0, enhanced_templates_js_1.createCommandDoc)(category, command);
                    if (doc) {
                        await node_compat_js_1.Deno.writeTextFile(`${categoryDir}/${command}.md`, doc);
                    }
                }
        ...
        if (!dryRun) {
            ...
            const unixWrapper = (0, enhanced_templates_js_1.createWrapperScript)('unix');
            await node_compat_js_1.Deno.writeTextFile(`${workingDir}/claude-flow`, unixWrapper);
            await fs.chmod(`${workingDir}/claude-flow`, 0o755);
            // Windows wrapper
            await node_compat_js_1.Deno.writeTextFile(`${workingDir}/claude-flow.bat`, (0, enhanced_templates_js_1.createWrapperScript)('windows'));
            // PowerShell wrapper
            await node_compat_js_1.Deno.writeTextFile(`${workingDir}/claude-flow.ps1`, (0, enhanced_templates_js_1.createWrapperScript)('powershell'));
            (0, utils_js_1.printSuccess)('✓ Created platform-specific wrapper scripts');
        }
        else {
        ...
        for (const helper of helpers) {
            if (!dryRun) {
                const content = (0, enhanced_templates_js_1.createHelperScript)(helper);
                if (content) {
                    await node_compat_js_1.Deno.writeTextFile(`${claudeDir}/helpers/${helper}`, content);
                    await fs.chmod(`${claudeDir}/helpers/${helper}`, 0o755);
                }
            }
        }
        if (!dryRun) {
            (0, utils_js_1.printSuccess)(`✓ Created ${helpers.length} helper scripts`);
        }
        else {
        ...
        if (!dryRun) {
            (0, utils_js_1.printSuccess)('✓ Created standard directory structure');
            // Initialize memory system
            const initialData = { agents: [], tasks: [], lastUpdated: Date.now() };
            await fs.writeFile(`${workingDir}/memory/claude-flow-data.json`, JSON.stringify(initialData, null, 2));
            // Create README files
            await fs.writeFile(`${workingDir}/memory/agents/README.md`, (0, readme_files_js_1.createAgentsReadme)());
            await fs.writeFile(`${workingDir}/memory/sessions/README.md`, (0, readme_files_js_1.createSessionsReadme)());
            (0, utils_js_1.printSuccess)('✓ Initialized memory system');
            // Initialize memory database with fallback support
            try {
                // Import and initialize FallbackMemoryStore to create the database
                const { FallbackMemoryStore } = await Promise.resolve().then(() => __importStar(require('../../../memory/fallback-store.js')));
                const memoryStore = new FallbackMemoryStore();
                await memoryStore.initialize();
                if (memoryStore.isUsingFallback()) {
                    (0, utils_js_1.printSuccess)('✓ Initialized memory system (in-memory fallback for npx compatibility)');
                    console.log('  💡 For persistent storage, install locally: npm install claude-flow@alpha');
                }
                else {
                    (0, utils_js_1.printSuccess)('✓ Initialized memory database (.swarm/memory.db)');
                }
                memoryStore.close();
            }
            catch (err) {
            ...
            try {
                ...
                await fs.writeFile(`${workingDir}/.hive-mind/config.json`, JSON.stringify(hiveMindConfig, null, 2));
                // Initialize hive.db
                try {
          ...
          `);
                    hiveDb.close();
                    (0, utils_js_1.printSuccess)('✓ Initialized hive-mind database (.hive-mind/hive.db)');
                }
                catch (dbErr) {
                    console.log(`  ⚠️  Could not initialize hive-mind database: ${dbErr.message}`);
                }
                (0, utils_js_1.printSuccess)('✓ Initialized hive-mind system');
            }
            catch (err) {
        ...
        const gitignoreResult = await (0, gitignore_updater_js_1.updateGitignore)(workingDir, force, dryRun);
        if (gitignoreResult.success) {
            if (!dryRun) {
                (0, utils_js_1.printSuccess)(`✓ ${gitignoreResult.message}`);
            }
            else {
                console.log(gitignoreResult.message);
            }
        ...
        try {
            // Run create-sparc
            console.log('  🔄 Running: npx -y create-sparc init --force');
            (0, child_process_1.execSync)('npx -y create-sparc init --force', {
                cwd: workingDir,
                stdio: 'inherit',
            });
            sparcInitialized = true;
            (0, utils_js_1.printSuccess)('✅ SPARC environment initialized successfully');
        }
        catch (err) {
        ...
        if (!dryRun) {
            await (0, agent_copier_js_1.createAgentDirectories)(workingDir, dryRun);
            const agentResult = await (0, agent_copier_js_1.copyAgentFiles)(workingDir, {
                force: force,
                dryRun: dryRun
            });
            if (agentResult.success) {
                await (0, agent_copier_js_1.validateAgentSystem)(workingDir);
        ...
        (0, utils_js_1.printError)(`Failed to initialize Claude Flow v2.0.0: ${err.message}`);
    }
}
