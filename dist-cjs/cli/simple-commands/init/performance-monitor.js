...
exports.BatchOptimizer = exports.ResourceThresholdMonitor = exports.PerformanceMonitor = void 0;
    ...
    static createDefaultCallbacks() {
        return {
            onMemoryWarning: (current, max) => {
                (0, utils_js_1.printInfo)(`⚠️ Memory usage high: ${current.toFixed(1)}MB / ${max}MB`);
            },
            ...
            onCPUWarning: (percent) => {
                (0, utils_js_1.printInfo)(`⚠️ CPU usage high: ${percent}%`);
            },
        };
    }
}
exports.ResourceThresholdMonitor = ResourceThresholdMonitor;
// Batch operation optimizer
class BatchOptimizer {
    static calculateOptimalConcurrency(projectCount, systemSpecs = {}) {
        const { cpuCores = 4, memoryGB = 8, diskSpeed = 'ssd', // 'ssd' or 'hdd'
         } = systemSpecs;
        let optimal = Math.min(cpuCores * 2, // 2x CPU cores
    ...
    static estimateCompletionTime(projectCount, options = {}) {
        const { concurrency = 5, template = 'basic', sparc = false, averageTimePerProject = 15, // seconds
         } = options;
