...
async function mcpCommand(subArgs, flags) {
...
async function showMcpStatus(subArgs, flags) {
    (0, utils_js_1.printSuccess)('MCP Server Status:');
...
async function startMcpServer(subArgs, flags) {
    ...
    if (stdio) {
        ...
        (0, utils_js_1.printSuccess)('Starting Claude Flow MCP server in stdio mode...');
        if (autoOrchestrator) {
...
async function listMcpTools(subArgs, flags) {
    ...
    (0, utils_js_1.printSuccess)('Claude-Flow MCP Tools & Resources (87 total):');
...
async function manageMcpAuth(subArgs, flags) {
    ...
    switch (authCmd) {
        case 'setup':
            (0, utils_js_1.printSuccess)('Setting up MCP authentication...');
            ...
            break;
        case 'status':
            (0, utils_js_1.printSuccess)('MCP Authentication Status:');
            console.log('🔐 Status: Not configured');
            console.log('🔑 API Keys: 0 active');
            console.log('🛡️  Security: Default settings');
            break;
        case 'rotate':
            (0, utils_js_1.printSuccess)('Rotating MCP authentication keys...');
...
async function showMcpConfig(subArgs, flags) {
    (0, utils_js_1.printSuccess)('Claude-Flow MCP Server Configuration:');
    console.log('\n📋 Server Settings:');
