...
async function memoryCommand(subArgs, flags) {
    ...
    async function loadMemory() {
        try {
            const content = await node_compat_js_1.Deno.readTextFile(memoryStore);
    ...
    async function saveMemory(data) {
        await node_compat_js_1.Deno.mkdir('./memory', { recursive: true });
        await node_compat_js_1.Deno.writeTextFile(memoryStore, JSON.stringify(data, null, 2));
    }
...
async function storeMemory(subArgs, loadMemory, saveMemory) {
    ...
    if (!key || !value) {
        (0, utils_js_1.printError)('Usage: memory store <key> <value>');
        return;
    }
    try {
        ...
        (0, utils_js_1.printSuccess)('Stored successfully');
        console.log(`📝 Key: ${key}`);
        console.log(`📦 Namespace: ${namespace}`);
        console.log(`💾 Size: ${new TextEncoder().encode(value).length} bytes`);
    }
    catch (err) {
        (0, utils_js_1.printError)(`Failed to store: ${err.message}`);
    }
}
async function queryMemory(subArgs, loadMemory) {
    const search = subArgs.slice(1).join(' ');
    if (!search) {
        (0, utils_js_1.printError)('Usage: memory query <search>');
        return;
    }
    try {
        const data = await loadMemory();
        ...
        if (results.length === 0) {
            (0, utils_js_1.printWarning)('No results found');
            return;
        }
        (0, utils_js_1.printSuccess)(`Found ${results.length} results:`);
        ...
        (0, utils_js_1.printError)(`Failed to query: ${err.message}`);
    }
}
async function showMemoryStats(loadMemory) {
    try {
        ...
        (0, utils_js_1.printSuccess)('Memory Bank Statistics:');
        ...
        (0, utils_js_1.printError)(`Failed to get stats: ${err.message}`);
    }
}
async function exportMemory(subArgs, loadMemory) {
    ...
    try {
        ...
        await node_compat_js_1.Deno.writeTextFile(filename, JSON.stringify(exportData, null, 2));
        (0, utils_js_1.printSuccess)(`Memory exported to ${filename}`);
        let totalEntries = 0;
        ...
        (0, utils_js_1.printError)(`Failed to export memory: ${err.message}`);
    }
}
async function importMemory(subArgs, saveMemory) {
    const filename = subArgs[1];
    if (!filename) {
        (0, utils_js_1.printError)('Usage: memory import <filename>');
        return;
    }
    try {
        const importContent = await node_compat_js_1.Deno.readTextFile(filename);
        ...
        (0, utils_js_1.printSuccess)(`Imported ${totalImported} new entries from ${filename}`);
    }
    catch (err) {
        (0, utils_js_1.printError)(`Failed to import memory: ${err.message}`);
    }
}
async function clearMemory(subArgs, saveMemory) {
    const namespace = getNamespaceFromArgs(subArgs);
    if (!namespace) {
        (0, utils_js_1.printError)('Usage: memory clear --namespace <namespace>');
        (0, utils_js_1.printWarning)('This will clear all entries in the specified namespace');
        return;
    }
    try {
        const data = await loadMemory();
        if (!data[namespace]) {
            (0, utils_js_1.printWarning)(`Namespace '${namespace}' does not exist`);
            return;
        }
        const entryCount = data[namespace].length;
        delete data[namespace];
        await saveMemory(data);
        (0, utils_js_1.printSuccess)(`Cleared ${entryCount} entries from namespace '${namespace}'`);
    }
    catch (err) {
        (0, utils_js_1.printError)(`Failed to clear memory: ${err.message}`);
    }
}
async function listNamespaces(loadMemory) {
    try {
        ...
        if (namespaces.length === 0) {
            (0, utils_js_1.printWarning)('No namespaces found');
            return;
        }
        (0, utils_js_1.printSuccess)('Available namespaces:');
        ...
        (0, utils_js_1.printError)(`Failed to list namespaces: ${err.message}`);
    }
}
...
async function loadMemory() {
    try {
        const content = await node_compat_js_1.Deno.readTextFile('./memory/memory-store.json');
        return JSON.parse(content);
    }
    catch {
        return {};
