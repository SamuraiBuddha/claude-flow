...
exports.showMonitorHelp = showMonitorHelp;
...
async function monitorCommand(subArgs, flags) {
...
async function runContinuousMonitoring(interval, format) {
    (0, utils_js_1.printSuccess)(`Starting continuous monitoring (interval: ${interval}ms)`);
...
async function collectMetrics() {
    ...
    return {
        ...
        system: {
            ...
            load_average: os_1.default.loadavg(),
...
async function getOrchestratorMetrics() {
    try {
        ...
        const metricsPath = path_1.default.join(process.cwd(), '.claude-flow', 'metrics.json');
        const metricsData = await promises_1.default.readFile(metricsPath, 'utf8');
...
async function checkOrchestratorRunning() {
    try {
        const pidPath = path_1.default.join(process.cwd(), '.claude-flow', 'orchestrator.pid');
        const pidData = await promises_1.default.readFile(pidPath, 'utf8');
...
async function getResourceMetrics() {
    try {
        ...
        const memoryDbPath = path_1.default.join(process.cwd(), '.claude-flow', 'memory.db');
        const memoryExists = await promises_1.default
            .access(memoryDbPath)
            .then(() => true)
            .catch(() => false);
...
async function countTerminalSessions() {
    try {
        const sessionsPath = path_1.default.join(process.cwd(), '.claude-flow', 'sessions');
        const files = await promises_1.default.readdir(sessionsPath);
...
async function countMCPConnections() {
    try {
        const mcpPath = path_1.default.join(process.cwd(), '.claude-flow', 'mcp-connections.json');
        const data = await promises_1.default.readFile(mcpPath, 'utf8');
