...
const index_js_1 = require("./sparc-modes/index.js");
const node_compat_js_1 = require("../node-compat.js");
const process_1 = __importDefault(require("process"));
async function sparcCommand(subArgs, flags) {
...
async function listSparcModes(subArgs) {
    try {
        ...
        let configContent;
        try {
            configContent = await node_compat_js_1.Deno.readTextFile(configPath);
        }
        catch (error) {
            (0, utils_js_1.printError)('SPARC configuration file (.roomodes) not found');
        ...
        (0, utils_js_1.printSuccess)('Available SPARC Modes:');
        console.log();
        ...
        (0, utils_js_1.printError)(`Failed to list SPARC modes: ${err.message}`);
    }
}
async function showModeInfo(subArgs) {
    const modeSlug = subArgs[1];
    if (!modeSlug) {
        (0, utils_js_1.printError)('Usage: sparc info <mode-slug>');
        return;
    }
    try {
        // Get the actual working directory where the command was run from
        ...
        if (!mode) {
            (0, utils_js_1.printError)(`Mode not found: ${modeSlug}`);
        ...
        (0, utils_js_1.printSuccess)(`SPARC Mode: ${mode.name}`);
        console.log();
        ...
        (0, utils_js_1.printError)(`Failed to show mode info: ${err.message}`);
    }
}
async function runSparcMode(subArgs, flags) {
    ...
    if (!runModeSlug || !taskDescription) {
        (0, utils_js_1.printError)('Usage: sparc run <mode-slug> <task-description>');
        return;
    }
    try {
        // Get the actual working directory where the command was run from
        ...
        if (!mode) {
            (0, utils_js_1.printError)(`Mode not found: ${runModeSlug}`);
            return;
        }
        ...
        const memoryNamespace = subArgs.includes('--namespace')
            ...
            : mode.slug;
        const enhancedTask = (0, index_js_1.createSparcPrompt)(mode, taskDescription, memoryNamespace);
        ...
        if (subArgs.includes('--dry-run') || subArgs.includes('-d')) {
            (0, utils_js_1.printWarning)('DRY RUN - SPARC Mode Configuration:');
        ...
        (0, utils_js_1.printSuccess)(`Starting SPARC mode: ${mode.name}`);
        console.log(`📝 Instance ID: ${instanceId}`);
        ...
        (0, utils_js_1.printError)(`Failed to run SPARC mode: ${err.message}`);
    }
}
async function runTddWorkflow(subArgs) {
    const tddTaskDescription = subArgs.slice(1).join(' ');
    if (!tddTaskDescription) {
        (0, utils_js_1.printError)('Usage: sparc tdd <task-description>');
        return;
    }
    (0, utils_js_1.printSuccess)('Starting SPARC TDD Workflow');
...
async function executeClaude(enhancedTask, toolsList, instanceId, memoryNamespace, subArgs) {
    ...
    if (isNonInteractive || subArgs.includes('--verbose') || subArgs.includes('-v')) {
        ...
        console.log('Permissions:', enablePermissions
            ? '✅ Enabled (will prompt)'
    ...
    try {
        ...
        console.log('\n🚀 Executing command:');
        console.log(`Command: claude`);
        console.log(`Working Directory: ${(0, node_compat_js_1.cwd)()}`);
        console.log(`Number of args: ${claudeArgs.length}`);
        // Check if claude command exists
        try {
            const checkCommand = new node_compat_js_1.Deno.Command('which', {
                args: ['claude'],
                stdout: 'piped',
                stderr: 'piped',
            });
        ...
        const command = new node_compat_js_1.Deno.Command('claude', {
            args: claudeArgs,
            cwd: (0, node_compat_js_1.cwd)(), // Explicitly set working directory to current directory
            env: {
                ...node_compat_js_1.Deno.env.toObject(),
                CLAUDE_INSTANCE_ID: instanceId,
                CLAUDE_SPARC_MODE: 'true',
                CLAUDE_FLOW_MEMORY_ENABLED: 'true',
                CLAUDE_FLOW_MEMORY_NAMESPACE: memoryNamespace,
                CLAUDE_WORKING_DIRECTORY: (0, node_compat_js_1.cwd)(), // Also pass as env variable
            },
            stdin: 'inherit',
            stdout: 'inherit',
            stderr: 'inherit',
        ...
        if (status.success) {
            (0, utils_js_1.printSuccess)(`SPARC instance ${instanceId} completed successfully`);
        }
        else {
            (0, utils_js_1.printError)(`SPARC instance ${instanceId} exited with code ${status.code}`);
        }
    }
    catch (err) {
        (0, utils_js_1.printError)(`Failed to execute Claude: ${err.message}`);
