...
async function startCommand(subArgs, flags) {
    ...
    try {
        (0, utils_js_1.printSuccess)('Starting Claude-Flow Orchestration System...');
        console.log();
        // Check if we should launch the web UI mode
        if (web) {
            try {
                // Launch the web server
                const { startWebServer } = await Promise.resolve().then(() => __importStar(require('./web-server.js')));
                const server = await startWebServer(port);
                (0, utils_js_1.printSuccess)(`🌐 Web UI is running!`);
        ...
        if (ui && !web) {
            try {
                ...
                const { ClaudeCodeWebServer } = await Promise.resolve().then(() => __importStar(require('./web-server.js')));
                ...
                (0, utils_js_1.printWarning)('Web UI failed, launching terminal UI...');
                try {
                    const { launchEnhancedUI } = await Promise.resolve().then(() => __importStar(require('./process-ui-enhanced.js')));
        ...
        for (const dir of requiredDirs) {
            try {
                await node_compat_js_1.Deno.stat(dir);
            }
            catch {
        ...
        if (missingDirs.length > 0) {
            (0, utils_js_1.printWarning)('Missing required directories: ' + missingDirs.join(', '));
        ...
        console.log('     - Shell: ' + (runtime_detector_js_1.compat.platform.os === 'windows' ? 'cmd.exe' : '/bin/bash'));
        // Task queue
        ...
        if (daemon) {
            ...
            (0, utils_js_1.printInfo)('Starting in daemon mode...');
            ...
            const pid = runtime_detector_js_1.compat.terminal.getPid();
            await runtime_detector_js_1.compat.safeCall(async () => {
                if (runtime_detector_js_1.compat.runtime === 'deno') {
                    await node_compat_js_1.Deno.writeTextFile('.claude-flow.pid', pid.toString());
                }
                else {
            ...
            // Interactive mode
            (0, utils_js_1.printSuccess)('Orchestration system started!');
            console.log();
            ...
            runtime_detector_js_1.compat.terminal.onSignal('SIGINT', () => {
                console.log('\n⏹️  Shutting down orchestrator...');
                cleanup();
                runtime_detector_js_1.compat.terminal.exit(0);
            });
        ...
        (0, utils_js_1.printError)(`Failed to start orchestration system: ${err.message}`);
...
async function cleanup() {
    ...
    try {
        await runtime_detector_js_1.compat.safeCall(async () => {
            if (runtime_detector_js_1.compat.runtime === 'deno') {
                await node_compat_js_1.Deno.remove('.claude-flow.pid');
            }
            else {
                const fs = await Promise.resolve().then(() => __importStar(require('fs/promises')));
...
//# sourceMappingURL=start-wrapper.js.map