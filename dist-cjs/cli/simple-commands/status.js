...
async function getResourceUsage() {
    ...
    try {
        ...
        let os;
        try {
            os = await Promise.resolve().then(() => __importStar(require('node:os')));
        }
        catch {
            try {
...
function displayStatus(status, verbose) {
    (0, utils_js_1.printSuccess)('Claude-Flow System Status:');
    // Overall status
...
if (import.meta.main) {
    ...
    if (typeof node_compat_js_1.Deno !== 'undefined' && node_compat_js_1.Deno.args) {
        for (let i = 0; i < node_compat_js_1.Deno.args.length; i++) {
            const arg = node_compat_js_1.Deno.args[i];
            if (arg.startsWith('--')) {
                const flagName = arg.substring(2);
                const nextArg = node_compat_js_1.Deno.args[i + 1];
