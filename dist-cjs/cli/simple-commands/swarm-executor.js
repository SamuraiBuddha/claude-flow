...
class SwarmCoordinator {
    constructor(config) {
        this.config = config;
        this.id = config.name || (0, helpers_js_1.generateId)('swarm');
        this.agents = [];
        this.tasks = [];
    ...
    async initialize() {
        ...
        // Save configuration
        await fs_1.promises.writeFile(path_1.default.join(swarmDir, 'config.json'), JSON.stringify(this.config, null, 2));
        return this;
    }
...
module.exports = app;
`;
        await fs_1.promises.writeFile(path_1.default.join(projectDir, 'server.js'), serverCode);
        // Create package.json
        const packageJson = {
            name: 'api-project',
            version: '1.0.0',
        ...
        await fs_1.promises.writeFile(path_1.default.join(projectDir, 'package.json'), JSON.stringify(packageJson, null, 2));
    ...
    async complete() {
        ...
        // Save summary
        const swarmDir = `./swarm-runs/${this.id}`;
        await fs_1.promises.writeFile(path_1.default.join(swarmDir, 'summary.json'), JSON.stringify(summary, null, 2));
        return summary;
    }
}
...
async function executeSwarm(objective, flags = {}) {
    try {
        ...
        const config = {
            name: (0, helpers_js_1.generateId)('swarm'),
