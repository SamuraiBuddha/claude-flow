...
The swarm should be self-documenting - use memory_store to save all important information, decisions, and results throughout the execution.`;
    ...
    if (flags && flags.background && !process_1.default.env.CLAUDE_SWARM_NO_BG) {
        ...
        if (typeof Deno !== 'undefined') {
            ...
            await (0, node_compat_js_1.mkdirAsync)(swarmRunDir, { recursive: true });
            ...
            const logHandle = await (0, promises_1.open)(logFile, 'w');
...
`;
            ...
            await (0, node_compat_js_1.writeTextFile)(scriptPath, scriptContent);
            // Save process info first
            await (0, node_compat_js_1.writeTextFile)(`${swarmRunDir}/process.json`, JSON.stringify({
                swarmId: swarmId,
                objective: objective,
            ...
            try {
                ...
                const bgProcess = (0, child_process_1.spawn)(bgScriptPath, bgArgs, {
...
`;
                ...
                await (0, node_compat_js_1.writeTextFile)(shellScriptPath, shellScript);
                (0, fs_1.chmodSync)(shellScriptPath, 0o755);
                // Execute the shell script
                const shellProcess = (0, child_process_1.spawn)('bash', [shellScriptPath], {
                    stdio: 'ignore',
                    detached: true,
                });
                shellProcess.unref();
    ...
    try {
        ...
        try {
            ...
            const module = await Promise.resolve(`${distPath}`).then(s => __importStar(require(s)));
        ...
        if (error.code === 'ERR_MODULE_NOT_FOUND' ||
            ...
            try {
                const { executeSwarm } = await Promise.resolve().then(() => __importStar(require('./swarm-executor.js')));
...
async function createSwarmFiles(objective, flags) {
    ...
    if (!targetDir.startsWith('/')) {
        targetDir = path.join(process_1.default.cwd(), targetDir);
    }
