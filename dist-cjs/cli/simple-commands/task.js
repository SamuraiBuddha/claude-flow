...
async function taskCommand(subArgs, flags) {
...
async function createTask(subArgs, flags) {
    ...
    if (!taskType || !description) {
        (0, utils_js_1.printError)('Usage: task create <type> "<description>"');
        console.log('Types: research, code, analysis, coordination, general');
        return;
    }
    const taskId = `task_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const priority = opts.priority || '5';
    (0, utils_js_1.printSuccess)(`Creating ${taskType} task: ${taskId}`);
...
async function listTasks(subArgs, flags) {
    ...
    (0, utils_js_1.printSuccess)('Task queue:');
    if (filter) {
...
async function showTaskStatus(subArgs, flags) {
    ...
    if (!taskId) {
        (0, utils_js_1.printError)('Usage: task status <task-id>');
        return;
    }
    (0, utils_js_1.printSuccess)(`Task status: ${taskId}`);
...
async function cancelTask(subArgs, flags) {
    const taskId = subArgs[1];
    if (!taskId) {
        (0, utils_js_1.printError)('Usage: task cancel <task-id>');
        return;
    }
    (0, utils_js_1.printSuccess)(`Cancelling task: ${taskId}`);
    console.log('🛑 Task would be gracefully cancelled');
}
async function executeWorkflow(subArgs, flags) {
    const workflowFile = subArgs[1];
    if (!workflowFile) {
        (0, utils_js_1.printError)('Usage: task workflow <workflow-file>');
        return;
    }
    (0, utils_js_1.printSuccess)(`Executing workflow: ${workflowFile}`);
...
async function manageCoordination(subArgs, flags) {
    ...
    switch (coordCmd) {
        case 'status':
            (0, utils_js_1.printSuccess)('Task coordination status:');
            ...
            break;
        case 'optimize':
            (0, utils_js_1.printSuccess)('Optimizing task coordination...');
