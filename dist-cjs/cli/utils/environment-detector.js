...
exports.detectExecutionEnvironment = detectExecutionEnvironment;
exports.applySmartDefaults = applySmartDefaults;
exports.getEnvironmentDescription = getEnvironmentDescription;
exports.shouldUseNonInteractiveMode = shouldUseNonInteractiveMode;
...
function detectExecutionEnvironment(options = {}) {
    ...
    env.isCI = Boolean(process.env.CI ||
        ...
        process.env.DRONE);
    // Docker detection
    env.isDocker = Boolean(process.env.DOCKER_CONTAINER ||
    ...
    // WSL detection
    env.isWSL = Boolean(process.env.WSL_DISTRO_NAME ||
...
function showEnvironmentWarnings(env) {
    ...
    if (env.recommendedFlags.length > 0) {
        ...
        console.log(chalk_1.default.gray(`   ${env.recommendedFlags.join(' ')}`));
    }
...
function applySmartDefaults(options, env) {
    ...
    if (options.verbose && appliedDefaults.length > 0) {
        console.log(chalk_1.default.gray(`ℹ️  Auto-applied flags: ${appliedDefaults.join(' ')}`));
    }
    return enhanced;
}
/**
