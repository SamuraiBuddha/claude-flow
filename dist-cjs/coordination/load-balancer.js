    ...
    setupEventHandlers() {
        ...
        this.eventBus.on('task:queued', (data) => {
            if ((0, type_guards_js_1.hasAgentTask)(data)) {
                this.updateTaskQueue(data.agentId.id, data.task, 'add');
            }
        });
        this.eventBus.on('task:started', (data) => {
            if ((0, type_guards_js_1.hasAgentTask)(data)) {
                this.updateTaskQueue(data.agentId.id, data.task, 'remove');
            }
        });
        ...
        this.eventBus.on('agent:performance-update', (data) => {
            if ((0, type_guards_js_1.isObject)(data) && 'agentId' in data && 'metrics' in data) {
                const agentId = data.agentId.id;
                this.updatePerformanceBaseline(agentId, data.metrics);
            }
        });
    }
    async initialize() {
