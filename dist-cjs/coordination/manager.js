...
const advanced_scheduler_js_1 = require("./advanced-scheduler.js");
const conflict_resolution_js_1 = require("./conflict-resolution.js");
...
class CoordinationManager {
    ...
    constructor(config, eventBus, logger) {
        ...
        this.logger = logger;
        this.scheduler = new scheduler_js_1.TaskScheduler(config, eventBus, logger);
        this.resourceManager = new resources_js_1.ResourceManager(config, eventBus, logger);
        this.messageRouter = new messaging_js_1.MessageRouter(config, eventBus, logger);
        this.conflictResolver = new conflict_resolution_js_1.ConflictResolver(logger, eventBus);
        this.metricsCollector = new metrics_js_1.CoordinationMetricsCollector(logger, eventBus);
    }
    async initialize() {
        if (this.initialized) {
            return;
        ...
        try {
            ...
            throw new errors_js_1.CoordinationError('Coordination manager initialization failed', { error });
        }
    }
    async shutdown() {
        if (!this.initialized) {
    ...
    async assignTask(task, agentId) {
        if (!this.initialized) {
            throw new errors_js_1.CoordinationError('Coordination manager not initialized');
        }
    ...
    setupEventHandlers() {
        ...
        this.eventBus.on(types_js_1.SystemEvents.AGENT_TERMINATED, async (data) => {
            const { agentId } = data;
            try {
        ...
        this.deadlockCheckInterval = setInterval(async () => {
            try {
                ...
                if (deadlock) {
                    ...
                    // Emit deadlock event
                    this.eventBus.emit(types_js_1.SystemEvents.DEADLOCK_DETECTED, deadlock);
    ...
    async resolveDeadlock(deadlock) {
        ...
        try {
            ...
            throw new errors_js_1.DeadlockError('Failed to resolve deadlock', deadlock.agents, deadlock.resources);
        }
    }
    ...
    enableAdvancedScheduling() {
        ...
        const advancedScheduler = new advanced_scheduler_js_1.AdvancedTaskScheduler(this.config, this.eventBus, this.logger);
