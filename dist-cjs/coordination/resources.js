...
class ResourceManager {
    ...
    async acquire(resourceId, agentId, priority = 0) {
        ...
        while (Date.now() - startTime < this.config.resourceTimeout) {
            ...
            if (!ourRequest) {
                ...
                throw new errors_js_1.ResourceLockError('Resource request cancelled');
            }
        ...
        throw new errors_js_1.ResourceLockError(`Resource acquisition timeout for ${resourceId}`, {
            resourceId,
            agentId,
    ...
    async lockResource(resourceId, agentId) {
        ...
        // Emit event
        this.eventBus.emit(types_js_1.SystemEvents.RESOURCE_ACQUIRED, { resourceId, agentId });
    }
    unlockResource(resourceId, agentId) {
        ...
        // Emit event
        this.eventBus.emit(types_js_1.SystemEvents.RESOURCE_RELEASED, { resourceId, agentId });
    }
