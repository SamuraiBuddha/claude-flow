...
class TaskScheduler {
    ...
    async assignTask(task, agentId) {
        ...
        if (task.dependencies.length > 0) {
            ...
            if (unmetDependencies.length > 0) {
                throw new errors_js_1.TaskDependencyError(task.id, unmetDependencies);
            }
        }
    ...
    async completeTask(taskId, result) {
        ...
        if (!scheduled) {
            throw new errors_js_1.TaskError(`Task not found: ${taskId}`);
        }
    ...
    async cancelTask(taskId, reason) {
        ...
        this.eventBus.emit(types_js_1.SystemEvents.TASK_CANCELLED, { taskId, reason });
        // Remove from active tasks
        this.tasks.delete(taskId);
        this.agentTasks.get(scheduled.agentId)?.delete(taskId);
        // Cancel dependent tasks
    ...
    async rescheduleAgentTasks(agentId) {
        ...
        for (const taskId of taskIds) {
            ...
            if (scheduled && scheduled.task.status === 'running') {
                ...
                this.eventBus.emit(types_js_1.SystemEvents.TASK_CREATED, {
                    task: scheduled.task,
                });
            }
        }
        ...
        for (const [taskId, scheduled] of this.tasks) {
            if (scheduled.task.status === 'running' && scheduled.task.startedAt) {
                ...
                if (runtime > this.config.resourceTimeout * 2) {
                    ...
                    // Force fail the task
                    await this.failTask(taskId, new errors_js_1.TaskTimeoutError(taskId, runtime));
                }
            }
        }
    }
    startTask(taskId) {
        ...
        this.eventBus.emit(types_js_1.SystemEvents.TASK_STARTED, {
            taskId,
            agentId: scheduled.agentId,
        });
        ...
        scheduled.timeout = setTimeout(() => {
            this.failTask(taskId, new errors_js_1.TaskTimeoutError(taskId, timeoutMs));
        }, timeoutMs);
    }
    canStartTask(task) {
