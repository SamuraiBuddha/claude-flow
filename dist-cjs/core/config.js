...
exports.SECURITY_CLASSIFICATIONS = exports.SENSITIVE_PATHS = exports.configManager = exports.ConfigManager = void 0;
...
exports.SECURITY_CLASSIFICATIONS = SECURITY_CLASSIFICATIONS;
/**
...
exports.SENSITIVE_PATHS = SENSITIVE_PATHS;
/**
...
class ConfigManager {
    ...
    async initializeEncryption() {
        try {
            const keyFile = (0, path_1.join)(this.userConfigDir, '.encryption-key');
            // Check if key file exists (simplified for demo)
            try {
                await fs_1.promises.access(keyFile);
    ...
    async save(path, format) {
        ...
        if (!savePath) {
            throw new errors_js_1.ConfigError('No configuration file path specified');
        }
        const detectedFormat = format || this.detectFormat(savePath);
        const parser = this.formatParsers[detectedFormat];
        if (!parser) {
            throw new errors_js_1.ConfigError(`Unsupported format for saving: ${detectedFormat}`);
        }
        ...
        await fs_1.promises.writeFile(savePath, content, 'utf8');
    ...
    getUserConfigDir() {
        ...
        return (0, path_1.join)(home, '.claude-flow');
    }
    /**
    ...
    async ensureUserConfigDir() {
        try {
            await fs_1.promises.mkdir(this.userConfigDir, { recursive: true });
        }
        catch (error) {
            if (error.code !== 'EEXIST') {
                throw new errors_js_1.ConfigError(`Failed to create config directory: ${error.message}`);
            }
        }
    }
    /**
     ...
     */
    async loadProfiles() {
        const profilesDir = (0, path_1.join)(this.userConfigDir, 'profiles');
        try {
            ...
            for (const entry of entries) {
                if (entry.isFile() && entry.name.endsWith('.json')) {
                    ...
                    const profilePath = (0, path_1.join)(profilesDir, entry.name);
                    try {
                        const content = await fs_1.promises.readFile(profilePath, 'utf8');
                        const profileConfig = (0, helpers_js_1.safeParseJSON)(content);
                        if (profileConfig) {
    ...
    async applyProfile(profileName) {
        ...
        if (!profile) {
            throw new errors_js_1.ConfigError(`Profile '${profileName}' not found`);
        }
    ...
    async saveProfile(profileName, config) {
        ...
        const profilesDir = (0, path_1.join)(this.userConfigDir, 'profiles');
        await fs_1.promises.mkdir(profilesDir, { recursive: true });
        const profileConfig = config || this.config;
        const profilePath = (0, path_1.join)(profilesDir, `${profileName}.json`);
        const content = JSON.stringify(profileConfig, null, 2);
        await fs_1.promises.writeFile(profilePath, content, 'utf8');
        this.profiles.set(profileName, profileConfig);
    }
    /**
     * Deletes a profile
    ...
    async deleteProfile(profileName) {
        const profilePath = (0, path_1.join)(this.userConfigDir, 'profiles', `${profileName}.json`);
        try {
            await fs_1.promises.unlink(profilePath);
            ...
            throw new errors_js_1.ConfigError(`Failed to delete profile: ${error.message}`);
        }
    }
    /**
    ...
    validateValue(value, schema, path) {
        if (schema.type === 'number') {
            if (typeof value !== 'number' || isNaN(value)) {
                throw new errors_js_1.ValidationError(`${path}: must be a number`);
            }
            if (schema.min !== undefined && value < schema.min) {
                throw new errors_js_1.ValidationError(`${path}: must be at least ${schema.min}`);
            }
            if (schema.max !== undefined && value > schema.max) {
                throw new errors_js_1.ValidationError(`${path}: must be at most ${schema.max}`);
            }
        }
        else if (schema.type === 'string') {
            if (typeof value !== 'string') {
                throw new errors_js_1.ValidationError(`${path}: must be a string`);
            }
            if (schema.values && !schema.values.includes(value)) {
                throw new errors_js_1.ValidationError(`${path}: must be one of [${schema.values.join(', ')}]`);
            }
        }
        else if (schema.type === 'boolean') {
            if (typeof value !== 'boolean') {
                throw new errors_js_1.ValidationError(`${path}: must be a boolean`);
            }
        }
    }
    /**
    ...
    import(data) {
        if (!data.config) {
            throw new errors_js_1.ConfigError('Invalid configuration export format');
        }
    ...
    async loadFromFile(path) {
        try {
            ...
            if (!parser) {
                throw new errors_js_1.ConfigError(`Unsupported configuration format: ${format}`);
            }
            const config = parser.parse(content);
            if (!config) {
                throw new errors_js_1.ConfigError(`Invalid ${format.toUpperCase()} in configuration file: ${path}`);
            }
            return config;
        }
        catch (error) {
            ...
            throw new errors_js_1.ConfigError(`Failed to load configuration from ${path}: ${error.message}`);
        }
    }
    /**
    ...
    validateWithDependencies(config) {
        ...
        // Throw errors
        if (errors.length > 0) {
            throw new errors_js_1.ValidationError(`Configuration validation failed:\n${errors.join('\n')}`);
        }
    }
    /**
    ...
    validatePath(path, value, config) {
        ...
        if (rule.required && (value === undefined || value === null)) {
            throw new errors_js_1.ValidationError(`${path} is required`);
        }
        ...
        if (typeof value === 'number') {
            if (rule.min !== undefined && value < rule.min) {
                throw new errors_js_1.ValidationError(`${path} must be at least ${rule.min}`);
            }
            if (rule.max !== undefined && value > rule.max) {
                throw new errors_js_1.ValidationError(`${path} must be at most ${rule.max}`);
            }
        }
        // Values validation
        if (rule.values && !rule.values.includes(value)) {
            throw new errors_js_1.ValidationError(`${path} must be one of: ${rule.values.join(', ')}`);
        }
        // Pattern validation
        if (rule.pattern && typeof value === 'string' && !rule.pattern.test(value)) {
            throw new errors_js_1.ValidationError(`${path} does not match required pattern`);
        }
        // Custom validator
        if (rule.validator) {
            const result = rule.validator(value, currentConfig);
            if (result) {
                throw new errors_js_1.ValidationError(`${path}: ${result}`);
            }
        }
    }
    /**
...
async function loadConfig(path) {
    return await exports.configManager.load(path);
}
