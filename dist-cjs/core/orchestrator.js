...
class SessionManager {
    ...
    constructor(terminalManager, memoryManager, eventBus, logger, config) {
        ...
        this.logger = logger;
        this.config = config;
        this.persistencePath = (0, path_1.join)(config.orchestrator.dataDir || './data', 'sessions.json');
        // Circuit breaker for persistence operations
        this.persistenceCircuitBreaker = (0, helpers_js_1.circuitBreaker)('SessionPersistence', {
            threshold: 5,
            timeout: 30000,
            resetTimeout: 60000,
        });
    ...
    async createSession(profile) {
        try {
            ...
            const terminalId = await (0, helpers_js_1.retry)(() => this.terminalManager.spawnTerminal(profile), {
                maxAttempts: 3,
                initialDelay: 1000,
            });
            // Create memory bank with retry logic
            const memoryBankId = await (0, helpers_js_1.retry)(() => this.memoryManager.createBank(profile.id), {
                maxAttempts: 3,
                initialDelay: 1000,
            });
            // Create session
            ...
            throw new errors_js_1.SystemError(`Failed to create session for agent ${profile.id}`, { error });
        }
    }
    ...
    async terminateSession(sessionId) {
        ...
        try {
            ...
            await Promise.race([
                ...
                (0, helpers_js_1.delay)(5000).then(() => {
    ...
    async persistSessions() {
        ...
        try {
            await this.persistenceCircuitBreaker.execute(async () => {
                ...
                await (0, promises_1.writeFile)(this.persistencePath, JSON.stringify(data, null, 2), 'utf8');
    ...
    async restoreSessions() {
        ...
        try {
            const data = await (0, promises_1.readFile)(this.persistencePath, 'utf8');
...
class Orchestrator {
    ...
    constructor(config, terminalManager, memoryManager, coordinationManager, mcpServer, eventBus, logger) {
        ...
        this.configManager = config_manager_js_1.ConfigManager.getInstance();
        // Initialize circuit breakers
        this.healthCheckCircuitBreaker = (0, helpers_js_1.circuitBreaker)('HealthCheck', {
            threshold: 3,
            timeout: 10000,
            resetTimeout: 30000,
        });
        this.taskAssignmentCircuitBreaker = (0, helpers_js_1.circuitBreaker)('TaskAssignment', {
            threshold: 5,
            timeout: 5000,
            resetTimeout: 20000,
        });
    ...
    async initialize() {
        if (this.initialized) {
            throw new errors_js_1.InitializationError('Orchestrator already initialized');
        }
        ...
        try {
            ...
            if (this.configManager.isClaudeAPIConfigured()) {
                try {
                    this.claudeClient = new claude_client_js_1.ClaudeAPIClient(this.logger, this.configManager);
            ...
            this.eventBus.emit(types_js_1.SystemEvents.SYSTEM_READY, { timestamp: new Date() });
            ...
            throw new errors_js_1.InitializationError('Orchestrator', { error });
        }
    }
    async shutdown() {
        ...
        try {
            ...
            await Promise.race([
                ...
                (0, helpers_js_1.delay)(this.config.orchestrator.shutdownTimeout),
            ]);
            const shutdownDuration = Date.now() - shutdownStart;
            this.eventBus.emit(types_js_1.SystemEvents.SYSTEM_SHUTDOWN, { reason: 'Graceful shutdown' });
            ...
            await this.emergencyShutdown();
            throw new errors_js_1.ShutdownError('Failed to shutdown gracefully', { error });
        }
        finally {
    ...
    async spawnAgent(profile) {
        if (!this.initialized) {
            throw new errors_js_1.SystemError('Orchestrator not initialized');
        }
        // Check agent limit
        if (this.agents.size >= this.config.orchestrator.maxConcurrentAgents) {
            throw new errors_js_1.SystemError('Maximum concurrent agents reached');
        }
        ...
        try {
            ...
            const session = await (0, helpers_js_1.retry)(() => this.sessionManager.createSession(profile), {
                maxAttempts: 3,
                initialDelay: 2000,
            });
            // Store agent profile
            ...
            // Emit event
            this.eventBus.emit(types_js_1.SystemEvents.AGENT_SPAWNED, {
                agentId: profile.id,
                profile,
                sessionId: session.id,
            });
    ...
    async terminateAgent(agentId) {
        ...
        if (!profile) {
            throw new errors_js_1.SystemError(`Agent not found: ${agentId}`);
        }
        ...
        try {
            ...
            // Emit event
            this.eventBus.emit(types_js_1.SystemEvents.AGENT_TERMINATED, {
                agentId,
            ...
            await this.taskAssignmentCircuitBreaker.execute(async () => {
                ...
                if (!task.assignedAgent) {
                    if (this.taskQueue.length >= this.config.orchestrator.taskQueueSize) {
                        throw new errors_js_1.SystemError('Task queue is full');
                    }
                    this.taskQueue.push(task);
                    this.eventBus.emit(types_js_1.SystemEvents.TASK_CREATED, { task });
                ...
                if (!agent) {
                    throw new errors_js_1.SystemError(`Agent not found: ${task.assignedAgent}`);
                }
                await this.coordinationManager.assignTask(task, task.assignedAgent);
                this.eventBus.emit(types_js_1.SystemEvents.TASK_ASSIGNED, {
                    taskId: task.id,
    ...
    setupEventHandlers() {
        ...
        this.eventBus.on(types_js_1.SystemEvents.TASK_STARTED, (data) => {
        ...
        this.eventBus.on(types_js_1.SystemEvents.TASK_COMPLETED, async (data) => {
        ...
        this.eventBus.on(types_js_1.SystemEvents.TASK_FAILED, async (data) => {
        ...
        // Handle agent events
        this.eventBus.on(types_js_1.SystemEvents.AGENT_ERROR, async (data) => {
        ...
        this.eventBus.on(types_js_1.SystemEvents.AGENT_IDLE, async (data) => {
            const { agentId } = data;
        ...
        this.eventBus.on(types_js_1.SystemEvents.SYSTEM_ERROR, (data) => {
        ...
        this.eventBus.on(types_js_1.SystemEvents.DEADLOCK_DETECTED, (data) => {
    ...
    startHealthChecks() {
        this.healthCheckInterval = setInterval(async () => {
            try {
                ...
                this.eventBus.emit(types_js_1.SystemEvents.SYSTEM_HEALTHCHECK, { status: health });
    ...
    async getComponentHealth(name, check) {
        try {
            const result = await Promise.race([
                check(),
                (0, helpers_js_1.delay)(5000).then(() => ({ healthy: false, error: 'Health check timeout' })),
            ]);
            const health = {
                name,
    ...
    async initializeComponent(name, init) {
        try {
            await (0, helpers_js_1.retry)(init, { maxAttempts: 3, initialDelay: 2000 });
            this.logger.info(`${name} initialized`);
        }
        catch (error) {
            this.logger.error(`Failed to initialize ${name}`, error);
            throw new errors_js_1.InitializationError(name, { error });
        }
    }
    async shutdownComponent(name, shutdown) {
        try {
            await Promise.race([
                shutdown(),
                (0, helpers_js_1.delay)(10000), // 10 second timeout per component
            ]);
    ...
    async handleAgentError(agentId, error) {
        ...
        if (errorCount < 3) {
            ...
            try {
                ...
                await (0, helpers_js_1.delay)(2000); // Wait before restart
    ...
    async cancelAgentTasks(agentId) {
        try {
            ...
            for (const task of tasks) {
                ...
                this.eventBus.emit(types_js_1.SystemEvents.TASK_CANCELLED, {
                    taskId: task.id,
