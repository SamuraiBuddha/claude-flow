...
class PersistenceManager {
    ...
    constructor(dataDir = './memory') {
        this.dbPath = (0, path_1.join)(dataDir, 'claude-flow.db');
    }
    async initialize() {
        // Ensure directory exists
        await (0, promises_1.mkdir)((0, path_1.join)(this.dbPath, '..'), { recursive: true });
        // Open database
    ...
    async getStats() {
        ...
        const pendingTasks = this.db
            .prepare("SELECT COUNT(*) as count FROM tasks WHERE status IN ('pending', 'in_progress', 'assigned')")
            .get();
