...
class AnalyticsManager extends events_1.EventEmitter {
    ...
    async initialize() {
        try {
            await (0, promises_1.mkdir)(this.analyticsPath, { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.analyticsPath, 'metrics'), { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.analyticsPath, 'dashboards'), { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.analyticsPath, 'insights'), { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.analyticsPath, 'models'), { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.analyticsPath, 'reports'), { recursive: true });
    ...
    async loadConfigurations() {
        try {
            // Load dashboards
            const dashboardFiles = await (0, promises_1.readdir)((0, path_1.join)(this.analyticsPath, 'dashboards'));
            for (const file of dashboardFiles.filter((f) => f.endsWith('.json'))) {
                const content = await (0, promises_1.readFile)((0, path_1.join)(this.analyticsPath, 'dashboards', file), 'utf-8');
                const dashboard = JSON.parse(content);
                this.dashboards.set(dashboard.id, dashboard);
            }
            // Load insights
            const insightFiles = await (0, promises_1.readdir)((0, path_1.join)(this.analyticsPath, 'insights'));
            for (const file of insightFiles.filter((f) => f.endsWith('.json'))) {
                const content = await (0, promises_1.readFile)((0, path_1.join)(this.analyticsPath, 'insights', file), 'utf-8');
                const insight = JSON.parse(content);
                this.insights.set(insight.id, insight);
            }
            // Load models
            const modelFiles = await (0, promises_1.readdir)((0, path_1.join)(this.analyticsPath, 'models'));
            for (const file of modelFiles.filter((f) => f.endsWith('.json'))) {
                const content = await (0, promises_1.readFile)((0, path_1.join)(this.analyticsPath, 'models', file), 'utf-8');
    ...
    async persistMetric(metric) {
        ...
        const filePath = (0, path_1.join)(this.analyticsPath, 'metrics', `${date}.json`);
        try {
            let existingData = [];
            try {
            ...
            await (0, promises_1.writeFile)(filePath, JSON.stringify(existingData, null, 2));
        }
        catch (error) {
    ...
    async saveDashboard(dashboard) {
        const filePath = (0, path_1.join)(this.analyticsPath, 'dashboards', `${dashboard.id}.json`);
        await (0, promises_1.writeFile)(filePath, JSON.stringify(dashboard, null, 2));
    }
    async saveInsight(insight) {
        const filePath = (0, path_1.join)(this.analyticsPath, 'insights', `${insight.id}.json`);
        await (0, promises_1.writeFile)(filePath, JSON.stringify(insight, null, 2));
    }
    async saveModel(model) {
        const filePath = (0, path_1.join)(this.analyticsPath, 'models', `${model.id}.json`);
        await (0, promises_1.writeFile)(filePath, JSON.stringify(model, null, 2));
    }
}
