...
class CloudManager extends events_1.EventEmitter {
    providers = new Map();
    resources = new Map();
    ...
    async initialize() {
        try {
            await (0, promises_1.mkdir)(this.cloudPath, { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.cloudPath, 'providers'), { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.cloudPath, 'resources'), { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.cloudPath, 'infrastructures'), { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.cloudPath, 'templates'), { recursive: true });
    ...
    async loadConfigurations() {
        try {
            // Load providers
            const providerFiles = await (0, promises_1.readdir)((0, path_1.join)(this.cloudPath, 'providers'));
            for (const file of providerFiles.filter((f) => f.endsWith('.json'))) {
                const content = await (0, promises_1.readFile)((0, path_1.join)(this.cloudPath, 'providers', file), 'utf-8');
                const provider = JSON.parse(content);
                this.providers.set(provider.id, provider);
            }
            // Load resources
            const resourceFiles = await (0, promises_1.readdir)((0, path_1.join)(this.cloudPath, 'resources'));
            for (const file of resourceFiles.filter((f) => f.endsWith('.json'))) {
                const content = await (0, promises_1.readFile)((0, path_1.join)(this.cloudPath, 'resources', file), 'utf-8');
                const resource = JSON.parse(content);
                this.resources.set(resource.id, resource);
            }
            // Load infrastructures
            const infraFiles = await (0, promises_1.readdir)((0, path_1.join)(this.cloudPath, 'infrastructures'));
            for (const file of infraFiles.filter((f) => f.endsWith('.json'))) {
                const content = await (0, promises_1.readFile)((0, path_1.join)(this.cloudPath, 'infrastructures', file), 'utf-8');
    ...
    async saveProvider(provider) {
        const filePath = (0, path_1.join)(this.cloudPath, 'providers', `${provider.id}.json`);
        await (0, promises_1.writeFile)(filePath, JSON.stringify(provider, null, 2));
    }
    async saveResource(resource) {
        const filePath = (0, path_1.join)(this.cloudPath, 'resources', `${resource.id}.json`);
        await (0, promises_1.writeFile)(filePath, JSON.stringify(resource, null, 2));
    }
    async saveInfrastructure(infrastructure) {
        const filePath = (0, path_1.join)(this.cloudPath, 'infrastructures', `${infrastructure.id}.json`);
        await (0, promises_1.writeFile)(filePath, JSON.stringify(infrastructure, null, 2));
    }
