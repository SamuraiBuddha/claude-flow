...
class DeploymentManager extends events_1.EventEmitter {
    ...
    async initialize() {
        try {
            await (0, promises_1.mkdir)(this.deploymentsPath, { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.deploymentsPath, 'environments'), { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.deploymentsPath, 'strategies'), { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.deploymentsPath, 'pipelines'), { recursive: true });
    ...
    async executeCommand(deployment, stage, command) {
        return new Promise((resolve, reject) => {
            ...
            const childProcess = (0, child_process_1.spawn)(command.command, command.args, {
    ...
    async loadConfigurations() {
        ...
        try {
            const envFiles = await (0, promises_1.readdir)((0, path_1.join)(this.deploymentsPath, 'environments'));
            for (const file of envFiles.filter((f) => f.endsWith('.json'))) {
                const content = await (0, promises_1.readFile)((0, path_1.join)(this.deploymentsPath, 'environments', file), 'utf-8');
                const env = JSON.parse(content);
                this.environments.set(env.id, env);
            }
            const strategyFiles = await (0, promises_1.readdir)((0, path_1.join)(this.deploymentsPath, 'strategies'));
            for (const file of strategyFiles.filter((f) => f.endsWith('.json'))) {
                const content = await (0, promises_1.readFile)((0, path_1.join)(this.deploymentsPath, 'strategies', file), 'utf-8');
                const strategy = JSON.parse(content);
                this.strategies.set(strategy.id, strategy);
            }
            const pipelineFiles = await (0, promises_1.readdir)((0, path_1.join)(this.deploymentsPath, 'pipelines'));
            for (const file of pipelineFiles.filter((f) => f.endsWith('.json'))) {
                const content = await (0, promises_1.readFile)((0, path_1.join)(this.deploymentsPath, 'pipelines', file), 'utf-8');
    ...
    async saveEnvironment(environment) {
        const filePath = (0, path_1.join)(this.deploymentsPath, 'environments', `${environment.id}.json`);
        await (0, promises_1.writeFile)(filePath, JSON.stringify(environment, null, 2));
    }
    async saveStrategy(strategy) {
        const filePath = (0, path_1.join)(this.deploymentsPath, 'strategies', `${strategy.id}.json`);
        await (0, promises_1.writeFile)(filePath, JSON.stringify(strategy, null, 2));
    }
    async saveDeployment(deployment) {
        const filePath = (0, path_1.join)(this.deploymentsPath, `${deployment.id}.json`);
        await (0, promises_1.writeFile)(filePath, JSON.stringify(deployment, null, 2));
    }
    ...
    evaluateCommandSuccess(command, exitCode, stdout, stderr) {
        if (command.successCriteria.exitCode !== undefined &&
