...
class ProjectManager extends events_1.EventEmitter {
    ...
    async initialize() {
        try {
            await (0, promises_1.mkdir)(this.projectsPath, { recursive: true });
    ...
    async deleteProject(projectId, userId = 'system') {
        ...
        const archivePath = (0, path_1.join)(this.projectsPath, 'archived');
        await (0, promises_1.mkdir)(archivePath, { recursive: true });
        await (0, promises_1.writeFile)((0, path_1.join)(archivePath, `${projectId}.json`), JSON.stringify(project, null, 2));
    ...
    async loadProjects() {
        try {
            const files = await (0, promises_1.readdir)(this.projectsPath);
            const projectFiles = files.filter((f) => f.endsWith('.json') && !f.startsWith('.'));
            for (const file of projectFiles) {
                try {
                    const content = await (0, promises_1.readFile)((0, path_1.join)(this.projectsPath, file), 'utf-8');
    ...
    async saveProject(project) {
        const filePath = (0, path_1.join)(this.projectsPath, `${project.id}.json`);
        await (0, promises_1.writeFile)(filePath, JSON.stringify(project, null, 2));
    }
