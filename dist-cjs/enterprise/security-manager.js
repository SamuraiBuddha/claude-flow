...
class SecurityManager extends events_1.EventEmitter {
    ...
    async initialize() {
        try {
            await (0, promises_1.mkdir)(this.securityPath, { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.securityPath, 'scans'), { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.securityPath, 'policies'), { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.securityPath, 'incidents'), { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.securityPath, 'reports'), { recursive: true });
            await (0, promises_1.mkdir)((0, path_1.join)(this.securityPath, 'databases'), { recursive: true });
    ...
    async createSecurityScan(scanData) {
        const scan = {
            ...
            configuration: {
                ...
                formats: ['json', 'html'],
                outputPath: (0, path_1.join)(this.securityPath, 'reports'),
    ...
    async loadConfigurations() {
        try {
            // Load scans
            const scanFiles = await (0, promises_1.readdir)((0, path_1.join)(this.securityPath, 'scans'));
            for (const file of scanFiles.filter((f) => f.endsWith('.json'))) {
                const content = await (0, promises_1.readFile)((0, path_1.join)(this.securityPath, 'scans', file), 'utf-8');
                const scan = JSON.parse(content);
                this.scans.set(scan.id, scan);
            }
            // Load policies
            const policyFiles = await (0, promises_1.readdir)((0, path_1.join)(this.securityPath, 'policies'));
            for (const file of policyFiles.filter((f) => f.endsWith('.json'))) {
                const content = await (0, promises_1.readFile)((0, path_1.join)(this.securityPath, 'policies', file), 'utf-8');
                const policy = JSON.parse(content);
                this.policies.set(policy.id, policy);
            }
            // Load incidents
            const incidentFiles = await (0, promises_1.readdir)((0, path_1.join)(this.securityPath, 'incidents'));
            for (const file of incidentFiles.filter((f) => f.endsWith('.json'))) {
                const content = await (0, promises_1.readFile)((0, path_1.join)(this.securityPath, 'incidents', file), 'utf-8');
    ...
    async saveScan(scan) {
        const filePath = (0, path_1.join)(this.securityPath, 'scans', `${scan.id}.json`);
        await (0, promises_1.writeFile)(filePath, JSON.stringify(scan, null, 2));
    }
    async savePolicy(policy) {
        const filePath = (0, path_1.join)(this.securityPath, 'policies', `${policy.id}.json`);
        await (0, promises_1.writeFile)(filePath, JSON.stringify(policy, null, 2));
    }
    async saveIncident(incident) {
        const filePath = (0, path_1.join)(this.securityPath, 'incidents', `${incident.id}.json`);
        await (0, promises_1.writeFile)(filePath, JSON.stringify(incident, null, 2));
    }
