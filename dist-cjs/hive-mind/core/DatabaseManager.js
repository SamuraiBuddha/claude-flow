...
async function loadSQLiteWrapper() {
    const module = await Promise.resolve().then(() => __importStar(require('../../memory/sqlite-wrapper.js')));
...
class DatabaseManager extends events_1.EventEmitter {
    ...
    async initialize() {
        ...
        try {
            ...
            // Set database path
            this.dbPath = path_1.default.join(dataDir, 'hive-mind.db');
            // Open database
...
`);
     ...
     */
    async loadSchema() {
        const schemaPath = path_1.default.join(__dirname, '..', '..', 'db', 'hive-mind-schema.sql');
    ...
    async getRecentConsensusProposals(swarmId, limit = 10) {
