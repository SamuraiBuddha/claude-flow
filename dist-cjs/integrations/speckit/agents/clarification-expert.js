...
exports.createClarificationExpertAgent = exports.ClarificationExpertAgent = void 0;
const base_agent_js_1 = require("../../../cli/agents/base-agent.js");
/**
 * Clarification Expert Agent - Structured specification refinement
 */
class ClarificationExpertAgent extends base_agent_js_1.BaseAgent {
    questions = new Map();
    sessions = new Map();
    questionTemplates = new Map();
    ...
    async generateQuestions(task) {
        ...
        const focusCategories = task.parameters?.categories || [
            'functional',
            'non-functional',
            'technical',
            'business',
        ...
        for (const section of sections) {
            for (const category of focusCategories) {
                const questions = this.generateSectionQuestions(specId, section, category, specification);
     ...
     */
    async recordAnswers(task) {
        const answers = task.input?.answers;
     ...
     */
    async validateCompleteness(task) {
    ...
    async analyzeCoverage(task) {
        ...
        const coveredSections = new Set(specQuestions.filter(q => q.status === 'answered').map(q => q.section));
        // Analyze by category
        const byCategory = {};
        const categories = [
            'functional',
    ...
    async manageSession(task) {
        ...
        switch (action) {
            case 'start': {
                if (!specId)
                    throw new Error('specId required to start session');
                const session = {
                    id: `session-${Date.now()}`,
                    specId,
                    startedAt: new Date(),
     ...
     */
    async performGeneralClarification(task) {
        const specification = task.input?.specification;
     ...
     */
    analyzeSpecificationSections(specification) {
        if (!specification)
            return [];
        const sections = [];
     ...
     */
    inferSectionType(name) {
        const lower = name.toLowerCase();
        if (lower.includes('requirement') || lower.includes('feature'))
            return 'functional';
        if (lower.includes('performance') || lower.includes('scale'))
            return 'non-functional';
        if (lower.includes('api') || lower.includes('data'))
            return 'technical';
        if (lower.includes('user') || lower.includes('story'))
     ...
     */
    generateSectionQuestions(specId, section, category, specification) {
     ...
     */
    generateContextualQuestions(section, category) {
        const questions = [];
     ...
     */
    determineQuestionPriority(question, section, category) {
        // Critical categories
     ...
     * Link related questions
     */
    linkRelatedQuestions(questions) {
     ...
     */
    determineFollowUpNeeded(answer, question) {
        const indicators = [
            'depends',
            'not sure',
            'maybe',
     ...
     */
    identifyCategoryGaps(questions) {
        const gaps = [];
        const requiredCategories = ['functional', 'technical', 'security'];
     ...
     */
    generateCompletenessRecommendations(gaps, score) {
        ...
        if (criticalGaps.length > 0) {
            recommendations.push(`Address ${criticalGaps.length} critical gaps before proceeding`);
        }
     ...
     */
    generateCoverageRecommendations(byCategory, gapSections) {
        const recommendations = [];
...
exports.ClarificationExpertAgent = ClarificationExpertAgent;
/**
 * Factory function to create a Clarification Expert Agent
 */
const createClarificationExpertAgent = (id, config, environment, logger, eventBus, memory) => {
    const defaultConfig = {
        autonomyLevel: 0.75,
        learningEnabled: true,
        adaptationEnabled: true,
    ...
    return new ClarificationExpertAgent(id, { ...defaultConfig, ...config }, { ...defaultEnv, ...environment }, logger, eventBus, memory);
};
exports.createClarificationExpertAgent = createClarificationExpertAgent;
//# sourceMappingURL=clarification-expert.js.map