...
exports.createNFRSpecialistAgent = exports.NFRSpecialistAgent = void 0;
const base_agent_js_1 = require("../../../cli/agents/base-agent.js");
/**
 * NFR Specialist Agent - Non-Functional Requirements expert
 */
class NFRSpecialistAgent extends base_agent_js_1.BaseAgent {
    nfrs = new Map();
    tests = new Map();
    ...
    async extractNFRs(task) {
        ...
        if (specification) {
            ...
            const contentNFRs = this.extractNFRsFromContent(JSON.stringify(specification), categories, source);
    ...
    async analyzeMetrics(task) {
        ...
        for (const nfrId of nfrIds) {
            ...
            if (!nfr)
                continue;
            const nfrMetrics = metrics.filter((m) => nfr.metrics.some(nm => nm.name === m.name));
            const metricResults = [];
            for (const metric of nfr.metrics) {
                const actual = nfrMetrics.find((m) => m.name === metric.name);
                if (actual) {
    ...
    async performGeneralAnalysis(task) {
        ...
        const addressed = nfrs.filter(n => ['addressed', 'tested', 'verified'].includes(n.status)).length;
        const tested = nfrs.filter(n => ['tested', 'verified'].includes(n.status)).length;
        ...
        const criticalUnaddressed = nfrs.filter(n => n.priority === 'critical' && n.status === 'extracted').length;
        let riskLevel = 'low';
        if (criticalUnaddressed > 0)
            riskLevel = 'critical';
        else if (gaps.length > nfrs.length * 0.3)
            riskLevel = 'high';
        else if (gaps.length > nfrs.length * 0.1)
            riskLevel = 'medium';
        const report = {
            specId,
     ...
     */
    parseNFRSection(section, source) {
        const nfrs = [];
     ...
     */
    extractNFRsFromContent(content, categories, source) {
        const nfrs = [];
     ...
     */
    extractPerformanceNFRs(section, source) {
        const nfrs = [];
     ...
     */
    extractSecurityNFRs(section, source) {
        const nfrs = [];
     ...
     */
    extractScalabilityNFRs(section, source) {
        const nfrs = [];
        if (section.users) {
            nfrs.push(this.createNFR({
                category: 'scalability',
     ...
     */
    validateNFRAgainstPlan(nfr, plan) {
        const gaps = [];
     ...
     */
    generateTestsForNFR(nfr, testTypes) {
        const tests = [];
        const categoryTests = {
     ...
     */
    generateMetricRecommendations(results) {
        const recommendations = [];
        for (const result of results) {
            if (!result.passed) {
                if (result.variance > 50) {
                    recommendations.push(`Critical: ${result.metric} is ${result.variance.toFixed(1)}% off target - requires immediate attention`);
                }
                else {
                    recommendations.push(`${result.metric} is ${result.variance.toFixed(1)}% off target - optimization needed`);
                }
            }
        }
     ...
     * Assess NFR risk
     */
    assessNFRRisk(nfr, validationResult) {
        const factors = [];
        const mitigations = [];
        let riskScore = 0;
        // Priority factor
     ...
     */
    generateAnalysisRecommendations(nfrs, gaps) {
        const recommendations = [];
        if (gaps.length > 0) {
...
exports.NFRSpecialistAgent = NFRSpecialistAgent;
/**
 * Factory function to create an NFR Specialist Agent
 */
const createNFRSpecialistAgent = (id, config, environment, logger, eventBus, memory) => {
    const defaultConfig = {
        autonomyLevel: 0.8,
        learningEnabled: true,
        adaptationEnabled: true,
    ...
    return new NFRSpecialistAgent(id, { ...defaultConfig, ...config }, { ...defaultEnv, ...environment }, logger, eventBus, memory);
};
exports.createNFRSpecialistAgent = createNFRSpecialistAgent;
//# sourceMappingURL=nfr-specialist.js.map