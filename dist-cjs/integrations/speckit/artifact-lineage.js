...
exports.ArtifactLineageTracker = void 0;
...
class ArtifactLineageTracker {
    ...
    removeArtifact(artifactId) {
        ...
        this.relationships = this.relationships.filter((r) => r.sourceId !== artifactId && r.targetId !== artifactId);
     ...
     */
    recordDerivation(sourceId, targetId, relationshipType, metadata = {}) {
        ...
        const existing = this.relationships.find((r) => r.sourceId === sourceId &&
            r.targetId === targetId &&
    ...
    getLineageGraph() {
        ...
        const rootTypes = ['specification', 'constitution'];
        const rootArtifacts = [];
     ...
     */
    getLineage(artifactId, depth = this.config.maxQueryDepth) {
    ...
    queryLineage(query) {
        ...
        if (query.relationshipType) {
            const relatedIds = new Set(this.relationships
     ...
     */
    getAncestors(artifactId, depth = this.config.maxQueryDepth) {
        const ancestors = [];
     ...
     */
    getDescendants(artifactId, depth = this.config.maxQueryDepth) {
        const descendants = [];
        const visited = new Set();
        const traverse = (id, currentDepth) => {
            if (currentDepth <= 0 || visited.has(id))
     ...
     */
    getImpactedArtifacts(artifactId) {
        ...
        const recommendations = this.generateImpactRecommendations(changedArtifact, impactedArtifacts);
        return {
            changedArtifact,
            impactedArtifacts,
            impactLevel,
     ...
     */
    calculateImpactLevel(changed, impacted) {
        ...
        const hasCodeImpact = impacted.some((a) => a.type === 'code' || a.type === 'test');
        if (hasCodeImpact) {
            return 'high';
        }
     ...
     */
    generateImpactRecommendations(changed, impacted) {
     ...
     */
    buildLineageNode(artifactId, depth, visited) {
        if (depth <= 0 || visited.has(artifactId))
            return null;
     ...
     */
    calculateDepth(artifactId, visited) {
...
exports.ArtifactLineageTracker = ArtifactLineageTracker;
exports.default = ArtifactLineageTracker;
