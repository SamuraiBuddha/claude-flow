...
class SpecParser {
    ...
    parseAcceptanceScenarios(content) {
        ...
        while ((match = numberedPattern.exec(content)) !== null) {
            ...
            const exists = scenarios.some((s) => s.given === match[1].trim() && s.when === match[2].trim());
            if (!exists) {
                scenarios.push({
                    id: `AC${index++}`,
                    given: match[1].trim(),
     ...
     */
    parseRequirementsList(content, type) {
        const requirements = [];
    ...
    validateDocument(doc) {
        ...
        const nonMeasurable = doc.successCriteria.filter((sc) => typeof sc === 'object' && !sc.measurable);
