...
class ConnectionStateManager {
    ...
    constructor(logger, config) {
        ...
        this.statePath = (0, node_path_1.join)(this.config.stateDirectory, 'connection-state.json');
        this.metricsPath = (0, node_path_1.join)(this.config.stateDirectory, 'connection-metrics.json');
    ...
    async initialize() {
        ...
        try {
            ...
            await node_fs_1.promises.mkdir(this.config.stateDirectory, { recursive: true });
            // Load existing state
    ...
    async loadState() {
        try {
            const data = await node_fs_1.promises.readFile(this.statePath, 'utf-8');
    ...
    async loadMetrics() {
        try {
            const data = await node_fs_1.promises.readFile(this.metricsPath, 'utf-8');
    ...
    async persistState() {
        ...
        try {
            if (this.currentState) {
                await node_fs_1.promises.writeFile(this.statePath, JSON.stringify(this.currentState, null, 2), 'utf-8');
            }
            // Also persist metrics
            await node_fs_1.promises.writeFile(this.metricsPath, JSON.stringify({
                ...this.metrics,
                connectionHistory: this.connectionHistory,
            }, null, 2), 'utf-8');
