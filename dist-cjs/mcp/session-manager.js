...
class SessionManager {
    ...
    createSession(transport) {
        ...
        if (this.sessions.size >= this.maxSessions) {
            ...
            this.cleanupExpiredSessions();
            if (this.sessions.size >= this.maxSessions) {
                throw new errors_js_1.MCPError('Maximum number of sessions reached');
            }
        }
    ...
    initializeSession(sessionId, params) {
        ...
        if (!session) {
            throw new errors_js_1.MCPError(`Session not found: ${sessionId}`);
        }
    ...
    validateProtocolVersion(version) {
        ...
        if (!isSupported) {
            throw new errors_js_1.MCPError(`Unsupported protocol version: ${version.major}.${version.minor}.${version.patch}`, { supportedVersions });
        }
    }
    authenticateToken(credentials) {
        ...
        return this.authConfig.tokens.some((validToken) => {
            ...
            return (0, node_crypto_1.timingSafeEqual)(validTokenBytes, providedTokenBytes);
        });
    }
    authenticateBasic(credentials) {
        ...
        return (0, node_crypto_1.timingSafeEqual)(hashedPasswordBytes, expectedHashedPasswordBytes);
    }
