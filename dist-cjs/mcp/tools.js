 ...
 */
class ToolRegistry extends node_events_1.EventEmitter {
    logger;
    tools = new Map();
    ...
    register(tool, capability) {
        if (this.tools.has(tool.name)) {
            throw new errors_js_1.MCPError(`Tool already registered: ${tool.name}`);
        }
    ...
    unregister(name) {
        if (!this.tools.has(name)) {
            throw new errors_js_1.MCPError(`Tool not found: ${name}`);
        }
    ...
    validateTool(tool) {
        if (!tool.name || typeof tool.name !== 'string') {
            throw new errors_js_1.MCPError('Tool name must be a non-empty string');
        }
        if (!tool.description || typeof tool.description !== 'string') {
            throw new errors_js_1.MCPError('Tool description must be a non-empty string');
        }
        if (typeof tool.handler !== 'function') {
            throw new errors_js_1.MCPError('Tool handler must be a function');
        }
        if (!tool.inputSchema || typeof tool.inputSchema !== 'object') {
            throw new errors_js_1.MCPError('Tool inputSchema must be an object');
        }
        // Validate tool name format (namespace/name)
        if (!tool.name.includes('/')) {
            throw new errors_js_1.MCPError('Tool name must be in format: namespace/name');
        }
    }
    /**
    ...
    validateInput(tool, input) {
        ...
        if (schema.type === 'object' && schema.properties) {
            if (typeof input !== 'object' || input === null) {
                throw new errors_js_1.MCPError('Input must be an object');
            }
            ...
            if (schema.required && Array.isArray(schema.required)) {
                for (const prop of schema.required) {
                    if (!(prop in inputObj)) {
                        throw new errors_js_1.MCPError(`Missing required property: ${prop}`);
                    }
                }
            }
            ...
            for (const [prop, propSchema] of Object.entries(schema.properties)) {
                if (prop in inputObj) {
                    ...
                    if (expectedType && !this.checkType(value, expectedType)) {
                        throw new errors_js_1.MCPError(`Invalid type for property ${prop}: expected ${expectedType}`);
                    }
                }
            }
        }
    ...
    async checkToolCapabilities(toolName, context) {
        ...
        if (capability.requiredPermissions && context?.permissions) {
            ...
            if (!hasAllPermissions) {
                throw new errors_js_1.MCPError(`Insufficient permissions for tool ${toolName}. Required: ${capability.requiredPermissions.join(', ')}`);
            }
        }
        ...
        if (context?.protocolVersion) {
            ...
            if (!isCompatible) {
                throw new errors_js_1.MCPError(`Tool ${toolName} is not compatible with protocol version ${context.protocolVersion.major}.${context.protocolVersion.minor}.${context.protocolVersion.patch}`);
            }
        }
    }
    /**
    ...
    getToolMetrics(name) {
        if (name) {
            const metrics = this.metrics.get(name);
            if (!metrics) {
                throw new errors_js_1.MCPError(`Metrics not found for tool: ${name}`);
            }
            return metrics;
        }
