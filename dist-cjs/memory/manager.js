...
const sqlite_js_1 = require("./backends/sqlite.js");
const markdown_js_1 = require("./backends/markdown.js");
...
class MemoryManager {
    ...
    constructor(config, eventBus, logger) {
        ...
        // Initialize cache
        this.cache = new cache_js_1.MemoryCache(this.config.cacheSizeMB * 1024 * 1024, // Convert MB to bytes
        this.logger);
        // Initialize indexer
        this.indexer = new indexer_js_1.MemoryIndexer(this.logger);
    }
    async initialize() {
        if (this.initialized) {
            return;
        ...
        try {
            ...
            throw new errors_js_1.MemoryError('Memory manager initialization failed', { error });
        }
    }
    async shutdown() {
        if (!this.initialized) {
    ...
    async createBank(agentId) {
        if (!this.initialized) {
            throw new errors_js_1.MemoryError('Memory manager not initialized');
        }
        const bank = {
    ...
    async closeBank(bankId) {
        ...
        if (!bank) {
            throw new errors_js_1.MemoryError(`Memory bank not found: ${bankId}`);
        }
    ...
    async store(entry) {
        ...
        try {
            ...
            throw new errors_js_1.MemoryError('Failed to store memory entry', { error });
        }
    }
    async retrieve(id) {
        if (!this.initialized) {
    ...
    async query(query) {
        ...
        try {
            ...
            throw new errors_js_1.MemoryError('Failed to query memory', { error });
        }
    }
    async update(id, updates) {
        ...
        if (!existing) {
            throw new errors_js_1.MemoryError(`Memory entry not found: ${id}`);
        }
    ...
    createBackend() {
        switch (this.config.backend) {
            case 'sqlite':
                return new sqlite_js_1.SQLiteBackend(this.config.sqlitePath || './claude-flow.db', this.logger);
            case 'markdown':
                return new markdown_js_1.MarkdownBackend(this.config.markdownDir || './memory', this.logger);
            case 'hybrid':
                // Use SQLite for structured data and Markdown for human-readable backup
                return new HybridBackend(new sqlite_js_1.SQLiteBackend(this.config.sqlitePath || './claude-flow.db', this.logger), new markdown_js_1.MarkdownBackend(this.config.markdownDir || './memory', this.logger), this.logger);
            default:
                throw new errors_js_1.MemoryError(`Unknown memory backend: ${this.config.backend}`);
        }
    }
    startSyncInterval() {
