...
const progress_reporter_js_1 = require("./progress-reporter.js");
const migration_validator_js_1 = require("./migration-validator.js");
...
class MigrationRunner {
    ...
    constructor(options) {
        ...
        this.progress = new progress_reporter_js_1.ProgressReporter();
        this.analyzer = new migration_analyzer_js_1.MigrationAnalyzer();
        this.validator = new migration_validator_js_1.MigrationValidator();
    ...
    async run() {
        ...
        try {
            ...
            if (!this.options.force && !this.options.dryRun) {
                ...
                if (!confirm) {
                    logger_js_1.logger.info('Migration cancelled');
                    return result;
                }
            }
            // Create backup
            ...
            if (result.rollbackPath && !this.options.dryRun) {
                logger_js_1.logger.warn('Attempting automatic rollback...');
                try {
                    await this.rollback(result.rollbackPath);
                    logger_js_1.logger.success('Rollback completed');
                }
                catch (rollbackError) {
                    logger_js_1.logger.error('Rollback failed:', rollbackError);
                }
            }
        }
        return result;
    ...
    async fullMigration(result) {
        ...
        if (this.options.dryRun) {
            logger_js_1.logger.info('[DRY RUN] Would replace entire .claude folder');
            return;
        }
    ...
    async selectiveMigration(result, analysis) {
        ...
        for (const command of Object.values(this.manifest.files.commands)) {
            ...
            if (this.options.dryRun) {
                logger_js_1.logger.info(`[DRY RUN] Would copy ${command.source} to ${command.target}`);
            }
            else {
        ...
        for (const guide of guides) {
            ...
            if (await fs.pathExists(sourceFile)) {
                if (this.options.dryRun) {
                    logger_js_1.logger.info(`[DRY RUN] Would copy ${guide}`);
                }
                else {
                    await fs.copy(sourceFile, targetFile, { overwrite: true });
    ...
    async copyRequiredFiles(result) {
        ...
        for (const file of files) {
            ...
            if (await fs.pathExists(sourcePath)) {
                if (this.options.dryRun) {
                    logger_js_1.logger.info(`[DRY RUN] Would copy ${file.source}`);
                }
                else {
    ...
    async createBackup() {
        ...
        if (await fs.pathExists(claudePath)) {
            ...
            const files = await (0, glob_1.glob)('**/*', { cwd: claudePath, nodir: true });
            for (const file of files) {
        ...
        logger_js_1.logger.success(`Backup created at ${backupPath}`);
        return backup;
    }
    async rollback(timestamp) {
        ...
        logger_js_1.logger.info(`Rolling back from ${backupPath}...`);
        // Confirm rollback
    ...
    async listBackups() {
        const backupDir = path.join(this.options.projectPath, this.options.backupDir || '.claude-backup');
        if (!(await fs.pathExists(backupDir))) {
            logger_js_1.logger.info('No backups found');
            return;
        }
        const backups = await fs.readdir(backupDir);
        if (backups.length === 0) {
