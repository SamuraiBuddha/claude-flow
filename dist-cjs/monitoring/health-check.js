...
class HealthCheckManager {
    ...
    async performHealthCheck() {
        ...
        try {
            ...
            const duration = Date.now() - startTime;
            this.logger.error('Health check failed:', (0, error_handler_js_1.getErrorMessage)(error));
            this.eventBus.emit('health:check:failed', {
                error: (0, error_handler_js_1.getErrorMessage)(error),
                duration,
                timestamp: Date.now(),
            });
            throw error;
        ...
        return checks.map((result, index) => {
            if (result.status === 'fulfilled') {
                ...
                return {
                    ...
                    healthy: false,
                    message: (0, error_handler_js_1.getErrorMessage)(result.reason),
                    timestamp: Date.now(),
                };
            }
        });
    ...
    async checkComponent(componentName) {
        ...
        try {
            ...
            const componentWithHealthCheck = component;
            if (typeof componentWithHealthCheck.healthCheck === 'function') {
                const result = await Promise.race([
                    componentWithHealthCheck.healthCheck(),
            ...
            return {
                component: componentName,
                healthy: false,
                message: (0, error_handler_js_1.getErrorMessage)(error),
                timestamp: Date.now(),
            };
        }
    }
    ...
    async collectSystemMetrics() {
        ...
        try {
            ...
            const agentManagerWithMetrics = agentManager;
            if (agentManagerWithMetrics && typeof agentManagerWithMetrics.getMetrics === 'function') {
                const agentMetrics = await agentManagerWithMetrics.getMetrics();
                activeAgents = agentMetrics.activeAgents || 0;
            }
            const taskEngineWithMetrics = taskEngine;
            if (taskEngineWithMetrics && typeof taskEngineWithMetrics.getMetrics === 'function') {
            ...
            this.logger.error('Failed to collect system metrics:', (0, error_handler_js_1.getErrorMessage)(error));
            return {
                cpu: 0,
                memory: 0,
                network: 0,
    ...
    setupEventHandlers() {
        ...
        this.eventBus.on('component:status:updated', (data) => {
            const status = data;
