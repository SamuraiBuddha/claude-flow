...
// Import providers
const anthropic_provider_js_1 = require("./anthropic-provider.js");
const openai_provider_js_1 = require("./openai-provider.js");
const google_provider_js_1 = require("./google-provider.js");
const cohere_provider_js_1 = require("./cohere-provider.js");
...
class ProviderManager extends events_1.EventEmitter {
    ...
    async createProvider(name, config) {
        ...
        try {
            ...
            switch (name) {
                case 'anthropic':
                    provider = new anthropic_provider_js_1.AnthropicProvider(providerOptions);
                    break;
                case 'openai':
                    provider = new openai_provider_js_1.OpenAIProvider(providerOptions);
                    break;
                case 'google':
                    provider = new google_provider_js_1.GoogleProvider(providerOptions);
                    break;
                case 'cohere':
                    provider = new cohere_provider_js_1.CohereProvider(providerOptions);
                    break;
                case 'ollama':
                    provider = new ollama_provider_js_1.OllamaProvider(providerOptions);
                    break;
                default:
    ...
    getErrorCondition(error) {
        if ((0, types_js_1.isRateLimitError)(error)) {
            return 'rate_limit';
        }
