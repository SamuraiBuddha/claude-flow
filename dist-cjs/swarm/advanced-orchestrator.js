...
exports.AdvancedSwarmOrchestrator = void 0;
const node_events_1 = require("node:events");
const node_perf_hooks_1 = require("node:perf_hooks");
const logger_js_1 = require("../core/logger.js");
const helpers_js_1 = require("../utils/helpers.js");
const swarm_coordinator_js_1 = require("../coordination/swarm-coordinator.js");
const advanced_scheduler_js_1 = require("../coordination/advanced-scheduler.js");
const swarm_monitor_js_1 = require("../coordination/swarm-monitor.js");
const manager_js_1 = require("../memory/manager.js");
const executor_js_1 = __importDefault(require("./executor.js"));
class AdvancedSwarmOrchestrator extends node_events_1.EventEmitter {
    logger;
    config;
    ...
    constructor(config = {}) {
        super();
        this.logger = new logger_js_1.Logger('AdvancedSwarmOrchestrator');
        this.config = this.createDefaultConfig(config);
        // Initialize components
        this.coordinator = new swarm_coordinator_js_1.SwarmCoordinator({
        ...
        this.memoryManager = new manager_js_1.MemoryManager({
            backend: 'sqlite',
    ...
    async createSwarm(objective, strategy = 'auto', options = {}) {
        ...
        const context = {
            ...
            agents: new Map(),
            tasks: new Map(),
            scheduler: new advanced_scheduler_js_1.AdvancedTaskScheduler({
            ...
            monitor: new swarm_monitor_js_1.SwarmMonitor({
                updateInterval: 1000,
                enableAlerts: true,
                enableHistory: true,
            ...
            taskExecutor: new executor_js_1.default({
    ...
    getOrchestratorMetrics() {
        ...
        return {
            ...
            system: {
                ...
                uptime: this.isRunning ? Date.now() - node_perf_hooks_1.performance.timeOrigin : 0,
    ...
    async decomposeObjective(objective) {
        const tasks = [];
        const baseTaskId = (0, helpers_js_1.generateId)('task');
        switch (objective.strategy) {
            case 'research':
                tasks.push(this.createTaskDefinition(`${baseTaskId}-1`, 'research', 'Conduct comprehensive research', 'high', []), this.createTaskDefinition(`${baseTaskId}-2`, 'analysis', 'Analyze research findings', 'high', [`${baseTaskId}-1`]), this.createTaskDefinition(`${baseTaskId}-3`, 'synthesis', 'Synthesize insights and recommendations', 'high', [`${baseTaskId}-2`]), this.createTaskDefinition(`${baseTaskId}-4`, 'documentation', 'Create research documentation', 'medium', [`${baseTaskId}-3`]));
                break;
            case 'development':
                tasks.push(this.createTaskDefinition(`${baseTaskId}-1`, 'system-design', 'Design system architecture', 'high', []), this.createTaskDefinition(`${baseTaskId}-2`, 'code-generation', 'Generate core implementation', 'high', [`${baseTaskId}-1`]), this.createTaskDefinition(`${baseTaskId}-3`, 'unit-testing', 'Create comprehensive tests', 'high', [`${baseTaskId}-2`]), this.createTaskDefinition(`${baseTaskId}-4`, 'integration-testing', 'Perform integration testing', 'high', [`${baseTaskId}-3`]), this.createTaskDefinition(`${baseTaskId}-5`, 'code-review', 'Conduct code review', 'medium', [`${baseTaskId}-4`]), this.createTaskDefinition(`${baseTaskId}-6`, 'documentation', 'Create technical documentation', 'medium', [`${baseTaskId}-5`]));
                break;
            case 'analysis':
                tasks.push(this.createTaskDefinition(`${baseTaskId}-1`, 'data-collection', 'Collect and prepare data', 'high', []), this.createTaskDefinition(`${baseTaskId}-2`, 'data-analysis', 'Perform statistical analysis', 'high', [`${baseTaskId}-1`]), this.createTaskDefinition(`${baseTaskId}-3`, 'visualization', 'Create data visualizations', 'medium', [`${baseTaskId}-2`]), this.createTaskDefinition(`${baseTaskId}-4`, 'reporting', 'Generate analysis report', 'high', [`${baseTaskId}-2`, `${baseTaskId}-3`]));
                break;
            default: // auto
                // Use AI-driven decomposition based on objective description
                tasks.push(this.createTaskDefinition(`${baseTaskId}-1`, 'exploration', 'Explore and understand requirements', 'high', []), this.createTaskDefinition(`${baseTaskId}-2`, 'planning', 'Create detailed execution plan', 'high', [`${baseTaskId}-1`]), this.createTaskDefinition(`${baseTaskId}-3`, 'execution', 'Execute main tasks', 'high', [`${baseTaskId}-2`]), this.createTaskDefinition(`${baseTaskId}-4`, 'validation', 'Validate and test results', 'high', [`${baseTaskId}-3`]), this.createTaskDefinition(`${baseTaskId}-5`, 'completion', 'Finalize and document outcomes', 'medium', [`${baseTaskId}-4`]));
        }
    ...
    async spawnRequiredAgents(context) {
        ...
        for (const agentType of requiredTypes) {
            ...
            await this.coordinator.registerAgent(agent.name, agent.type, { tools: agent.capabilities });
            agents.push(agent);
    ...
    async scheduleAndExecuteTasks(context) {
        ...
        for (const task of context.tasks.values()) {
            await context.scheduler.assignTask(task);
        }
...
exports.AdvancedSwarmOrchestrator = AdvancedSwarmOrchestrator;
exports.default = AdvancedSwarmOrchestrator;
