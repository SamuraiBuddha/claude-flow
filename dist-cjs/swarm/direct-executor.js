...
exports.DirectTaskExecutor = void 0;
...
class DirectTaskExecutor {
    ...
    constructor(config = {}) {
        this.logger =
            config.logger ||
                new logger_js_1.Logger({ level: 'info', format: 'text', destination: 'console' }, { component: 'DirectTaskExecutor' });
    ...
    async executeTaskByType(task, agent, targetDir) {
        ...
        switch (agent.type) {
            ...
            case 'coder':
                if (isRestAPI)
                    return this.createRestAPI(targetDir || process.cwd(), task);
                if (isTodo)
                    return this.createTodoApp(targetDir || process.cwd(), task);
                if (isChat)
                    return this.createChatApp(targetDir || process.cwd(), task);
                if (isAuth)
                    return this.createAuthService(targetDir || process.cwd(), task);
                if (isHelloWorld)
                    return this.createHelloWorld(targetDir || process.cwd(), task);
                if (isCalculator)
                    return this.createCalculator(targetDir || process.cwd(), task);
                return this.createGenericApp(targetDir || process.cwd(), task);
            case 'tester':
                return this.executeTestingTask(task, targetDir);
            case 'reviewer':
                if (task.name.toLowerCase().includes('analyze') ||
...
exports.DirectTaskExecutor = DirectTaskExecutor;
