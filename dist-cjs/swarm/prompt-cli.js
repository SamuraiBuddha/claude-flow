...
program
    ...
    try {
        const configManager = new prompt_utils_js_1.PromptConfigManager();
        ...
        copyOptions.progressCallback = (progress) => {
            if (!progressBar) {
                progressBar = (0, prompt_utils_js_1.createProgressBar)(progress.total);
            }
        ...
        const copyFunction = options.enhanced ? prompt_copier_enhanced_js_1.copyPromptsEnhanced : prompt_copier_js_1.copyPrompts;
        ...
        console.log(`Duration: ${(0, prompt_utils_js_1.formatDuration)(result.duration)}`);
...
program
    ...
    .action(async (options) => {
    try {
        const resolver = new prompt_utils_js_1.PromptPathResolver(options.base);
...
program
    ...
    try {
        ...
        for (const file of files) {
            const result = await prompt_utils_js_1.PromptValidator.validatePromptFile(file);
            if (result.valid) {
                validFiles++;
                console.log(`✅ ${file}`);
            }
...
program
    ...
    try {
        const { PromptCopier } = await Promise.resolve().then(() => __importStar(require('./prompt-copier.js')));
