...
const vscode_js_1 = require("./adapters/vscode.js");
const native_js_1 = require("./adapters/native.js");
...
class TerminalManager {
    ...
    constructor(config, eventBus, logger) {
        ...
        this.pool = new pool_js_1.TerminalPool(this.config.poolSize, this.config.recycleAfter, this.adapter, this.logger);
    }
    async initialize() {
        if (this.initialized) {
            return;
        ...
        try {
            ...
            throw new errors_js_1.TerminalError('Terminal manager initialization failed', { error });
        }
    }
    async shutdown() {
        if (!this.initialized) {
    ...
    async spawnTerminal(profile) {
        if (!this.initialized) {
            throw new errors_js_1.TerminalError('Terminal manager not initialized');
        }
        ...
        try {
            ...
            // Create session
            const session = new session_js_1.TerminalSession(terminal, profile, this.config.commandTimeout, this.logger);
            // Initialize session
            ...
            throw new errors_js_1.TerminalSpawnError('Failed to spawn terminal', { error });
        }
    }
    async terminateTerminal(terminalId) {
        const session = this.sessions.get(terminalId);
        if (!session) {
            throw new errors_js_1.TerminalError(`Terminal not found: ${terminalId}`);
        }
    ...
    createAdapter() {
        switch (this.config.type) {
            case 'vscode':
                return new vscode_js_1.VSCodeAdapter(this.logger);
            case 'native':
                return new native_js_1.NativeAdapter(this.logger);
            case 'auto':
            ...
            default:
                throw new errors_js_1.TerminalError(`Unknown terminal type: ${this.config.type}`);
        }
    }
    isVSCodeEnvironment() {
