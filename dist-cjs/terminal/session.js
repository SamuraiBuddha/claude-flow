...
class TerminalSession {
    ...
    constructor(terminal, profile, commandTimeout, logger) {
        ...
        this.logger = logger;
        this.id = (0, helpers_js_1.generateId)('session');
    ...
    async executeCommand(command) {
        if (!this.initialized) {
            throw new errors_js_1.TerminalCommandError('Session not initialized');
        }
        if (!this.terminal.isAlive()) {
            throw new errors_js_1.TerminalCommandError('Terminal is not alive');
        }
        ...
        try {
            ...
            const result = await (0, helpers_js_1.timeout)(this.terminal.executeCommand(command), this.commandTimeout, `Command timeout after ${this.commandTimeout}ms`);
            ...
            throw new errors_js_1.TerminalCommandError('Command execution failed', {
                command,
                error,
            });
        }
    ...
    async performHealthCheck() {
        try {
            const result = await (0, helpers_js_1.timeout)(this.terminal.executeCommand('echo "HEALTH_CHECK_OK"'), 5000, 'Health check timeout');
