...
export interface AgentInfo {
  ...
  workload: number; // 0-1, current load percentage
...
export interface AssignmentConstraints {
  maxWorkload?: number; // Maximum workload per agent (0-1)
...
export class AgentAssignment extends EventEmitter {
  private agents: Map<string, AgentInfo> = new Map();
  private taskAssignments: Map<string, string> = new Map(); // taskId -> agentId
  private agentTaskQueues: Map<string, string[]> = new Map(); // agentId -> taskIds
  private taskAffinities: Map<string, Map<string, number>> = new Map(); // agentId -> (capability -> score)
  ...
  getAvailableAgents(): AgentInfo[] {
    return Array.from(this.agents.values()).filter(
      a => a.status !== 'overloaded' && a.status !== 'offline' && a.status !== 'error'
    );
  }
  ...
  assignTask(
    ...
    const estimatedCompletion = new Date(
      estimatedStart.getTime() + task.estimatedDuration * 60 * 1000
    );
  ...
  private findCandidateAgents(
    ...
    if (constraints.preferredAgentTypes && constraints.preferredAgentTypes.length > 0) {
      const preferred = candidates.filter(a =>
        constraints.preferredAgentTypes!.includes(a.type)
      );
    ...
    if (constraints.requiresCapabilities && constraints.requiresCapabilities.length > 0) {
      candidates = candidates.filter(a =>
        this.agentHasCapabilities(a, constraints.requiresCapabilities!)
      );
    }
   ...
   */
  private agentHasCapabilities(agent: AgentInfo, required: string[]): boolean {
    ...
    if (agent.capabilities.codeGeneration) agentCapabilities.add('codeGeneration');
    if (agent.capabilities.codeReview) agentCapabilities.add('codeReview');
    if (agent.capabilities.testing) agentCapabilities.add('testing');
    if (agent.capabilities.documentation) agentCapabilities.add('documentation');
    if (agent.capabilities.research) agentCapabilities.add('research');
    if (agent.capabilities.analysis) agentCapabilities.add('analysis');

    for (const cap of required) {
      if (!agentCapabilities.has(cap)) return false;
    }
   ...
   */
  private calculateCapabilityScore(agent: AgentInfo, task: ParsedTask): number {
    let score = 0;
    let maxScore = 0;
    ...
    // Tag matching
    maxScore += 30;
    const tagMatches = task.tags.filter(t =>
      agentKeywords.has(t.toLowerCase()) ||
      agent.specializations.includes(t.toLowerCase())
    ).length;
   ...
   */
  private getAgentKeywords(agent: AgentInfo): Set<string> {
   ...
   */
  private calculateAffinityScore(agentId: string, task: ParsedTask): number {
    const affinities = this.taskAffinities.get(agentId);
    if (!affinities) return 50; // Neutral score
   ...
   */
  private buildAffinity(agentId: string, task: ParsedTask): void {
   ...
   */
  private formatScoreReason(score: {
    total: number;
    breakdown: string[];
  }): string {
   ...
   */
  private calculateWorkload(agent: AgentInfo): number {
  ...
  rebalance(): RebalanceResult {
    ...
    const underloaded = agents.filter(
      a => a.workload < 0.3 && a.status !== 'offline' && a.status !== 'error'
    );
    ...
    for (const overloadedAgent of overloaded) {
      ...
      const movableTasks = queue.filter(
        taskId => !overloadedAgent.currentTasks.includes(taskId)
      );

      for (const taskId of movableTasks) {
        if (overloadedAgent.workload <= this.config.maxWorkloadThreshold) break;
    ...
    if (moved.length === 0) {
      recommendations.push(
        'Could not rebalance: all queued tasks are already running or no suitable targets.'
      );
    } else {
    ...
    if (avgWorkload > 0.7) {
      recommendations.push(
        `Average workload is high (${(avgWorkload * 100).toFixed(0)}%). Consider spawning additional agents.`
      );
    }
    ...
    if (idleAgents.length > agents.length * 0.3) {
      recommendations.push(
        `${idleAgents.length} agents are idle. Consider reducing agent pool or assigning more tasks.`
      );
    }
