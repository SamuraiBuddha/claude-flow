 ...
 */
export class ConstitutionalEnforcerAgent extends BaseAgent {
  private constitution: ConstitutionRule[] = [];
  private exceptions: Map<string, ConstitutionException> = new Map();
  private violationHistory: ConstitutionViolation[] = [];
   ...
   */
  private async readConstitution(task: TaskDefinition): Promise<any> {
   ...
   */
  private async checkGate(task: TaskDefinition): Promise<GateCheckResult> {
    ...
    const gateRules = this.constitution.filter(r =>
      r.category === 'process' || r.category === 'quality'
    );

    const blockers: ConstitutionViolation[] = [];
    ...
    const gateResult: GateCheckResult = {
      gate,
   ...
   */
  private async recordException(task: TaskDefinition): Promise<any> {
    ...
    const exception: ConstitutionException = {
   ...
   */
  private async auditViolations(task: TaskDefinition): Promise<any> {
   ...
   */
  private async performGeneralEnforcement(task: TaskDefinition): Promise<any> {
   ...
   */
  private getException(ruleId: string): ConstitutionException | undefined {
   ...
   */
  private isExceptionExpired(exception: ConstitutionException): boolean {
   ...
   */
  private generateRecommendations(violations: ConstitutionViolation[]): string[] {
   ...
   */
  private generateAuditRecommendations(violations: ConstitutionViolation[]): string[] {
    const recommendations: string[] = [];
    ...
    for (const [ruleId, count] of ruleViolationCounts) {
      if (count > 5) {
        ...
        recommendations.push(
          `Rule "${rule?.name || ruleId}" violated ${count} times - consider team training`
        );
      }
    }
 ...
 */
export const createConstitutionalEnforcerAgent = (
  id: string,
  ...
  return new ConstitutionalEnforcerAgent(
    id,
