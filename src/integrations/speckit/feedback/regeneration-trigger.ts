...
export interface Customization {
  ...
  path: string; // JSONPath or similar to locate the customization
  value: unknown;
  source: string; // File or component where customization was made
  timestamp: Date;
  reason?: string;
}
...
export interface RegenerationTriggerEvents {
  'spec:changed': { file: SpecFile; previousHash?: string };
  'regeneration:started': RegenerationJob;
  'regeneration:completed': RegenerationJob;
  'regeneration:failed': { job: RegenerationJob; error: Error };
  'cascade:started': { sourceJob: string; cascadedJobs: string[] };
  'cascade:completed': { sourceJob: string; cascadedJobs: string[] };
  'customization:preserved': Customization;
  'customization:conflict': { customization: Customization; newValue: unknown };
  'error': Error;
}
 ...
 */
export class RegenerationTrigger extends EventEmitter {
  private config: RegenerationTriggerConfig;
  private watchedFiles = new Map<string, SpecFile>();
  private customizations = new Map<string, Customization[]>();
  private activeJobs = new Map<string, RegenerationJob>();
  private jobQueue: RegenerationJob[] = [];
  private pollTimer?: ReturnType<typeof setInterval>;
  private debounceTimers = new Map<string, ReturnType<typeof setTimeout>>();
  private isWatching = false;
  private regenerators = new Map<string, (spec: SpecFile) => Promise<void>>();
   ...
   */
  async watchSpec(specPath?: string): Promise<void> {
   ...
   */
  async cascadeUpdates(
    sourceJob: RegenerationJob,
    options: {
      skipPlans?: boolean;
      skipTasks?: boolean;
    } = {}
  ): Promise<string[]> {
    const cascadedJobIds: string[] = [];
  ...
  registerRegenerator(
    type: 'spec' | 'plan' | 'task',
    regenerator: (spec: SpecFile) => Promise<void>
  ): void {
  ...
  async applyCustomizations(
    ...
    if (customizations.length === 0) return content;
   ...
   */
  private async scanWatchedPaths(): Promise<void> {
   ...
   * Scan a single file
   */
  private async scanFile(filePath: string): Promise<SpecFile> {
   ...
   */
  private handleFileChange(filePath: string, file: SpecFile, previousHash?: string): void {
  ...
  private async processNextJob(): Promise<void> {
    ...
    try {
      ...
      job.completedAt = new Date();
      this.emit('regeneration:failed', { job, error: error as Error });
    } finally {
   ...
   */
  private async reapplyCustomizations(job: RegenerationJob): Promise<void> {
    ...
    try {
      ...
      const customized = await this.applyCustomizations(
        job.specPath,
        parsed,
        job.preservedCustomizations
      );

      // Write back
      await fs.writeFile(outputPath, JSON.stringify(customized, null, 2));
    } catch (error) {
   ...
   */
  private async findDependentPlans(specPath: string): Promise<string[]> {
    const dependentPlans: string[] = [];
   ...
   */
  private async findDependentTasks(specPath: string): Promise<string[]> {
      ...
      try {
        ...
        for (const file of files) {
          ...
          for (const plan of plans) {
            const planName = path.basename(plan, path.extname(plan));
   ...
   */
  private getOutputPath(inputPath: string, type: string): string {
    const baseName = path.basename(inputPath);
    return path.join(this.config.outputDir, type, baseName);
  }
  ...
  private async loadCustomizations(): Promise<void> {
    try {
      const content = await fs.readFile(this.config.customizationsFile, 'utf-8');
      const data = JSON.parse(content);

      this.customizations.clear();
      for (const [source, customs] of Object.entries(data)) {
        this.customizations.set(source, (customs as any[]).map(c => ({
          ...c,
  ...
  private async saveCustomizations(): Promise<void> {
    const data: Record<string, Customization[]> = {};

    for (const [source, customs] of this.customizations) {
      data[source] = customs;
    }

    const dir = path.dirname(this.config.customizationsFile);
    await fs.mkdir(dir, { recursive: true });
    await fs.writeFile(
      this.config.customizationsFile,
   ...
   */
  private applyJsonPath(obj: any, jsonPath: string, value: unknown): any {
   ...
   */
  private getJsonPath(obj: any, jsonPath: string): unknown {
    const parts = jsonPath.split('.').filter(p => p);
    let current = obj;
