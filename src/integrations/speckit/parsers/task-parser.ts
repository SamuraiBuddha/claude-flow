...
export class TaskParser {
   ...
   */
  public parse(content: string, filePath: string): ParseResult<TaskList> {
    this.reset();
   ...
   */
  private parsePhases(sections: MarkdownSection[]): TaskPhase[] {
   ...
   */
  private parsePhase(section: MarkdownSection, index: number): TaskPhase | null {
    ...
    const priority = priorityMatch ? (priorityMatch[1].toUpperCase() as Priority) : undefined;
   ...
   */
  private createDefaultPhase(sections: MarkdownSection[]): TaskPhase | null {
    const tasks: Task[] = [];
   ...
   */
  private parseTasksFromContent(content: string, defaultUserStory?: string): Task[] {
    const tasks: Task[] = [];
    const lines = content.split('\n');
   ...
   */
  private parseTaskLine(line: string, defaultUserStory?: string): Task | null {
   ...
   */
  private inferTaskType(description: string, filePath?: string): TaskType {
    const lower = description.toLowerCase();

    // Test-related
    if (lower.includes('test') || filePath?.includes('test')) {
      if (lower.includes('contract')) return 'test';
  ...
  ): { dependencies: TaskDependency[]; dependencyGraph: DependencyEdge[] } {
    ...
    if (depSection) {
      ...
      for (const dep of additionalDeps) {
        ...
        for (const depId of dep.dependsOn) {
          const edgeExists = dependencyGraph.some(
            (e) => e.from === depId && e.to === dep.taskId
          );
          if (!edgeExists) {
   ...
   */
  private parseDependencySection(content: string): TaskDependency[] {
    const dependencies: TaskDependency[] = [];
  ...
  private inferPhaseDependencies(
    ...
    let lastNonParallelTask: Task | null = null;
    ...
    for (const task of tasks) {
      if (!task.parallelizable && lastNonParallelTask) {
        ...
        if (!hasExplicitDep) {
          ...
          const edgeExists = dependencyGraph.some(
            (e) => e.from === lastNonParallelTask!.taskId && e.to === task.taskId
          );
          if (!edgeExists) {
   ...
   */
  private parseExecutionStrategy(sections: MarkdownSection[]): ExecutionStrategy {
   ...
   */
  private determineMvpScope(tasks: Task[], phases: TaskPhase[]): string[] {
   ...
   */
  private validateTaskList(taskList: TaskList): void {
   ...
   */
  public getTopologicalOrder(taskList: TaskList): Task[] {
    const sorted: Task[] = [];
    const visited = new Set<string>();
   ...
   */
  public getParallelGroups(taskList: TaskList): Task[][] {
    const groups: Task[][] = [];
 ...
 */
export function parseTasks(content: string, filePath: string): ParseResult<TaskList> {
