...
program
  ...
  .action(async (projectPath: string = '.', options: { detailed?: boolean; output?: string }) => {
    try {
...
program
  ...
  .action(async (projectPath: string = '.', options: { strategy?: string; backup?: string; force?: boolean; dryRun?: boolean; preserveCustom?: boolean; skipValidation?: boolean }) => {
    try {
...
program
  ...
  .action(async (projectPath: string = '.', options: { backup?: string; timestamp?: string; force?: boolean }) => {
    try {
      const runner = new MigrationRunner({
        projectPath: path.resolve(projectPath),
        strategy: 'full',
...
program
  ...
  .option('-b, --backup <dir>', 'Backup directory', '.claude-backup')
  .action(async (projectPath: string = '.', options: { backup?: string }) => {
    try {
      const runner = new MigrationRunner({
        projectPath: path.resolve(projectPath),
        strategy: 'full',
